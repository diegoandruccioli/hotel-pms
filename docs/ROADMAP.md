...
| E24 | RBAC — `@PreAuthorize` su `GuestController`/`GuestPrivacySettingsController` | 🟠 Alta | 1-2gg | Nessuna | Trovato da `docs/COMPLIANCE_AUDIT_2026-08.md` §3 (aggiornamento 2026-08-04) — nessuna `@PreAuthorize` a livello endpoint, protezione delegata interamente alla whitelist per-prefisso del gateway; `GuestPrivacySettingsController` (retention policy hotel) condivide lo stesso gap. Fix a basso rischio/alto valore, va su `feature/secure-coding-hardening` |
