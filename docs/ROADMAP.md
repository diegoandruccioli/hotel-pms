...
| P10 | ~~Refactor file grossi (SRP) — `StayServiceImpl`, `Stays.tsx`, `SettingsModal.tsx`, `AdminUsers.tsx`, `StayGuestFieldSection.tsx`~~ | ⚠️ **Parziale** (corretto 2026-07-25 dopo audit; `AdminUsers.tsx`/`StayGuestFieldSection.tsx` risolti davvero 2026-08-06/07) | — | `SettingsModal.tsx` **davvero eliminata** (sostituita da `pages/Settings/`). `AdminUsers.tsx`: `CreateUserModal`/`ResetPasswordModal`/`UserRow` estratti in `pages/AdminUsers/` (commit `409cd5b`), file passa da 399 a 134 righe, un test file per componente estratto. `StayGuestFieldSection.tsx`: `LookupOptionItem`/`LookupAutocomplete`/`StatoSelect`/`ComuneAutocomplete`/`GuestFieldSection` estratti in file propri sotto `pages/Stays/`, file originale eliminato, consumer (`CheckInForm.tsx`, `WalkInCheckInForm.tsx`) e test aggiornati a importare da `./GuestFieldSection` — nessuna duplicazione residua, lint/typecheck/test verdi. `StayServiceImpl.java`: helper privati presenti, `CheckInContext` è file separato ma pre-esisteva da ADR-001 (non estratto da questo lavoro) — il file **non è mai stato ridotto**, anzi è cresciuto 543→591 righe nei commit successivi. `Stays.tsx`: l'estrazione di `AlloggiatiReportSection` è reale (129 righe, test propri) ma da allora è **ricresciuto a 472 righe** (~74% del guadagno perso). Debito residuo: `StayServiceImpl.java` (mai toccato) e `Stays.tsx` (ricresciuto) |
