...
| Affidabilità | 8.5/10 | Circuit breaker, saga checkIn, `@Version` Invoice, pgBackRest repo1 locale + repo2 Backblaze B2 cifrato, backup full e restore di prova verificati 2026-09-24 (P3 ✅) — no K8s HA (SPOF singolo host) |
...
| P3 | ~~Backup PostgreSQL automatizzato (pg_dump cron)~~ | ✅ **Fatto** (2026-09-24: superato da pgBackRest — repo1 locale + repo2 Backblaze B2 cifrato AES-256-CBC, `check` ok con WAL archiving su entrambi, backup full 62.5MB→8.5MB e restore di prova da B2 riuscito su directory scratch; nota storica sotto sulla fase pg_dump) | — | Container `db-backup` in `docker-compose.yml`: `pg_dumpall` ogni 24h (configurabile), gzip, retention 14gg, volume dedicato `postgres_backups`, nessuna porta host. Verificato con dump reale (240K, 46 CREATE TABLE/DATABASE). RPO 24h accettato esplicitamente, non PITR (`backup/DECISIONS.md §3.5`). Restano da fare: copia esterna cifrata automatica (S3/B2) — oggi il volume Docker resta single-host, single-disk, single point of failure fisico — e un test di restore end-to-end reale (piano di rifinitura Fase 6, item 4/6) |
...
| P10 | ~~Refactor file grossi (SRP) — `StayServiceImpl`, `Stays.tsx`, `SettingsModal.tsx`, `AdminUsers.tsx`, `StayGuestFieldSection.tsx`~~ | ✅ **Fatto** (corretto 2026-07-25 dopo audit sulla sovrastima originale; tutti e 5 i file chiusi davvero 2026-08-06/07) | — | `SettingsModal.tsx` eliminata (sostituita da `pages/Settings/`). `AdminUsers.tsx`: `CreateUserModal`/`ResetPasswordModal`/`UserRow` estratti in `pages/AdminUsers/` (commit `409cd5b`), 399→134 righe. `StayGuestFieldSection.tsx`: `LookupOptionItem`/`LookupAutocomplete`/`StatoSelect`/`ComuneAutocomplete`/`GuestFieldSection` estratti in file propri sotto `pages/Stays/`, file originale eliminato (commit `e2cbd47`). `Stays.tsx`: `StayRow`/`StayStatusChip`/`getStatusTone` estratti in `pages/Stays/`, 472→314 righe, firme dei componenti invariate, `Stays.test.tsx` non toccato (non referenziava i sotto-component direttamente) + un test colocato per componente estratto. `StayServiceImpl.java`: 5 collaboratori estratti nello stesso package (`StayCheckInValidator`, `StayBillingCoordinator`, `StayNotificationCoordinator`, `StayAlloggiatiCoordinator`, `StayReservationSync`, più l'utility `StayFailureReason`), 658→335 righe; `StayService` (contratto pubblico) invariato, propagazione transazionale invariata (collaboratori senza `@Transactional` proprio, joinano la transazione del chiamante). `StayServiceImplTest` (1323 righe/41 test): sostituito `@InjectMocks` con costruzione reale dei 5 collaboratori a partire dagli stessi 9 mock — zero corpi di test riscritti, tutte le verify sui client esterni restano valide due hop più in basso. Verificato con `./gradlew :frontdesk-service:build` (PMD/Checkstyle/SpotBugs zero-warning, jacoco ≥40%) e `npm run build`/`lint`/`knip` lato frontend — nessun riferimento residuo ai vecchi nomi in tutto il repo |
...
| P16 | Pentest agentico con `usestrix/strix` (source review + test dinamico su stack live) | 🟡 Media | 1-2gg + costo LLM variabile | Non ancora partito, solo pianificato (2026-08-11). Copertura diversa da CodeQL/Trivy/Dependabot già attivi: quelli sono statici (CVE note, pattern noti), strix è agente AI che esegue exploitation reale (IDOR, RBAC, auth bypass, business logic) — utile mirato su auth-service/RBAC dato lo storico T-FB-03/T-GST-07/T-BILL-07. Blocco attuale: nessuna `ANTHROPIC_API_KEY` configurata (Claude Pro/Max non copre chiamate API — servizi separati); alternative valutate: modello locale Ollama (gratis, qualità bassa) o ChatGPT Plus/Pro se disponibile (zona grigia ToS OpenAI) — **Copilot Pro Unibo non supportato, zero integrazione**. Da fare quando ripreso: isolare l'esecuzione su copia del docker-compose (project name separato, non toccare `hotel-pms_postgres_data`), `--scan-mode quick --max-budget 10` come primo giro di prova prima di scalare. Setup completo in memoria di sessione (`project_hotel_pms`) |
...
| C4 | Report KPI avanzati (RevPAR, ADR, GOPPAR, Occupancy) | 🟡 Alta | 3-5gg (solo GOPPAR) | Occupancy/ADR/RevPAR **fatto** (2026-09-07): `OwnerDashboard.tsx` aveva già `KpiTrendSection` (trend, granularità giorno/settimana/mese); questa sessione ha aggiunto occupancy/ADR/RevPAR di oggi alla Dashboard principale (`OwnerSummarySection.tsx`, riuso `useKpiReport`) e badge delta +/-N% vs periodo precedente sulle 3 card finanziarie di OwnerDashboard (client-side, nessun `compareWith` sul backend). **GOPPAR resta non fatto** — serve un modello di costo operativo che oggi non esiste nel dominio |
...
| C12 | ~~Ricerca e paginazione server-side (Guests, Billing, Reservations)~~ | ✅ **Fatto** | — | **Guests** (`81abdc8`): `getAllGuests()` teneva solo `.content` di un endpoint già paginato (default 20), mostrando sempre e solo i primi 20 ospiti senza modo di vederne altri; ricerca filtrava lato client dentro quella slice troncata. **Billing** (`4e84fbb`): nuovo `GET /invoices/search` combinabile (status, intervallo date, query testuale su invoiceNumber o nome/email ospite via cross-service call a guest-service). **Reservations** (`acc362b`): nuovo `GET /reservations/search` combinabile (upcomingOnly, query su nome/email ospite); scope inizialmente limitato alla sola lista tabellare, planning-board resta su `getAllReservations` invariato (serve l'intero set per il calendario) — **aggiornamento 2026-09-07**: la Dashboard principale ora usa anche `searchReservations` (esteso con `dateFrom`/`dateTo`/`status`) per il pannello arrivi/partenze del front-desk, non più solo `getAllReservations`. Pattern comune ai 3: cross-service guest-name resolution via `searchGuests`+`getGuestsBatch`, mai una query N+1. |
...
| E17 | ROSS1000 — rilevazione statistica turistica regionale (ISTAT/SISTAN) | 🟢 Bassa | 3-5gg | Cliente reale in una regione coperta | Obbligo di legge (D.Lgs. 322/1989 art. 7, sanzioni fino a €2.500/mese) ma copre solo ~13/20 regioni italiane via piattaforma GIES (Piemonte, Veneto, Emilia-Romagna, Marche, Lombardia, Calabria, Sardegna, Liguria, Basilicata, Lazio, Molise, Abruzzo, Toscana-Firenze/Pistoia/Prato) — le altre regioni usano sistemi diversi non ancora ricercati. Oggi l'adempimento manuale via portale regionale resta legale e sufficiente. Protocollo verificato: SOAP, un WSDL pubblico per regione (`.../ws/checkinV2?wsdl`), autenticazione HTTP Basic, payload giornaliero (apertura/camere occupate-disponibili/letti + arrivi/partenze/prenotazioni/rettifiche idempotenti) — riusa le stesse tabelle Comuni/Nazioni/TipoAlloggiato Polizia di Stato già implementate per Alloggiati Web (`AlloggiatiCsvParser` + repository). Mancano 6 campi dominio (tipoturismo, mezzotrasporto, canaleprenotazione, titolostudio, professione, esenzioneimposta) in Guest/Reservation. **Non implementare finché non c'è un cliente pagante in una delle regioni coperte che lo richiede esplicitamente** — verdetto LLM Council 2026-06-22, **riconfermato 2026-09-05** (piano pilota Punto 5, nessun fatto nuovo): costo di opportunità pre-revenue troppo alto, copertura regionale parziale, nessuna libreria terza disponibile (stesso caso Alloggiati: stub generabile da WSDL reale invece di XML a mano, quando attivato). Adempimento manuale via portale regionale resta a carico dell'hotel |
| E18 | ~~Imposta di soggiorno (tassa di soggiorno comunale)~~ | ✅ **Fatto** | — | — | Trovato da `docs/COMPLIANCE_AUDIT_2026-08.md` §4. Implementato su `feature/imposta-di-soggiorno`: `ChargeType.CITY_TAX` (billing-service) fuori campo IVA via `Natura` FatturaPA N1 art. 15 DPR 633/1972 (da confermare col commercialista prima del go-live — costante isolata, `e19d59e`); nuovo package `citytax` (frontdesk-service) con `HotelCategoryHistory`/`CityTaxRate`/`CityTaxAssessment` — tariffa per (hotel, comune, categoria/stelle) con storico categoria, `EXCLUDE USING gist` per non-overlap, snapshot immutabile per audit fiscale (`700cb6d`); calcolo automatico postato al check-in insieme a `ROOM_NIGHT` con guardia di idempotenza per-charge indipendente (una CITY_TAX fallita non ripubblica la camera al retry, `4ccf744`); frontend display (`2961fd3`) e UI amministrazione tariffe/categoria su `/settings/city-tax`, ADMIN/OWNER (`1a47ad2`). ISTAT/ROSS1000 (E17) resta rinviato, invariato — nessun cliente pagante lo richiede oggi. **Da fare prima di inserire tariffe reali**: confermare il codice Natura (N1 vs N2.2) e l'importo esatto della delibera comunale con un commercialista |
| E19 | Corrispettivi telematici (Horeca) | 🔴 Alta | Da definire con un commercialista prima di stimare | Nessuna | Trovato da `docs/COMPLIANCE_AUDIT_2026-08.md` §5 — **obbligo di legge già in vigore dal 2026-01-01** per l'intero settore Horeca (hotel+ristorazione), non un gap futuro. Prima verificare con un commercialista se/come il pernottamento stesso vi ricade (oggi il sistema non ha comunque un caso d'uso "vendita F&B diretta a non alloggiato", `stayId` è `NOT NULL` su ogni ordine) — poi eventualmente implementare collegamento POS↔registratore telematico via portale "Fatture e Corrispettivi" AE. **Piano pilota Punto 5 (2026-09-05)**: nessuna integrazione nel pilota, solo valutazione scritta — vedi `backup/DECISIONS.md` ADR-006, domande per il commercialista in `docs/legal/CORRISPETTIVI_TELEMATICI_COMMERCIALISTA_BRIEF.md`, banner di avviso in `/billing` (`VITE_PILOT_MODE`). **Fix 2026-09-12** (`2d183c7`): `InvoiceSequence` condivideva un solo contatore fiscale tra `FATTURA`/`RICEVUTA` per hotel+anno — risolto con un prefisso `PILOT/` (nuovo flag `billing.pilot-mode`/`BILLING_PILOT_MODE`) su ogni numero generato durante il pilota, indipendente dal tipo documento; il sezionale separato per tipo si è rivelato non praticabile (il tipo documento non è noto al momento in cui il numero viene assegnato — vedi ADR-006). **Ricerca di mercato 2026-09-13**: come i PMS commerciali implementano i corrispettivi telematici (RT fisico vs soluzione software certificata via API) e perché nessuno dei due modelli va costruito ora — vedi `docs/legal/CORRISPETTIVI_TELEMATICI_MARKET_OPTIONS.md` |
...
| E22 | ~~GDPR — retention `StayGuest` (frontdesk-service)~~ | ✅ **Fatto** | — | — | Trovato da `docs/COMPLIANCE_AUDIT_2026-08.md` §3 — il job notturno di anonimizzazione copriva solo `guest-service.Guest`; `StayGuest` conservava per sempre una copia indipendente della PII Alloggiati per ogni soggiorno storico. Risolto come T-STAY-08 (`fe189c76`, `feature/secure-coding-hardening`): nuovo `StayGuestRetentionJobServiceImpl` (cron 02:30), stessa doppia garanzia TULPS(5y)/FISCALE(10y) di `GuestRetentionJobServiceImpl` — FISCALE via nuovo endpoint billing-service `GET /invoices/stay/{stayId}/last-date`, che considera anche i master folio di gruppo (Punto 4) |
| E23 | ~~GDPR Art. 32 — cifratura a riposo `documentNumber`~~ | ✅ **Fatto** | — | — | Trovato da `docs/COMPLIANCE_AUDIT_2026-08.md` §3 — in chiaro in `guest-service.IdentityDocument` e `frontdesk-service.StayGuest`. Risolto come GAP-28 (`f20c5904`, `feature/secure-coding-hardening`): due coppie encryptor+`AttributeConverter` JPA indipendenti (chiave/salt per servizio), stesso `Encryptors.delux` AES-256-GCM già in uso per le credenziali Alloggiati (P8). Nessuna migrazione dati (solo dati di test/pilota oggi) — solo widening schema (`V10`/`V26`) e rimozione dell'indice ormai morto su `identity_documents.document_number` |
