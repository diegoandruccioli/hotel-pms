...
\textbf{Commit}: \texttt{f177e36} — branch \texttt{feature/secure-coding-hardening}
...
\begin{securebox}{BOM property override su tutti gli 8 microservizi}
Il plugin \texttt{io.spring.dependency-management} di Spring Boot espone le
proprietà \texttt{commons-fileupload.version} e \texttt{commons-io.version}
come punti di override controllati. Aggiungendo le due righe al blocco
\texttt{ext} di ogni \texttt{build.gradle.kts}:
...
l'intero grafo di dipendenze transitivo viene allineato alle versioni patchate
senza modificare il BOM di Spring Boot né le dipendenze degli altri moduli.
Questo approccio è identico a quello adottato per Netty in DEP-CVE-01 e
garantisce la reversibilità: rimuovere le due righe ripristina i valori del BOM.
\end{securebox}
