...
\paragraph{Follow-up non risolto in questo commit \threat{T-AUTH-15}}
La stessa revisione di sicurezza ha isolato un secondo problema, reale ma
volutamente fuori scope qui: \texttt{LoginAttemptServiceImpl.attemptsKey}/
\texttt{lockKey} concatenano \texttt{clientIp} grezzo nella chiave Redis
(\texttt{login:fail:<username>:<clientIp>}) --- un \texttt{clientIp} nullo
produce silenziosamente la chiave letterale \texttt{...\string:null} (concatenazione
Java, nessuna eccezione), collassando il binding per-IP di T-AUTH-12 in un
lockout puramente per-username per chiunque ometta l'header. Tracciato in
\texttt{THREAT\_MODEL.md} come \textbf{\textcolor{warnyellow}{DA ANALIZZARE}}, nessun fix applicato.
