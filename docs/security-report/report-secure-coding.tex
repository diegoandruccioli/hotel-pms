...
\paragraph{Piano di mitigazione raccomandato}

\begin{enumerate}
  \item \textbf{Campo consenso}: aggiungere \texttt{gdprConsentDate LocalDate}
        e \texttt{gdprConsentIp String} a \texttt{Guest.java} (nuova migration
        Flyway); valorizzare al momento della creazione del profilo.
  \item \textbf{Retention job}: scheduled task notturno
        (\texttt{@Scheduled(cron = "0 2 * * *")}) che interroga tutti i
        \texttt{Guest} con \texttt{lastStayDate < now() - retentionYears}
        e li anonimizza: pseudo-nome, campi PII a \texttt{null}, cascade
        delete fisica degli \texttt{IdentityDocument}.
  \item \textbf{Hard-delete endpoint}: \texttt{DELETE /api/v1/guests/\{id\}}
        che verifica la presenza di vincoli legali attivi prima di procedere;
        in caso di blocco restituisce \texttt{HTTP 451 Unavailable For Legal
        Reasons} con payload che indica la data di sblocco.
  \item \textbf{Configurabilità per hotel}: periodo di retention come
        parametro in \texttt{hotel\_settings} (default 5 anni), gestibile
        dall'admin dalla stessa \texttt{SettingsModal} già implementata.
\end{enumerate}

\paragraph{Stato}
\textbf{\textcolor{vulnred}{🔴 APERTO --- non implementato.}}
La minaccia è identificata e classificata; il piano di mitigazione è definito
ma non ancora realizzato. La struttura dati attuale (\texttt{IdentityDocument}
separato, \texttt{hotel\_settings} disponibile) è già predisposta per
l'implementazione futura senza modifiche architetturali significative.
...
(Loki + Grafana). La superficie di attacco residua si riduce al CVE-2026-22739
del config-service (path traversal, soppresso con mitigazioni compensative documentate)
e all'upgrade pianificato a Spring Boot 3.5.x.
...
\owasp{A04} & Insecure Design
  & T-RES-01 (optimistic lock), T-STAY-01 (state allowlist),
    ...
    T-GST-04 (mass-assignment), T-GST-05 (GDPR retention \textcolor{vulnred}{$\bullet$ aperto}) \\
...
\owasp{A06} & Vulnerable and Outdated Components
  & DEP-CVE-01 (Netty 4.1.132, Gateway 4.2.6),
    ...
    DEP-CVE-03 (config-server 4.2.0 CVE-2026-22739, rischio residuale mitigato) \\
\owasp{A07} & Identification \& Authentication Failures
