...
% Definizione linguaggio YAML (non incluso in tutte le versioni di listings)
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Cambio password obbligatorio non applicato lato server \threat{T-AUTH-06}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante un giro di navigazione E2E reale con Playwright (non un pentest
mirato --- \texttt{docs/LIVE\_E2E\_AUDIT\_2026-07.md}, BUG-5). Un utente creato da
un ADMIN o sottoposto a reset password riceve \texttt{mustChangePassword=true}:
il frontend reindirizza correttamente a \texttt{/settings/password} al login, ma
quel reindirizzamento è l'\emph{unico} controllo esistente. Navigando direttamente
a un'altra URL, o chiamando l'API direttamente con le credenziali provvisorie
valide, ogni endpoint autenticato rispondeva normalmente: il claim non esisteva
nel JWT (solo colonna \texttt{must\_change\_password} in DB, letta esclusivamente
da \texttt{GET /auth/me}) e nessun filtro server-side lo controllava mai. La
rotazione forzata della password era quindi puramente cosmetica.
Classificato OWASP A01 (Broken Access Control) / A07 (Identification \&
Authentication Failures).

\begin{insecurebox}[title={Codice Vulnerabile --- enforcement assente (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: mustChangePassword non e' nel JWT, nessun filtro lo controlla mai
public String generateToken(String username, Role role, UUID hotelId, int tokenVersion) {
    Map<String, Object> claims = new HashMap<>();
    claims.put("role", role.name());
    claims.put("hotelId", hotelId.toString());
    ...
    return buildToken(claims, username, jwtExpiration);
    // credenziali provvisorie valide su QUALUNQUE endpoint autenticato
}
\end{lstlisting}
\end{insecurebox}

\paragraph{Mitigazione applicata}
\texttt{mustChangePassword} è ora un claim JWT firmato, aggiunto a
\texttt{JwtService.generateToken}/\texttt{generateRefreshToken} e popolato su
tutti e 4 i percorsi che emettono token: \texttt{register}, \texttt{login},
\texttt{refresh} (propaga lo stato corrente dell'utente) e
\texttt{changePassword} (emette sempre \texttt{false}, coerente col reset appena
avvenuto --- l'utente non resta bloccato dopo aver adempiuto). L'\texttt{AuthenticationFilter}
del gateway --- unico punto che copre tutte le route business (billing/guest/
frontdesk/fb) e \texttt{/api/v1/auth/users/**} --- rifiuta con \texttt{403
PASSWORD\_CHANGE\_REQUIRED} qualunque richiesta il cui claim sia \texttt{true} e
il cui path non sia nell'allow-list (\texttt{/api/v1/auth/change-password},
\texttt{/me}, \texttt{/logout}, \texttt{/refresh}); il controllo gira
\emph{prima} della RBAC, così nemmeno un ruolo ADMIN può bypassare l'obbligo
sfruttando i propri privilegi. Nota architetturale: le route pubbliche di
auth-service (login/refresh/change-password/me/logout) non passano comunque da
\texttt{AuthenticationFilter} (nessun filtro su quella route in
\texttt{api-gateway.yml}, \texttt{permitAll} per design) --- l'allow-list è
quindi un backstop difensivo, non il gate primario per quei quattro path.

\begin{securebox}[title={Codice Sicuro --- AuthenticationFilter.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: bloccato prima della RBAC, indipendentemente dal ruolo
...
\paragraph{Test}
\texttt{AuthServiceImplTest.loginPropagatesMustChangePasswordIntoJwtClaims}
verifica che il claim raggiunga \texttt{JwtService} con valore \texttt{true};
\texttt{changePasswordClearsMustChangePasswordEvenWhenItWasTrue} verifica che il
token emesso subito dopo il cambio password porti \texttt{false} (nessun
lockout residuo). Nuovo gruppo \texttt{MustChangePasswordTests} in
\texttt{AuthenticationFilterTest}: blocco di un endpoint business con
\texttt{mustChangePassword=true} sia per RECEPTIONIST sia per ADMIN (il ruolo
non basta a bypassare), richiesta permessa quando il claim è assente/\texttt{false}
(retro-compatibilità con token pre-esistenti), e verifica esplicita che un path
in allow-list resti raggiungibile nonostante il claim \texttt{true}.

\paragraph{Riferimento} Commit: \commitref{a82ce14}{fix(security): T-AUTH-06 --- mustChangePassword enforced server-side via JWT claim}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Riattivazione utente disattivato sempre fallita \threat{T-AUTH-07}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante un audit E2E reale (\texttt{docs/LIVE\_E2E\_AUDIT\_2026-07.md},
BUG-12). Il bottone ``Activate'' su un utente disattivato in Admin Users falliva
**sempre** con 404. \texttt{UserManagementServiceImpl.activateUser} cercava il
target con \texttt{userRepository.findById(targetUserId)}, ma \texttt{UserAccount}
porta \texttt{@SQLRestriction("active = true")} a livello di classe --- un
filtro Hibernate iniettato in \emph{ogni} query generata per l'entità,
comprese le query HQL-derivate ereditate come \texttt{findById}, non solo le
query JPQL custom. Un utente con \texttt{active=false} non veniva quindi mai
trovato, e la funzione simmetrica di ``Deactivate'' --- azione di primo
livello, non nascosta, presente su ogni riga della tabella --- non aveva alcun
percorso di ritorno da UI. Il commento nel codice originale (``Must query
including inactive --- use raw query bypassing @SQLRestriction'') descriveva
l'intento corretto ma il codice sottostante non lo implementava mai.
Classificato OWASP A04 --- Insecure Design (il pattern
\texttt{@SQLRestriction} + query ereditate non è mai stato validato end-to-end
per il caso ``recupera includendo gli inattivi'', un gap di design più che
un'iniezione di vulnerabilità).

\begin{insecurebox}[title={Codice Vulnerabile --- UserManagementServiceImpl.java (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: findById e' filtrato da @SQLRestriction("active = true")
// su UserAccount -- un utente disattivato non viene MAI trovato
public UserResponse activateUser(UUID hotelId, UUID targetUserId) {
    UserAccount target = userRepository.findById(targetUserId)
        .filter(u -> hotelId.equals(u.getHotelId()))
        .orElseThrow(() -> new NotFoundException("USER_NOT_FOUND"));  // sempre qui
...
\paragraph{Mitigazione applicata}
Nuovo metodo \texttt{findByIdAndHotelIdIncludingInactive} in
\texttt{UserAccountRepository}, basato su query \emph{nativa} anziché
JPQL/HQL: a differenza delle query derivate o annotate con HQL,
\texttt{@SQLRestriction} non viene iniettato in una query nativa scritta a
mano --- primo caso nel repository, nessun precedente esisteva da copiare.
Mantenuto hotel-scoped (predicato \texttt{hotel\_id} nella query) per non
trasformare il bypass in un leak cross-tenant; nome conforme alla regola
ArchUnit \texttt{TenantIsolationArchTest} (deve contenere \texttt{HotelId} nel
nome per essere riconosciuto come query tenant-scoped senza necessitare
un'esenzione esplicita).

\begin{securebox}[title={Codice Sicuro --- UserAccountRepository.java / UserManagementServiceImpl.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: query nativa, non soggetta a @SQLRestriction, hotel-scoped
@Query(value = "SELECT * FROM user_account WHERE id = :id AND hotel_id = :hotelId",
       nativeQuery = true)
Optional<UserAccount> findByIdAndHotelIdIncludingInactive(
        @Param("id") UUID id, @Param("hotelId") UUID hotelId);

// Service:
public UserResponse activateUser(UUID hotelId, UUID targetUserId) {
    UserAccount target = userRepository
        .findByIdAndHotelIdIncludingInactive(targetUserId, hotelId)
...
\paragraph{Test}
Il test esistente \texttt{activateUserShouldSetActiveTrue} mockava
\texttt{findById} direttamente con Mockito --- lo stub bypassava del tutto
Hibernate, quindi il test passava anche col bug realmente presente in
produzione: nessuna copertura di regressione poteva mai catturarlo. Riscritto
per mockare \texttt{findByIdAndHotelIdIncludingInactive}; il test
\texttt{activateUserShouldThrowWhenUserBelongsToAnotherHotel} riscritto per
riflettere che ora è la query stessa, hotel-scoped, a non restituire un
utente di un altro hotel (non più un filtro applicativo separato). Verificato
anche dal vivo via browser reale: utente disattivato → ``Activate'' → torna
``Active'' senza alcun intervento diretto sul database.

\paragraph{Riferimento} Commit: \commitref{[pending]}{fix(security): T-AUTH-07 --- activateUser bypasses @SQLRestriction via native query}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Refresh silenzioso escludeva /me, logout forzato ogni 15 minuti \threat{T-AUTH-08}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante un audit E2E reale (\texttt{docs/LIVE\_E2E\_AUDIT\_2026-07.md},
BUG-8). L'interceptor Axios responsabile del refresh silenzioso (T-AUTH-04)
escludeva esplicitamente \texttt{/api/v1/auth/me} dal meccanismo di
retry-dopo-refresh su un 401, con il commento ``Never attempt refresh on auth
endpoints themselves to prevent loops''. La motivazione è corretta per
\texttt{/login} (un tentativo di login fallito non deve innescare un
refresh), ma sovra-estesa a \texttt{/me}: un endpoint di sola lettura,
chiamato una sola volta al bootstrap dell'app (\texttt{App.tsx}, in un
\texttt{useEffect} al mount, non ad ogni navigazione client-side). L'access
token JWT dura 15 minuti; il refresh token dura 7 giorni ed è pienamente
valido. Ma se l'utente ricarica la pagina, apre un link diretto, o la scheda
viene ricaricata dal sistema operativo \emph{dopo} che i 15 minuti sono
scaduti, \texttt{GET /me} falliva con 401, l'interceptor non tentava il
refresh, e l'utente veniva reindirizzato a \texttt{/login} nonostante un
refresh token valido per altri giorni in un cookie httpOnly --- un logout
forzato che il meccanismo di refresh a 7 giorni esiste esplicitamente per
evitare. Classificato OWASP A07 --- Identification \& Authentication
Failures (nega al legittimo proprietario la persistenza di sessione che il
sistema di refresh è progettato per garantire).

\begin{insecurebox}[title={Codice Vulnerabile --- api.ts (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: /me escluso dal retry-dopo-refresh, nessuna ragione valida
// per un endpoint di sola lettura -- un 401-poi-refresh-poi-retry
// non puo' creare un loop
if (url.includes('/login') || url.includes('/me')) {
  return Promise.reject(error);  // niente refresh, logout forzato
}
\end{lstlisting}
\end{insecurebox}

\paragraph{Mitigazione applicata}
Rimossa l'esclusione di \texttt{/me} dalla condizione. Nessun rischio di loop:
il refresh stesso viene gestito da un branch dedicato subito dopo
(\texttt{url.includes('/refresh')}) che chiama \texttt{performLogout()} una
sola volta e non ritenta mai un secondo refresh; \texttt{/me} in fallimento
ora percorre lo stesso identico percorso già usato con successo da tutti gli
altri endpoint business (refresh silenzioso, poi retry della richiesta
originale).

\begin{securebox}[title={Codice Sicuro --- api.ts (fix)}]
\begin{lstlisting}[style=java]
// SICURO: solo /login e' escluso -- un login fallito non deve
// mai tentare un refresh, ma /me (sola lettura) si'
if (url.includes('/login')) {
  return Promise.reject(error);
}
if (url.includes('/refresh')) {
  performLogout();  // l'unico punto che chiama logout, mai un retry
  return Promise.reject(error);
}
// ... refresh silenzioso + retry, invariato per /me come per tutti
// gli altri endpoint
\end{lstlisting}
\end{securebox}

\paragraph{Test}
Il test \texttt{'rejects immediately on 401 from the /me endpoint without
attempting refresh'} codificava il comportamento buggato (nessuna copertura
di regressione lo avrebbe mai contraddetto). Riscritto in 2 test: uno verifica
che un 401 su \texttt{/me} innesca refresh silenzioso e retry (stesso pattern
già testato per un 401 generico su un endpoint business), l'altro che un
refresh fallito dopo un 401 su \texttt{/me} produce comunque un solo logout,
non un loop.

\paragraph{Riferimento} Commit: \commitref{[pending]}{fix(security): T-AUTH-08 --- silent refresh no longer excludes /me}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Cambio password volontario forzava un logout non necessario \threat{T-AUTH-09}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante un audit E2E reale (\texttt{docs/LIVE\_E2E\_AUDIT\_2026-07.md},
BUG-13). \texttt{AuthServiceImpl.changePassword} è specificamente progettato
per \emph{non} disconnettere l'utente (T-AUTH-04 residuo): incrementa
\texttt{tokenVersion}, aggiorna la cache Redis, ed emette un nuovo token pair
valido subito dopo --- proprio per garantire continuità di sessione dopo un
cambio password volontario, comportamento esplicitamente documentato nel
codice backend e in questo stesso report. Ma \texttt{SettingsPassword.tsx}
ignorava questo intento: dopo una risposta \texttt{200 OK} (i nuovi cookie
erano già stati impostati dal backend), chiamava comunque
\texttt{logout()}+redirect a \texttt{/login} incondizionatamente, buttando via
la sessione appena rinnovata. Non è un rischio di sicurezza in senso stretto
(il comportamento era per puro accidente ``più prudente'' del necessario), ma
contraddiceva esplicitamente un comportamento backend deliberatamente
progettato: l'utente doveva ridigitare da capo la password che aveva appena
finito di scrivere due volte nel form. Classificato OWASP A07 ---
Identification \& Authentication Failures (vanifica parzialmente la
persistenza di sessione che T-AUTH-04 residuo esiste per garantire).

\begin{insecurebox}[title={Codice Vulnerabile --- SettingsPassword.tsx (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: il backend ha appena emesso un token pair fresco apposta
// per NON disconnettere l'utente -- questo lo butta via comunque
await authService.changePassword({ currentPassword, newPassword });
addToast(t('password_changed_success'), 'success');
logout();
navigate('/login');  // re-login inutile, l'utente conosce gia' entrambe le password
\end{lstlisting}
\end{insecurebox}

\paragraph{Mitigazione applicata}
Rimossa la chiamata a \texttt{logout()}. Dopo il successo, la sessione viene
ri-sincronizzata (\texttt{authService.fetchMe()} +
\texttt{authStore.checkAuth(user)}) così un eventuale \texttt{mustChangePassword}
stale nello store client (se il cambio veniva dal flusso forzato al primo
login) viene azzerato al valore fresco \texttt{false} già restituito dal
backend, invece di restare appeso finché lo store non viene ricaricato altrove.
Navigazione post-successo: alla dashboard se il cambio veniva dal flusso
forzato (\texttt{mustChangePassword} in arrivo da \texttt{location.state}),
altrimenti indietro alla pagina di provenienza --- mai più a \texttt{/login}.
Se il refresh della sessione fallisce per una causa indipendente (es. un blip
di rete), la navigazione procede comunque: il cambio password stesso è già
riuscito, non ha senso bloccare l'utente su quello.

\begin{securebox}[title={Codice Sicuro --- SettingsPassword.tsx (fix)}]
\begin{lstlisting}[style=java]
// SICURO: nessun logout, la sessione fresca emessa dal backend resta valida
await authService.changePassword({ currentPassword, newPassword });
addToast(t('password_changed_success'), 'success');
try {
  const freshUser = await authService.fetchMe();
  checkAuth(freshUser);  // azzera mustChangePassword stale nello store
} catch {
  // il cambio password e' comunque riuscito, non blocca la navigazione
}
navigate(mustChangePassword ? '/' : -1);
\end{lstlisting}
\end{securebox}

\paragraph{Test}
Il test \texttt{'calls changePassword and logs out on success'} codificava
il comportamento buggato (asseriva che \texttt{logout} venisse chiamato).
Riscritto in 3 test: sessione mantenuta viva con `checkAuth` chiamato con
\texttt{mustChangePassword: false}, navigazione alla dashboard quando il
cambio veniva dal flusso forzato, e navigazione comunque completata se il
refresh post-cambio fallisce.

\paragraph{Riferimento} Commit: \commitref{[pending]}{fix(security): T-AUTH-09 --- voluntary password change preserves the session}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{NullPointerException su login fallito senza clientIp --- 401 diventava 500 \threat{T-AUTH-14}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato come effetto collaterale durante una sessione di verifica browser
end-to-end (2026-09), a seguito di un reset password manuale in ambiente
di sviluppo che aveva esposto per caso questo percorso di codice.
\texttt{AuthServiceImpl.sanitizeForLog(String value)} --- il metodo che
strippa CR/LF da un valore prima che finisca in un log, per prevenire
log forging da username o IP controllati dall'attaccante (CWE-117,
T-AUTH-05) --- non gestiva \texttt{null}:
\texttt{value.replaceAll("[\textbackslash r\textbackslash n]", "\_")} lanciava
\texttt{NullPointerException} se \texttt{value} era \texttt{null}.
\texttt{clientIp} è passato a \texttt{sanitizeForLog} in tre punti di
\texttt{login()} (\texttt{LOGIN\_BLOCKED}, \texttt{ACCOUNT\_LOCKED},
\texttt{LOGIN\_FAILED}) e arriva da
\texttt{@RequestHeader(value = "X-Client-IP", required = false)} in
\texttt{AuthController} --- \texttt{null} per qualunque richiesta che
raggiunge \texttt{auth-service} senza passare dal gateway (in sviluppo,
la porta \texttt{:8087} è esposta direttamente sull'host,
\texttt{docker-compose.yml}). Effetto: un login con password errata, o
su un account già bloccato, privo di quell'header, restituiva un
\texttt{500} grezzo invece del normale \texttt{401}/\texttt{ACCOUNT\_TEMPORARILY\_LOCKED}
--- \texttt{loginAttemptService.recordFailure}/\texttt{getLockedUntil}
girano comunque prima del logging (il conteggio del lockout non viene
bypassato), ma il chiamante riceve un'eccezione non gestita invece della
risposta attesa. Classificato OWASP A09 --- Security Logging \&
Monitoring Failures (il meccanismo di audit log introdotto per T-AUTH-05
è il punto di fallimento).

\begin{insecurebox}[title={Codice Vulnerabile --- AuthServiceImpl.java (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: nessun controllo null -- clientIp puo' essere null
// (header X-Client-IP opzionale, richiesta diretta senza gateway)
...
\paragraph{Mitigazione applicata}
\texttt{sanitizeForLog} reso null-safe: un valore \texttt{null} risolve al
letterale fisso \texttt{"unknown"} invece di propagare l'eccezione. Il
letterale non introduce alcun rischio di log forging (nessun contenuto
attaccante-controllato) né di information disclosure. Il fix non tocca
le chiamate a \texttt{loginAttemptService} (che ricevono \texttt{clientIp}
grezzo, separatamente, per il binding del lockout) né la difesa a tempo
costante di T-AUTH-13, entrambe verificate invariate in una revisione di
sicurezza dedicata prima del commit.

\begin{securebox}[title={Codice Sicuro --- AuthServiceImpl.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: null-safe, un login senza clientIp risolve comunque a 401/AccountLocked
private static String sanitizeForLog(final String value) {
...
\paragraph{Test}
Due nuovi test in \texttt{AuthServiceImplTest}:
\texttt{loginWithNullClientIpOnWrongPasswordThrowsCredentialsNotNpe} (password
errata + \texttt{clientIp == null} → \texttt{BadCredentialsException}, non NPE) e
\texttt{loginWithNullClientIpOnLockedAccountThrowsAccountLockedNotNpe} (stesso
scenario sul branch di lockout → \texttt{AccountLockedException}, non NPE).

\paragraph{Follow-up}
La stessa revisione di sicurezza ha isolato un secondo problema nello stesso
percorso di codice, sulla stessa causa (\texttt{clientIp} nullo su richiesta
diretta ad \texttt{auth-service} senza gateway) ma in un punto diverso ---
la chiave di lockout Redis anziché il logging. Risolto separatamente,
vedi \threat{T-AUTH-15} più sotto.

\paragraph{Riferimento} Commit: \commitref{d6b113a}{fix(security): T-AUTH-14 --- sanitizeForLog NPE on null clientIp}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Collasso della chiave di lockout Redis su clientIp nullo \threat{T-AUTH-15}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato dalla stessa revisione di sicurezza del fix precedente
(\threat{T-AUTH-14}), stessa causa di fondo ma un punto di codice diverso:
non il logging, ma la costruzione della chiave Redis del lockout.
\texttt{LoginAttemptServiceImpl.attemptsKey}/\texttt{lockKey} concatenavano
\texttt{clientIp} grezzo nella chiave (\texttt{login:fail:<username>:<clientIp>},
\texttt{login:lock:<username>:<clientIp>}) senza alcun controllo. Quando
\texttt{clientIp} è \texttt{null} --- qualunque richiesta che raggiunge
\texttt{auth-service} senza passare dal gateway, stesso scenario di
T-AUTH-14 --- la concatenazione Java produce silenziosamente la chiave
letterale \texttt{...\string:null}, senza eccezione. Effetto: ogni richiesta
priva dell'header \texttt{X-Client-IP} per un dato username finisce nello
\textbf{stesso} bucket di lockout, indipendentemente dalla sorgente reale
--- un attaccante che raggiunge la porta del servizio direttamente
(bypassando il gateway) fa collassare il binding per-\texttt{(username, ip)}
di T-AUTH-12 in un lockout puramente per-username, riaprendo esattamente
il DoS contro il vero proprietario dell'account che T-AUTH-12 era nato
per chiudere. Riprodotto dal vivo su un container \texttt{auth-service}
pre-fix in esecuzione: tre richieste dirette a \texttt{:8087} senza header
hanno scritto in Redis la chiave letterale \texttt{login:fail:admin:null}.
Classificato OWASP A07 --- Identification \& Authentication Failures.

\begin{insecurebox}[title={Codice Vulnerabile --- LoginAttemptServiceImpl.java (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: nessun controllo su username/clientIp -- un valore null
// produce silenziosamente la chiave letterale "...:null"
...
\paragraph{Mitigazione applicata}
Fix a due livelli, nessuno dei due sufficiente da solo. Primo livello,
\texttt{AuthController}: \texttt{clientIp} viene risolto prima di chiamare
il service --- l'header \texttt{X-Client-IP} quando presente e non vuoto,
altrimenti \texttt{HttpServletRequest.getRemoteAddr()} (indirizzo TCP del
peer, non un header, quindi non falsificabile dal client per eludere il
lockout), stesso fallback già applicato dal rate limiter del gateway sullo
stesso header (\texttt{RateLimiterConfig.resolveClientIp}). Secondo
livello, \texttt{LoginAttemptServiceImpl}: username/clientIp nulli o vuoti
vengono comunque normalizzati al letterale fisso \texttt{"unknown"} prima
di costruire la chiave, così il formato \texttt{...\string:null} resta
irraggiungibile anche da un futuro chiamante che reintroducesse l'errore.

\begin{securebox}[title={Codice Sicuro --- LoginAttemptServiceImpl.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: username/clientIp nulli o vuoti risolvono a "unknown",
// mai al letterale ":null"
private static String attemptsKey(final String username, final String clientIp) {
...
\paragraph{Hardening adiacente}
Trovato nello stesso file durante il fix, stessa radice (nessuna difesa
sui dati che tornano da Redis). \texttt{getLockedUntil} non proteggeva
\texttt{Long.parseLong(raw)}: un valore corrotto nella lock key faceva
uscire una \texttt{NumberFormatException} non gestita da \texttt{login()}
--- un \texttt{401} pulito diventava un \texttt{500}. Ora fallisce aperto:
la chiave corrotta viene cancellata e il metodo restituisce nessun lockout,
mentre il contatore dei tentativi (chiave separata) continua comunque a
contare. \texttt{recordFailure} impostava il TTL del contatore solo al
primo tentativo (\texttt{attempts == 1}): se quell'\texttt{expire()}
falliva per un disservizio momentaneo di Redis, il contatore restava senza
scadenza e l'account non si sarebbe più sbloccato da solo. Il TTL viene ora
riparato ad ogni tentativo in cui risulta assente
(\texttt{redisTemplate.getExpire}), non solo al primo.

\paragraph{Codice morto rimosso}
Il meccanismo di lockout pre-Redis (T-AUTH-02) teneva il contatore in
Postgres; è interamente superato da Redis ma i residui non erano mai letti
né scritti da codice applicativo: colonne \texttt{UserAccount.failedAttempts}/
\texttt{lockedUntil} e il metodo \texttt{UserAccountRepository.updateFailedAttempts}
(che portava una \texttt{@TenantScopeExempt} --- un'eccezione documentata
all'isolamento multi-tenant --- su un metodo che nessuno chiamava).
Migration \texttt{V8} droppa le colonne ora inutilizzate; \texttt{V2}
(già applicata negli ambienti esistenti) resta intatta per non rompere il
checksum Flyway.

\paragraph{Test}
Sei nuovi test in \texttt{LoginAttemptServiceImplTest}:
\texttt{clientIp} nullo o vuoto risolve al bucket \texttt{"unknown"}, mai al
letterale \texttt{":null"}; due IP diversi restano su bucket distinti
(non-regressione del binding T-AUTH-12); un valore corrotto nella lock key
fallisce aperto e cancella la chiave; un contatore senza TTL viene riparato
al tentativo successivo.

\paragraph{Verifica end-to-end}
Migration Flyway \texttt{V1}$\to$\texttt{V8} su database pulito e
\texttt{V8} da sola su una baseline \texttt{V1}--\texttt{V7} (che simula un
ambiente esistente) applicano entrambe senza errori di checksum. Una
richiesta diretta al container \texttt{auth-service} pre-fix in esecuzione
ha riprodotto dal vivo la chiave letterale \texttt{login:fail:admin:null}
in Redis, confermando la premessa della vulnerabilità sull'ambiente reale
di sviluppo.

\paragraph{Riferimento} Commit: \commitref{1c24f72}{fix(security): T-AUTH-15 --- resolve clientIp instead of collapsing lockout keys on null}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Log Injection sulla chiave di lockout corrotta \threat{T-AUTH-16}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato triagando gli alert CodeQL \texttt{java/log-injection} (CWE-117)
segnalati da GitHub sul branch. A differenza di \texttt{AuthServiceImpl}
(già sanitizzato tramite \texttt{sanitizeForLog} sin da T-AUTH-14/T-AUTH-05
--- i suoi tre alert erano falsi positivi, vedi nota SAST più sotto),
\texttt{LoginAttemptServiceImpl.getLockedUntil} loggava \texttt{lockKey}
(costruita da \texttt{username}/\texttt{clientIp} grezzi tramite
\texttt{normalize()}) e il valore Redis corrotto \texttt{raw} senza alcuno
strip di CR/LF, sul ramo che gestisce un valore di lock illeggibile
(\texttt{NumberFormatException} su \texttt{Long.parseLong}, introdotto dal
fix T-AUTH-15). Un \texttt{username} o \texttt{clientIp} contenente
\texttt{\textbackslash r\textbackslash n} poteva iniettare righe di audit
log false nell'output console/Loki --- ad esempio una falsa riga
\texttt{LOGIN\_SUCCESS} per un altro utente. Classificato OWASP A09.

\begin{insecurebox}[title={Codice Vulnerabile --- LoginAttemptServiceImpl.java (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: lockKey/raw non sanitizzati -- username/clientIp grezzi
// possono contenere CR/LF e falsificare righe di log successive
log.warn("[AUTH] LOCKOUT_KEY_CORRUPTED | key={} | value={}", lockKey, raw);
\end{lstlisting}
\end{insecurebox}

\paragraph{Mitigazione applicata}
Aggiunto un \texttt{sanitizeForLog} locale alla classe, stesso pattern già
in uso in \texttt{AuthServiceImpl} (\texttt{null} $\to$ \texttt{"unknown"},
altrimenti strip di \texttt{[\textbackslash r\textbackslash n]}), applicato
a entrambi gli argomenti del log.

\begin{securebox}[title={Codice Sicuro --- LoginAttemptServiceImpl.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: entrambi gli argomenti passano da sanitizeForLog prima del log
log.warn("[AUTH] LOCKOUT_KEY_CORRUPTED | key={} | value={}",
        sanitizeForLog(lockKey), sanitizeForLog(raw));
...
\paragraph{Riferimento} Commit: \commitref{3b62006}{fix(security): harden CWE-117 log injection across auth-service and gateway}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{CSV Formula Injection sul CsvWriter condiviso \threat{GAP-23}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Un nuovo \texttt{CsvWriter} condiviso (\texttt{common-web-lib}) stampa righe verso gli
export CSV di reservations/guests/invoices/owner-report basandosi su
\texttt{commons-csv}, che fornisce quoting/escaping RFC 4180 corretto ma nessuna
protezione contro la \emph{CSV formula injection} (CWE-1236): quando una cella il cui
valore stringificato comincia per \texttt{=}, \texttt{+}, \texttt{-}, \texttt{@}, tab o
CR viene aperta in Excel/LibreOffice/Google Sheets, lo spreadsheet la interpreta come
formula anziché come testo. Le colonne esportate includono campi liberi
riempiti dall'utente (nome ospite, note), quindi un valore come
\texttt{=cmd|'/c calc'!A1} salvato come nome ospite eseguirebbe quella formula al primo
apertura del file da parte dell'operatore. Trovato durante un self-review di sicurezza
mirato eseguito sulla classe stessa durante la sua implementazione, prima che un
qualunque endpoint di export la richiamasse in produzione: nessuna finestra di
sfruttabilità reale nel repository. La mappatura OWASP è \textbf{A03 --- Injection}.

\paragraph{Mitigazione applicata}
Ogni cella (incluso l'header) passa da un controllo sul primo carattere prima di
essere data in pasto a \texttt{commons-csv}; se coincide con uno dei caratteri
trigger, viene prefissata con un apice letterale (\texttt{'}), che ogni spreadsheet
maggiore interpreta come ``forza testo'' e non mostra nell'interfaccia:

\begin{insecurebox}[title={CsvWriter.java --- nessun controllo sul contenuto della cella (baseline)}]
\begin{lstlisting}[style=java]
...
\begin{securebox}[title={CsvWriter.java --- neutralizzazione dei caratteri-trigger (main)}]
\begin{lstlisting}[style=java]
...
La stessa neutralizzazione si applica alla riga di header in \texttt{CsvWriter.open()},
così un nome colonna non può veicolare la stessa iniezione.

\paragraph{Riferimento} Commit: \commitref{627194f}{feat(common-web-lib): shared CsvWriter with CSV formula injection mitigation}
...
\paragraph{GAP-10 --- L'ereditarietà silenziosa di nginx rompeva ogni download dell'app}
Gli header di sicurezza T-GW-03/T-FE-04 sopra (\texttt{X-Frame-Options},
CSP con \texttt{frame-ancestors}) erano dichiarati solo nel server block di
\texttt{frontend/nginx.conf}. La location \texttt{/api/} non definisce un proprio
blocco \texttt{add\_header}, quindi li eredita silenziosamente --- lo stesso
comportamento di ereditarietà di nginx già documentato nel commento del file per le
location \texttt{/} e \texttt{/assets/}, ma mai esteso a \texttt{/api/}. La
conseguenza non è solo un gap di hardening: fattura PDF, XML FatturaPA e preventivo
PDF si scaricano tutti tramite un \texttt{<iframe src="/api/...">} nascosto
same-origin, e una risposta con \texttt{X-Frame-Options: DENY} fa sì che il browser
rifiuti di caricarla in quell'iframe --- ogni download falliva silenziosamente,
senza errore visibile e senza file scaricato.

Scoperto durante un audit dal vivo con click reali sul bottone ``Scarica PDF''
(non solo apertura di modal o esecuzione dei test unitari, che mockano l'iframe e
non attraversano mai nginx reale), innescato da una segnalazione diretta
dell'utente. Isolata la causa confrontando i percorsi: \texttt{curl} e
\texttt{fetch()} diretti sullo stesso URL restituivano sempre \texttt{200} con
\texttt{Content-Disposition} corretto --- solo il caricamento reale nell'iframe
falliva, confermando che il blocco avveniva lato browser sull'header ereditato, non
lato server.

\begin{insecurebox}[title={frontend/nginx.conf --- \texttt{/api/} eredita gli header del server block (baseline)}]
...
location /api/ {
    ...
    # Nessun add_header qui: eredita X-Frame-Options: DENY e
    # frame-ancestors 'none' dal server block, bloccando l'iframe
    # same-origin che il download di PDF/XML usa deliberatamente.
}
\end{lstlisting}
\end{insecurebox}

\begin{securebox}[title={frontend/nginx.conf --- \texttt{/api/} dichiara i propri header (main)}]
\begin{lstlisting}[style=bash]
location /api/ {
    set $upstream       http://api-gateway:8080;
    proxy_pass         $upstream;
    ...
    add_header Content-Security-Policy
        "default-src 'self' blob:; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'; frame-ancestors 'self'; form-action 'self'; base-uri 'self'"
        always;
    add_header X-Content-Type-Options "nosniff" always;
...
\texttt{SAMEORIGIN}/\texttt{frame-ancestors 'self'} permette l'iframe same-origin
che l'app usa deliberatamente per i download, mantenendo bloccato l'embedding da
qualunque origine esterna --- la minaccia reale contro cui l'header esiste.
Verificato dal vivo dopo rebuild e redeploy del container \texttt{frontend}: un
click reale su ``Scarica PDF'' produce ora il file sul disco (25149 byte,
corrispondenti alla dimensione generata dal backend).

\paragraph{Riferimento} Commit: \commitref{41bedde}{fix(security): stop inheriting X-Frame-Options: DENY on /api/, it broke every download}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Denial of Service su Generazione PDF Fattura \threat{T-BILL-05}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
\texttt{ThymeleafPdfTemplateRenderer} (modulo condiviso \texttt{pdf-template-engine},
usato da billing-service per generare il PDF fattura) rileggeva e ri-subsettava i
font Noto Sans dal classpath ad \emph{ogni} chiamata a \texttt{render()} --- l'unico
punto di allocazione Metaspace per-richiesta in tutta la pipeline di generazione.
Nessun limite era imposto al numero di generazioni concorrenti o sequenziali: un
volume anche moderato di richieste (traffico legittimo, non uno scenario di attacco
mirato) esauriva progressivamente il Metaspace e crashava la JVM del servizio ---
CWE-400, uncontrolled resource consumption, classificabile OWASP A04:2021 ---
Insecure Design (assenza di un limite di capacità progettato, non una
misconfigurazione puntuale). Il sintomo è rimasto a lungo mascherato da un secondo
bug indipendente e concorrente (\texttt{GuestClient}, il client Feign
billing→guest-service, privo di configurazione discovery: ogni chiamata falliva
con ``No servers available'' e ogni fattura mostrava ``Unknown Guest''), che
riduceva drasticamente quanto la funzionalità venisse effettivamente esercitata
prima della sua riparazione.

\begin{insecurebox}[title={ThymeleafPdfTemplateRenderer.java --- font ricaricati ad ogni render() (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: ogni chiamata a render() rilegge e ri-subsetta i font dal classpath
public byte[] render(final String templateName, final Map<String, Object> model) {
    final PdfRendererBuilder builder = new PdfRendererBuilder();
    // font caricati per-richiesta: allocazione Metaspace non delimitata
    builder.useFont(() -> loadFontFromClasspath("NotoSans-Regular.ttf"), "Noto Sans");
    builder.useFont(() -> loadFontFromClasspath("NotoSans-Bold.ttf"), "Noto Sans", 700);
    // ... render ...
}
\end{lstlisting}
\end{insecurebox}
...
\paragraph{Mitigazione applicata}
\begin{enumerate}
    \item \textbf{Font caricati una sola volta} --- letti in campi statici
          all'inizializzazione della classe, non più ad ogni \texttt{render()}.
    \item \textbf{GuestClient pinnato a URL esplicito} --- nuova proprietà
          \texttt{application.config.guest-service-url} in config-service, stesso
          pattern già in uso per \texttt{HotelSettingsClient}/\texttt{frontdesk-service-url}
          (Eureka è disabilitato cluster-wide nel progetto, la discovery automatica
          non è mai stata disponibile).
    \item \textbf{Rete di sicurezza operativa} --- \texttt{MaxMetaspaceSize} alzato
          da 128m a 192m come margine addizionale, non come mitigazione primaria.
\end{enumerate}

\begin{securebox}[title={ThymeleafPdfTemplateRenderer.java --- font caricati una sola volta (fix)}]
\begin{lstlisting}[style=java]
// SICURO: font letti una sola volta, riusati su ogni render()
private static final FSSupplier<InputStream> NOTO_SANS_REGULAR =
        loadFontFromClasspath("NotoSans-Regular.ttf");
private static final FSSupplier<InputStream> NOTO_SANS_BOLD =
        loadFontFromClasspath("NotoSans-Bold.ttf");

public byte[] render(final String templateName, final Map<String, Object> model) {
    final PdfRendererBuilder builder = new PdfRendererBuilder();
    builder.useFont(NOTO_SANS_REGULAR, "Noto Sans");
    builder.useFont(NOTO_SANS_BOLD, "Noto Sans", 700);
    // ... render ...
}
\end{lstlisting}
\end{securebox}
...
\paragraph{Verifica}
Dal vivo: rebuild e restart di config-server e billing-service, 20 generazioni PDF
consecutive → 20×200, nome ospite reale risolto correttamente, nessun crash,
Metaspace stabile sotto il nuovo cap.

\paragraph{Riferimento} Commit: \commitref{c803390}{fix(billing): risolve crash OOM su generazione PDF fattura}
...
\paragraph{Riferimento} Commit: \commitref{6e5bc0e}{fix(security): T-STAY-06 --- cross-tenant data leak on check-in pre-fill}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{IDOR e Tampering Cross-Tenant sul Catalogo Room Type \threat{T-ROOM-02}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
La tabella \texttt{room\_types} risale alla baseline V1, precedente alla convenzione
\texttt{hotel\_id} adottata per l'isolamento multi-tenant, ed era rimasta priva della
colonna. Il gap era stato inizialmente registrato in GAP-6 come limite architetturale
accettato (``catalogo tipologie camera globale, non per-hotel''); un approfondimento
successivo ha invece dimostrato che si trattava di un IDOR reale e sfruttabile: qualunque
ADMIN/OWNER autenticato di un hotel, noto un UUID di room-type di un hotel diverso,
poteva leggerlo, modificarne prezzo/nome/capienza o cancellarlo (soft-delete) --- non
solo un filtro di lettura mancante, ma superficie di scrittura cross-tenant. Il vincolo
\texttt{UNIQUE(name)} globale impediva inoltre a due hotel indipendenti di usare entrambi
lo stesso nome di tipologia (es. ``Deluxe''). Stessa classe di T-BILL-04/T-STAY-06,
trovato con lo stesso metodo (verifica dei gap noti della regola ArchUnit di GAP-6).
Classificato OWASP A01 --- Broken Access Control.

\begin{insecurebox}[title={Codice Vulnerabile --- RoomTypeServiceImpl.java (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: nessun hotelId, room_types senza colonna hotel_id
public RoomTypeResponse getRoomTypeById(UUID id) {
    RoomType roomType = roomTypeRepository.findById(id)
        .orElseThrow(() -> new NotFoundException("ROOM_TYPE_NOT_FOUND"));
    return roomTypeMapper.toResponse(roomType);  // di QUALSIASI hotel
}

public RoomTypeResponse updateRoomType(UUID id, RoomTypeRequest request) {
    RoomType roomType = roomTypeRepository.findById(id)
        .orElseThrow(() -> new NotFoundException("ROOM_TYPE_NOT_FOUND"));
    // scrive su una room-type di un hotel arbitrario
    roomType.setName(request.name());
    roomType.setBasePrice(request.basePrice());
    return roomTypeMapper.toResponse(roomTypeRepository.saveAndFlush(roomType));
}
\end{lstlisting}
\end{insecurebox}

\paragraph{Mitigazione applicata}
Flyway V7 aggiunge \texttt{hotel\_id UUID NOT NULL} a \texttt{room\_types} (backfill
al placeholder esistente, poi \texttt{NOT NULL}) e sostituisce \texttt{UNIQUE(name)}
con \texttt{UNIQUE(hotel\_id, name)}. \texttt{RoomTypeRepository} guadagna
\texttt{findByIdAndHotelId}/\texttt{findAllByHotelId}; tutti i 5 metodi di
\texttt{RoomTypeService} richiedono ora \texttt{hotelId}, risolto da
\texttt{RoomTypeController.resolveHotelId()} (stesso pattern di \texttt{RoomController}),
mai dal client. Le chiavi della cache Spring sono state ri-namespaced per hotelId
(\texttt{\#hotelId + ':' + \#id} e \texttt{\#hotelId}) per evitare cache poisoning
cross-tenant. Fix connesso individuato nello stesso giro: \texttt{RoomServiceImpl}
chiamava \texttt{roomTypeRepository.findById(roomTypeId)} non scoped in \texttt{createRoom}/
\texttt{updateRoom} --- un consumer del repository, non catturato dalla regola ArchUnit
di GAP-6 che analizza solo i metodi dichiarati sul repository stesso, non i call-site dei
suoi metodi ereditati. Sostituito con \texttt{findByIdAndHotelId}. \texttt{RoomTypeRepository}
aggiunto ai \texttt{TENANT\_ROOT\_REPOSITORIES} di \texttt{TenantIsolationArchTest}.

\begin{securebox}[title={Codice Sicuro --- RoomTypeServiceImpl.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: hotelId propagato dal controller, mai dal client
public RoomTypeResponse getRoomTypeById(UUID id, UUID hotelId) {
    RoomType roomType = roomTypeRepository.findByIdAndHotelId(id, hotelId)
        .orElseThrow(() -> new NotFoundException("ROOM_TYPE_NOT_FOUND"));
    return roomTypeMapper.toResponse(roomType);
}

public RoomTypeResponse updateRoomType(UUID id, UUID hotelId, RoomTypeRequest request) {
    RoomType roomType = roomTypeRepository.findByIdAndHotelId(id, hotelId)
        .orElseThrow(() -> new NotFoundException("ROOM_TYPE_NOT_FOUND"));
    roomType.setName(request.name());
    roomType.setBasePrice(request.basePrice());
...
\paragraph{Test}
\texttt{RoomTypeServiceImplTest} estesa con 2 regressioni cross-hotel
(\texttt{testGetRoomTypeByIdBelongsToDifferentHotelReturnsNotFound},
\texttt{testUpdateRoomTypeBelongsToDifferentHotelReturnsNotFound});
\texttt{RoomTypeControllerTest} estesa con \texttt{SecurityContext} hotel-scoped
(pattern \texttt{RoomControllerTest}); nuovo \texttt{RoomTypeServiceIntegrationTest}
contro PostgreSQL reale (Testcontainers) valida la migration V7 end-to-end: due hotel
possono usare lo stesso nome di room-type, e nessuno dei tre metodi
(get/update/delete) espone o modifica una room-type di un hotel diverso.

\paragraph{Riferimento} Commit: \commitref{121e4d0}{fix(security): T-ROOM-02 --- cross-tenant IDOR and tampering on room types}

% ─────────────────────────────────────────────────────────────────────────────

\subsection{Bypass di Autenticazione sul Canale SSE Realtime via Collisione HandlerMapping \threat{T-GW-11}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato in security review prima del merge (non da un audit post-hoc): il
nuovo endpoint SSE \texttt{GET /api/v1/events/stream} (sincronizzazione
realtime dello stato camere/check-in/check-out, \texttt{api-gateway}, primo
uso di SSE nel backend) era raggiunto tramite una route
self-referenziale --- \texttt{uri: forward:/api/v1/events/stream} in
\texttt{api-gateway.yml} --- che mappava \texttt{EventStreamController} sulla
\textbf{stessa} path pubblica su cui la route stessa predica. L'intento era
che \texttt{forward:} facesse comunque passare la richiesta per
\texttt{AuthenticationFilter} e i quattro \texttt{GlobalFilter} del gateway,
esattamente come ogni endpoint proxato.

Non è così in questa applicazione: \texttt{DispatcherHandler} di Spring
interroga i bean \texttt{HandlerMapping} in ordine crescente di
\texttt{@Order} e delega al primo che produce un match.
\texttt{RequestMappingHandlerMapping} di WebFlux (che risolve i controller
\texttt{@GetMapping}, incluso questo) si registra all'ordine 0;
\texttt{RoutePredicateHandlerMapping} di Spring Cloud Gateway si registra
all'ordine 1. Poiché il controller era mappato sulla stessa path su cui la
route predicava, ogni richiesta diretta a quella path veniva risolta dal
primo mapping --- il controller stesso --- e la catena di filtri della route
non veniva mai raggiunta: \texttt{AuthenticationFilter} non veniva mai
eseguito. Un chiamante privo di qualunque autenticazione (nessun cookie JWT)
poteva inviare \texttt{GET /api/v1/events/stream} con un header
\texttt{X-Auth-Hotel} arbitrario e ricevere lo stream realtime di un hotel
qualsiasi. Non un rischio residuo o teorico: bypass completo di
autenticazione più IDOR cross-tenant sul canale, verificato riproducendo il
percorso di dispatch con \texttt{RequestMappingHandlerMapping} in isolamento.
Classificato OWASP A01 --- Broken Access Control.

\begin{insecurebox}[title={Codice Vulnerabile --- EventStreamController.java + api-gateway.yml (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: il controller e la route puntano alla STESSA path pubblica --
// RequestMappingHandlerMapping (order 0) intercetta la richiesta prima che
// RoutePredicateHandlerMapping (order 1) possa applicare AuthenticationFilter.
@GetMapping(path = "/api/v1/events/stream", ...)
public Flux<ServerSentEvent<RoomEvent>> stream(...) { ... }
\end{lstlisting}
\begin{lstlisting}[language=yaml]
- id: events-stream
  uri: forward:/api/v1/events/stream   # stessa path del predicate sotto!
  predicates:
    - Path=/api/v1/events/stream
  filters:
    - AuthenticationFilter               # mai eseguito da una richiesta diretta
\end{lstlisting}
\end{insecurebox}

\begin{securebox}[title={Codice Sicuro --- EventStreamController.java + api-gateway.yml (fix)}]
\begin{lstlisting}[style=java]
// SICURO: il controller vive su una path interna, mai esposta. Nessun
// @GetMapping risolve piu' /api/v1/events/stream, quindi solo
// RoutePredicateHandlerMapping (e quindi AuthenticationFilter) puo' risolverla.
public static final String INTERNAL_STREAM_PATH = "/local/events/stream";

@GetMapping(path = INTERNAL_STREAM_PATH,
        produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public Flux<ServerSentEvent<RoomEvent>> stream(
        @RequestHeader(value = "X-Auth-Hotel", required = false)
        String hotelHeader) {
    UUID hotelId = parseHotelId(hotelHeader); // 401 se assente/malformato
    return broadcaster.streamFor(hotelId)     // un solo Sinks.Many per hotelId
            .map(event -> ServerSentEvent.builder(event).build());
}
\end{lstlisting}
\begin{lstlisting}[language=yaml]
- id: events-stream
  uri: forward:/local/events/stream    # path DIVERSA dal predicate pubblico
  predicates:
    - Path=/api/v1/events/stream
  filters:
    - AuthenticationFilter               # ora l'unico modo di raggiungere il controller
\end{lstlisting}
\end{securebox}

\paragraph{Mitigazione applicata}
\begin{enumerate}
  \item Separazione strutturale dei due path: la path pubblica
    (\texttt{/api/v1/events/stream}, su cui la route predica) e la path
    interna a cui il controller è realmente mappato
    (\texttt{/local/events/stream}) non coincidono mai. Poiché nessun
    \texttt{@GetMapping} risolve più la path pubblica,
    \texttt{RequestMappingHandlerMapping} non può intercettarla --- resta
    l'unica strada \texttt{RoutePredicateHandlerMapping}, che applica sempre
    \texttt{AuthenticationFilter} prima di inoltrare.
  \item Nuovo \texttt{EventStreamControllerMappingTest}: asserisce via
    reflection che la path del \texttt{@GetMapping} non sia mai uguale alla
    path pubblica hardcoded nel test stesso (deliberatamente non derivata
    dalla stessa costante che una regressione modificherebbe) --- guardia di
    regressione per esattamente questa classe di bug.
  \item L'hotelId resta comunque letto esclusivamente dall'header
    \texttt{X-Auth-Hotel}, iniettato da \texttt{AuthenticationFilter} dal
    claim JWT già verificato, mai da un parametro di query o da un header non
    firmato che il client potrebbe manipolare direttamente.
  \item \texttt{RoomEventBroadcaster} mantiene un
    \texttt{ConcurrentHashMap<UUID, Sinks.Many<RoomEvent>>}, un sink per
    hotelId: anche assumendo un chiamante autenticato, un tenant può ottenere
    soltanto il \texttt{Flux} del proprio hotel.
  \item Il payload dell'evento (\texttt{RoomEvent}) è deliberatamente
    minimale --- solo tipo e timestamp --- il frontend invalida la cache
    React Query e rifà il fetch autenticato, invece di fidarsi di dati
    applicativi ricevuti sul canale push.
\end{enumerate}

\paragraph{Test}
\texttt{EventStreamControllerMappingTest} (nuovo, la regressione specifica di
questo finding): la path del controller non coincide mai con quella pubblica.
\texttt{EventStreamControllerTest}: 401 con header hotel assente o non-UUID;
un evento pubblicato per un hotelId arriva solo agli iscritti di quello stesso
hotelId, non a quelli di un hotelId diverso (test dedicato con due sink
paralleli). \texttt{RoomEventBroadcasterTest} verifica lo stesso isolamento a
livello di sink, indipendentemente dal layer HTTP.

\paragraph{Riferimento} Commit: \commitref{a2620fc}{feat(secure-coding-hardening): SSE realtime room-status sync (T-GW-11, T-GW-12)}

% ─────────────────────────────────────────────────────────────────────────────

\subsection{Verifica HMAC in Ingresso sul Gateway \threat{T-GW-12}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Anche T-GW-12 è una minaccia modellata proattivamente, non una vulnerabilità
trovata in codice esistente. Il nuovo endpoint \texttt{POST /internal/events/notify}
(\texttt{frontdesk-service} $\to$ \texttt{api-gateway}, chiamata interna che
innesca la pubblicazione di un evento sul canale SSE di T-GW-11) è il primo
caso in cui il gateway \emph{riceve}, e non solo produce, una chiamata interna
firmata HMAC. Il verificatore condiviso da tutti gli altri sei servizi,
\texttt{InternalAuthFilter} (\texttt{internal-auth-lib}, vedi T-GW-08), estende
\texttt{OncePerRequestFilter} ed è quindi legato allo stack servlet --- non
riusabile as-is su \texttt{api-gateway}, che è reattivo (WebFlux/Netty). Il
rischio: un verificatore reimplementato in modo indipendente potrebbe
divergere dall'algoritmo condiviso (finestra di tolleranza sul timestamp,
confronto non a tempo costante, nessun controllo anti-replay sul nonce),
indebolendo silenziosamente la garanzia che solo il gateway stesso possa
autenticare chiamate interne (T-GW-01/T-GW-08).

\begin{securebox}[title={Codice Sicuro --- InternalEventsAuthFilter.java}]
\begin{lstlisting}[style=java]
// SICURO: stesso algoritmo byte-per-byte di InternalAuthFilter, reimplementato
// per lo stack reattivo -- stesso payload, stessa finestra di tolleranza,
// stesso confronto a tempo costante, stesso meccanismo anti-replay.
private boolean isSignatureValid(String username, String role, String hotelId,
        String timestamp, String nonce, String signature) {
    String expected = computeHmac(username, role, hotelId, timestamp, nonce);
    return MessageDigest.isEqual(
            expected.getBytes(StandardCharsets.UTF_8),
            signature.getBytes(StandardCharsets.UTF_8));
}

// Nonce store reattivo: il gateway ha solo lo stack Redis reattivo,
// non quello bloccante da cui dipende RedisNonceStore.
private Mono<Boolean> claimNonce(String nonce) {
    return redisTemplate.opsForValue().setIfAbsent(
            "internal-auth:nonce:" + nonce, "1",
...
\paragraph{Mitigazione applicata}
\begin{enumerate}
  \item Payload HMAC identico a \texttt{InternalAuthFilter}/\texttt{AuthenticationFilter.computeHmac}:
    \texttt{HMAC-SHA256(secret, "user:role:hotelId:timestamp:nonce")}, esadecimale minuscolo.
  \item Finestra di tolleranza identica: $\pm$60 secondi (T-GW-08).
  \item Nonce store reattivo (\texttt{ReactiveStringRedisTemplate.setIfAbsent},
    semantica SETNX) con TTL 120s, stessa politica di \texttt{RedisNonceStore}
    ma senza condividerne il codice --- prima parte reattiva del backend,
    forzare un'astrazione comune servlet/reactive sarebbe stata
    over-engineering prematuro.
  \item Lato \texttt{frontdesk-service} (chiamante), zero codice nuovo di
    firma: \texttt{InternalFeignAuthInterceptor} è già registrato come bean
    globale in \texttt{FeignHeaderConfig} e firma automaticamente ogni nuovo
    client Feign, incluso \texttt{GatewayEventsClient}.
\end{enumerate}

\paragraph{Test}
\texttt{InternalEventsAuthFilterTest} replica l'intera matrice di
\texttt{InternalAuthFilterTest}: header mancanti, firma non valida, firma
calcolata per un hotelId diverso (isolamento tenant), timestamp scaduto,
replay dello stesso nonce rifiutato.

\paragraph{Riferimento} Commit: \commitref{a2620fc}{feat(secure-coding-hardening): SSE realtime room-status sync (T-GW-11, T-GW-12)}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Log Injection via Header X-Correlation-ID Non Validato \threat{T-GW-13}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante il triage degli alert CodeQL \texttt{java/log-injection} sul
branch: \texttt{CorrelationIdFilter.resolveCorrelationId} accettava l'header
\texttt{X-Correlation-ID} così come inviato dal client, senza alcuna
validazione né limite di lunghezza, quando presente e non vuoto. Il valore
veniva poi (1) propagato inalterato a ogni servizio downstream come header
HTTP, (2) riflesso sulla risposta al client, e (3) inserito nell'MDC SLF4J
da \texttt{InternalAuthFilter} (\texttt{MDC.put("correlationId", ...)},
condiviso da sei servizi) --- da cui ogni pattern Logback plain-text lo
rendeva letteralmente via \texttt{\%X\{correlationId:-\}}. Un header
contenente \texttt{\textbackslash r\textbackslash n} era quindi log
forging (CWE-117) iniettabile da \emph{qualunque} richiesta pubblica
attraverso il gateway, senza credenziali né bypass di alcun controllo ---
un vettore più ampio dei tre alert CodeQL su \texttt{AuthServiceImpl}
(entrambi falsi positivi, vedi nota SAST), perché non richiedeva neanche
un flusso di autenticazione per essere raggiunto. Non era coperto da alcun
threat ID esistente né da alcun alert automatico: CodeQL non analizza il
modulo \texttt{api-gateway} (WebFlux) con la stessa profondità dei servizi
servlet, e Trivy fa solo CVE scanning, non taint analysis del codice
applicativo. Classificato OWASP A09.

\begin{insecurebox}[title={Codice Vulnerabile --- CorrelationIdFilter.java (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: l'header client e' accettato verbatim, nessun limite di
// lunghezza o di alfabeto -- puo' contenere CR/LF
...
\paragraph{Mitigazione applicata}
Allowlist esplicita sul formato atteso di un correlation ID
(\texttt{\^{}[A-Za-z0-9\_-]\{1,64\}\$}); un header assente, vuoto, troppo
lungo o non conforme (incluso qualunque valore con CR/LF) genera un UUID
nuovo, esattamente come già avveniva per l'header assente --- nessun
cambio di comportamento per il traffico legittimo.

\begin{securebox}[title={Codice Sicuro --- CorrelationIdFilter.java (fix)}]
\begin{lstlisting}[style=java]
private static final Pattern VALID_CORRELATION_ID =
        Pattern.compile("^[A-Za-z0-9_-]{1,64}$");

// SICURO: solo un valore che rispetta l'allowlist viene propagato;
// qualunque altro caso (assente, vuoto, CRLF, troppo lungo) genera un UUID
private static String resolveCorrelationId(final ServerHttpRequest request) {
    final String existing = request.getHeaders().getFirst(CORRELATION_ID_HEADER);
...
\paragraph{Test}
Nuovo \texttt{CorrelationIdFilterTest}, quattro casi: header valido
preservato invariato; header contenente CR/LF sostituito da un UUID; header
oltre 64 caratteri sostituito da un UUID; header assente genera un UUID
(comportamento pre-esistente, verificato come non-regressione).
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Account ADMIN fixture con password nota spedito su ogni installazione \threat{T-AUTH-17}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante un audit di pilot-readiness (valutazione se il sistema
fosse pronto per il primo hotel pilota reale, non un pentest dedicato).
\texttt{V7\_\_seed\_second\_hotel\_admin\_for\_e2e\_tests.sql} era una
migration Flyway \emph{permanente}: girava incondizionatamente su
qualunque installazione che eseguisse le migration del servizio, incluso
un hotel reale in produzione, non solo l'ambiente di test in cui era
pensata per vivere. Creava un account \texttt{e2e-live-other-hotel-admin},
ruolo \texttt{ADMIN}, password nota (\texttt{"password"}, stesso hash
bcrypt del seed admin di default), \texttt{must\_change\_password=FALSE},
su un secondo tenant fittizio --- pensata solo come fixture per la suite
E2E live locale (\texttt{frontend/e2e-live/idor-cross-tenant-live.spec.ts}),
ma senza alcun gate di profilo/ambiente. Chiunque leggesse il codice
sorgente pubblico del repository otteneva le credenziali complete di un
account ADMIN funzionante su qualunque installazione reale. Classificato
OWASP A05 --- Security Misconfiguration (un default/credenziale nota
spedita per errore, non un bypass di un controllo di accesso esistente).

\begin{insecurebox}[title={V7\_\_seed\_second\_hotel\_admin\_for\_e2e\_tests.sql --- migration permanente (baseline)}]
\begin{lstlisting}[style=sql]
-- INSICURO: nessun gate di profilo/ambiente -- questa riga viene inserita
-- su OGNI installazione che applica le migration Flyway di auth-service
INSERT INTO user_account (
    id, username, password_hash, email, role, hotel_id,
...
\paragraph{Mitigazione applicata}
Flyway tratta le migration come immutabili una volta applicate (checksum
sul contenuto del file), quindi \texttt{V7} non è stata modificata --- è
stata annullata. Una nuova migration \texttt{V9} rimuove retroattivamente
la riga (\texttt{DELETE FROM user\_account WHERE username = ...}),
idempotente sui volumi dove \texttt{V7} non è mai girata. Il seed è ora
on-demand, mai parte del path Flyway: un nuovo \texttt{ApplicationRunner}
(\texttt{E2eFixtureSeeder}) annotato \texttt{@ConditionalOnProperty} lo
esegue solo se \texttt{auth.seed-e2e-fixtures=true}, valore risolto da
\texttt{AUTH\_SEED\_E2E\_FIXTURES}, mai settata né in
\texttt{docker-compose.yml} né in \texttt{docker-compose.prod.yml}. Un
developer che vuole eseguire la suite E2E live locale la attiva
esplicitamente sul proprio stack --- non è più un comportamento di
default su nessuna installazione.

\begin{securebox}[title={E2eFixtureSeeder.java --- seed opt-in, gated da env var (fix)}]
\begin{lstlisting}[style=java]
// SICURO: questo bean non esiste nemmeno se auth.seed-e2e-fixtures
// non e' esplicitamente true -- default false ovunque, mai settato
// in docker-compose.yml/docker-compose.prod.yml
@Component
...
public class E2eFixtureSeeder implements ApplicationRunner {
    ...
    public void run(final ApplicationArguments args) {
        log.warn("[AUTH] E2E_FIXTURE_SEED_ENABLED -- ... "
                + "Never enable this on an installation with real hotel data.");
...
\paragraph{Verifica dal vivo}
Non solo unit test: riavviato \texttt{auth-service} contro un volume
Postgres di sviluppo esistente che aveva già applicato \texttt{V7} (la riga
fixture era presente e attiva). Dopo il deploy della nuova immagine,
\texttt{V9} è girata e la riga è stata rimossa; il seed admin reale
(\texttt{must\_change\_password=true}) è risultato intatto. Verificato
anche che il commento di \texttt{V7} stesso documentasse in chiaro la
password nota --- ulteriore motivo per non modificarla ma limitarsi ad
annullarne l'effetto.

\paragraph{Riferimento} Commit: \commitref{5e7651a2}{fix(security): remove e2e fixture ADMIN account from production migration path}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Regressione della guardia legale GDPR TULPS/fiscale \threat{T-GST-06}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato dal secondo giro di test esplorativo dal vivo
(\texttt{docs/EXPLORATORY\_TEST\_2026-08.md}, Round 2 bug \#1). La guardia
legale già corretta con T-GST-05 --- che deve bloccare l'anonimizzazione
GDPR di un ospite finché sono attivi vincoli TULPS (5 anni) o fiscali
(10 anni) --- si era di nuovo rotta, con una tripla causa indipendente
accumulata su refactoring successivi (consolidamento ADR-001,
introduzione di \texttt{@EnableMethodSecurity}). Riprodotto dal vivo su
un ospite reale con due fatture \texttt{PAID} del 2026, ben dentro la
finestra fiscale decennale: \texttt{DELETE /guests/\{id\}} restituiva
\texttt{204~No~Content} invece del \texttt{451} atteso, distruggendo in
modo irreversibile l'identità dell'ospite nonostante l'obbligo di
conservazione fosse ancora attivo. Classificato OWASP A04:2025 ---
Insecure Design: tre difetti indipendenti che si compensavano a vicenda
nel non-far-scattare mai il blocco, nessuno dei quali un singolo bug
ovvio isolatamente.

\begin{enumerate}
  \item \textbf{Path Feign errato}: \texttt{BillingServiceClient.getLastInvoiceDate}
        chiamava \texttt{/api/v1/invoices/guest/\{guestId\}/last-invoice-date},
        ma la rotta reale del controller è \texttt{.../last-date} --- 404
        garantito ad ogni chiamata.
  \item \textbf{Metodi \texttt{final} sotto proxy CGLIB}: \texttt{InvoiceServiceImpl.getLastInvoiceDateForGuest}
        e \texttt{StayServiceImpl.getLastStayDateForGuest} erano gli unici
        due metodi \texttt{public final} nelle rispettive classi. Con
        \texttt{proxy-target-class=true} (introdotto da
        \texttt{@EnableMethodSecurity}, T-FB-03/GAP-8), Spring genera un
        proxy CGLIB per sottoclassamento: un metodo \texttt{final} non è
        overridabile dalla sottoclasse proxy, quindi la chiamata esegue il
        bytecode originale su un'istanza-guscio mai inizializzata via
        dependency injection --- \texttt{NullPointerException} garantita
        su \texttt{stayRepository}/\texttt{invoiceRepository}.
  \item \textbf{Fallback dichiarato fail-safe, di fatto fail-open}: il
        fallback del circuit breaker (Javadoc: ``fail-safe... block
        deletion'') ritorna \texttt{hasStays/hasInvoices=true} con data
        \texttt{null}. Ma la logica che lo consuma
        (\texttt{computeTulpsExpiry}/\texttt{computeFiscalExpiry} in
        \texttt{GuestServiceImpl}, \texttt{shouldAnonymise} in
        \texttt{GuestRetentionJobServiceImpl}) leggeva
        \texttt{lastStayDate()==null} come ``nessun vincolo'' --- esattamente
        l'esito opposto a quello dichiarato nel commento.
\end{enumerate}

\begin{insecurebox}[title={Codice Vulnerabile --- GuestServiceImpl.java (prima del fix)}]
\begin{lstlisting}[style=java]
// INSICURO: se il fallback ritorna hasStays=true con data null
// (dependent service degradato), tulpsExpiry resta null e il vincolo
// e' letto come "nessun blocco" -- l'opposto del fail-safe dichiarato
private static LocalDate computeTulpsExpiry(
        GuestLastStayClientResponse info, int retentionYears) {
    if (!info.hasStays() || info.lastStayDate() == null) {
        return null;   // <-- qui: indeterminato trattato come "libero"
    }
...
\paragraph{Mitigazione applicata}
Tre correzioni indipendenti, tutte necessarie contemporaneamente: (1)
path Feign allineato a \texttt{/last-date}; (2) \texttt{final} rimosso
da entrambi i metodi (con \texttt{/** \{@inheritDoc\} */} per soddisfare
la regola Checkstyle \texttt{DesignForExtension} sul metodo ora
overridabile); (3) \texttt{verifyLegalHolds} e \texttt{shouldAnonymise}
ora distinguono esplicitamente il caso indeterminato (\texttt{hasStays()=true}
con data \texttt{null}) dal caso ``nessun vincolo'' (\texttt{hasStays()=false}),
trattando il primo come bloccante.

\begin{securebox}[title={Codice Sicuro --- GuestServiceImpl.java (fix)}]
\begin{lstlisting}[style=java]
// SICURO: l'indeterminazione blocca, non libera
final boolean tulpsIndeterminate =
        stayInfo.hasStays() && stayInfo.lastStayDate() == null;
final boolean tulpsBlocked = tulpsIndeterminate
        || tulpsExpiry != null && !LocalDate.now().isAfter(tulpsExpiry);
// analogo per fiscalIndeterminate/fiscalBlocked; l'eccezione 451
// riporta un messaggio esplicito "unable to verify... blocked as
// a precaution" e unlocksAt=null quando la causa e' indeterminazione
\end{lstlisting}
\end{securebox}

\paragraph{Test}
4 nuovi test boundary-value: due sul guard sincrono
(\texttt{GuestServiceImplDeleteGuardTest.shouldThrow451WhenTulpsStatusIndeterminate}/
\texttt{...FiscalStatusIndeterminate}, riproducono esattamente la risposta
del fallback) e due sul job notturno
(\texttt{GuestRetentionJobServiceImplTest.shouldSkipGuestWhenTulpsStatusIndeterminate}/
\texttt{...FiscalStatusIndeterminate}, verificano \texttt{guestRepository.save}
mai invocato). Verificato dal vivo contro lo stack Docker reale: guest con
2 fatture \texttt{PAID} 2026 → \texttt{DELETE} → \texttt{451 LEGAL\_HOLD\_ACTIVE}
con \texttt{unlocksAt=2036-08-04}, \texttt{legalBasis=FISCAL}; guest senza
storico → \texttt{DELETE} → \texttt{204} invariato (nessuna regressione sul
percorso felice).

\paragraph{Riferimento} Commit: \commitref{4db09fc}{fix(security): T-GST-06 --- GDPR legal-hold guard fail-open regression (Feign path, final under CGLIB, fallback semantics)}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Retention GDPR mancante su StayGuest \threat{T-STAY-08}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato da \texttt{docs/COMPLIANCE\_AUDIT\_2026-08.md} \S3, gap gemello di
\threat{T-GST-05}: il job notturno di anonimizzazione GDPR
(\texttt{GuestRetentionJobServiceImpl}, guest-service) copre solo l'entità
\texttt{Guest}. \texttt{StayGuest} (frontdesk-service) conserva però una
copia indipendente della PII Alloggiati --- nome, data di nascita,
cittadinanza, numero documento --- per ogni soggiorno storico, popolata al
check-in e mai più toccata dopo. Anonimizzare \texttt{Guest} non anonimizza
\texttt{StayGuest}: un profilo ospite formalmente cancellato lasciava
intatta, a tempo indeterminato, la stessa PII duplicata sotto ogni suo
soggiorno passato. Classificato OWASP A04 --- Insecure Design, stessa
categoria di \threat{T-GST-05}: manca un intero meccanismo di protezione,
non un bug in uno esistente.

\paragraph{Mitigazione applicata}
Nuovo \texttt{StayGuestRetentionJobServiceImpl}, eseguito ogni notte alle
02:30, che replica la stessa doppia garanzia legale già validata per
\texttt{Guest}: il vincolo TULPS (5 anni) è verificato localmente da
\texttt{StayGuest.departureDate}, senza chiamata inter-servizio; il vincolo
FISCALE (10 anni) richiede invece un nuovo endpoint in billing-service,
\texttt{GET /api/v1/invoices/stay/\{stayId\}/last-date}, perché lo stesso
soggiorno può avere le sue spese trasferite al master folio di un gruppo
(Punto 4) --- il nuovo endpoint verifica sia il folio individuale dello stay
sia un eventuale master folio con un addebito instradato da quello stay
(\texttt{InvoiceCharge.routedFromStayId}), così il vincolo fiscale segue
sempre il documento che porta davvero l'importo, non necessariamente il
folio originale dello stay.

\begin{securebox}[title={StayGuestRetentionJobServiceImpl.java --- doppio vincolo legale (main)}]
\begin{lstlisting}[style=java]
// SICURO: TULPS locale, FISCALE via billing-service, fail-closed su indeterminazione
final LocalDate tulpsExpiry = latestDeparture.plusYears(TULPS_MIN_YEARS);
if (!LocalDate.now().isAfter(tulpsExpiry)) {
    return false; // vincolo TULPS ancora attivo
}
...
\paragraph{Dettaglio non ovvio}
A differenza di \texttt{Guest}, le colonne PII di \texttt{StayGuest}
(\texttt{gender}, \texttt{dateOfBirth}, \texttt{placeOfBirth},
\texttt{citizenship}) sono \texttt{NOT NULL} --- vincolo del tracciato
Alloggiati. L'anonimizzazione non può quindi impostarle a \texttt{null} come
fa \texttt{GuestRetentionJobServiceImpl} sui campi equivalenti (nullable su
\texttt{Guest}): sovrascritte invece con sentinel fissi non identificativi
(\texttt{"ERASED"}, \texttt{LocalDate.EPOCH}), stesso principio già usato
per \texttt{firstName}/\texttt{lastName} nel job \texttt{Guest}
(\texttt{"GDPR"}/\texttt{"ERASED\_<id>"}).

\paragraph{Test}
5 nuovi test in \texttt{StayGuestRetentionJobServiceImplTest}: entrambi i
vincoli scaduti (anonimizza), TULPS attivo (skip), FISCALE attivo (skip),
stato fiscale indeterminato dal circuit breaker (skip, non libera), ospite
ancora in casa senza \texttt{departureDate} (skip).

\paragraph{Riferimento} Commit: \commitref{fe189c76}{feat(security): E22 -- nightly GDPR retention job for StayGuest}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Cifratura a riposo del numero documento \threat{GAP-28}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato da \texttt{docs/COMPLIANCE\_AUDIT\_2026-08.md} \S3: il numero di
documento d'identità dell'ospite era l'unico campo PII senza protezione a
riposo in tutto il sistema, salvato in chiaro sia in
\texttt{guest-service.IdentityDocument.documentNumber} sia in
\texttt{frontdesk-service.StayGuest.documentNumber} (stesso dato,
duplicato indipendentemente nei due servizi). Le credenziali Alloggiati Web
per-hotel avevano già questo trattamento (P8,
\texttt{AlloggiatiCredentialEncryptor}); il numero di documento no,
nonostante sia dato identificativo sensibile ai sensi dell'Art. 32 GDPR.
Classificato OWASP A02 --- Cryptographic Failures.

\begin{insecurebox}[title={IdentityDocument.java --- campo in chiaro (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: nessuna protezione a riposo su un identificativo sensibile
...
\paragraph{Mitigazione applicata}
Due coppie encryptor + \texttt{AttributeConverter} JPA indipendenti, una
per servizio con chiave/salt proprie (mai condivise: una chiave compromessa
in un servizio non espone i dati dell'altro) --- stessa scelta di libreria
già validata per P8, \texttt{Encryptors.delux} (AES-256-GCM, Spring
Security Crypto). Il converter è transparente: l'entità Java vede sempre il
testo in chiaro, la cifratura/decifratura avviene solo nel round-trip
JPA/Hibernate, nessun altro livello applicativo tocca il ciphertext.
\texttt{@Convert(autoApply = false)} applicato esplicitamente solo al
campo interessato, non a ogni colonna \texttt{String} dell'entità.

\begin{securebox}[title={IdentityDocument.java --- cifratura trasparente via converter (fix)}]
\begin{lstlisting}[style=java]
// SICURO: cifrato at-rest, l'entita' vede sempre il plaintext in memoria
...
\paragraph{Migrazione schema, non dati}
Nessuna riscrittura di righe esistenti: confermato con l'utente che solo
dati di test/pilota esistono ad oggi, non un hotel reale in produzione ---
il lavoro resta puramente di schema. Entrambe le colonne allargate a
\texttt{VARCHAR(512)} (ciphertext + IV + tag pesa molto più del numero
documento in chiaro che sostituisce): migrazione \texttt{V10} in
guest-service, \texttt{V26} in frontdesk-service. Rimosso anche
\texttt{idx\_id\_docs\_document\_number} su \texttt{identity\_documents} ---
già codice morto (le query che lo sfruttavano erano state rimosse
nell'audit di isolamento tenant, Punto 7 item 2, senza più alcun chiamante)
e ora doppiamente inutile: un indice di uguaglianza su una colonna cifrata
non deterministica (IV casuale ad ogni scrittura) non può più trovare
corrispondenze comunque.

\paragraph{Test}
Nuovi \texttt{DocumentNumberEncryptorTest}/\texttt{StayGuestDocumentEncryptorTest}
(round-trip cifra/decifra, IV casuale --- due cifrature dello stesso
plaintext producono ciphertext diverso, gestione \texttt{null}/blank) e
\texttt{DocumentNumberConverterTest}/\texttt{StayGuestDocumentNumberConverterTest}
(delega corretta all'encryptor, senza esercitare crypto reale). Due
suite \texttt{@SpringBootTest} preesistenti in frontdesk-service
(\texttt{RoomTypeServiceIntegrationTest}, \texttt{SoftDeleteVersionCheckIntegrationTest})
necessitavano un nuovo \texttt{@MockitoBean} per
\texttt{StayGuestDocumentEncryptor} --- stesso pattern già in uso lì per
\texttt{AlloggiatiCredentialEncryptor} --- per evitare di richiedere le
chiavi di cifratura reali all'avvio del contesto di test.

\paragraph{Riferimento} Commit: \commitref{f20c5904}{fix(security): E23 -- encrypt documentNumber at rest (GDPR Art. 32)}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Startup check fail-closed sulle chiavi di cifratura PII \threat{GAP-29}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Complementare a \threat{GAP-28}: le due coppie chiave/salt introdotte da
quel fix (\texttt{guest.documents.encryption-key/-salt} in guest-service;
\texttt{frontdesk.documents.encryption-key/-salt} e
\texttt{alloggiati.credentials.encryption-key/-salt} in frontdesk-service)
non avevano alcun equivalente di \texttt{HmacSecretStartupCheck}
(api-gateway, già esistente per \texttt{internal.hmac.secret}). Se le
variabili d'ambiente reali non sono impostate, \texttt{docker-compose.yml}
ricade sullo stesso placeholder letterale ovunque
(\texttt{ci\_placeholder\_encryption\_key} /
\texttt{deadbeefdeadbeefdeadbeefdeadbeef}), committato in chiaro in questo
stesso repository. Trovato durante lo stesso audit di pilot-readiness di
\threat{T-AUTH-17}: verificato che il file \texttt{.env} reale di questa
installazione non conteneva nessuna delle sei variabili --- lo stack
girava cifrando numeri di documento d'identità e credenziali Alloggiati
con una chiave pubblica. Classificato OWASP A02 --- Cryptographic
Failures (una chiave nota equivale, ai fini pratici, a nessuna cifratura).

\begin{insecurebox}[title={docker-compose.yml --- fallback a placeholder pubblico (baseline)}]
\begin{lstlisting}[style=yaml]
# INSICURO: se la env var reale non e' nell'.env, il servizio parte
# comunque, cifrando con una chiave pubblicamente nota nel repository
- GUEST_DOCUMENTS_ENCRYPTION_KEY=${GUEST_DOCUMENTS_ENCRYPTION_KEY:-ci_placeholder_encryption_key}
- GUEST_DOCUMENTS_ENCRYPTION_SALT=${GUEST_DOCUMENTS_ENCRYPTION_SALT:-deadbeefdeadbeefdeadbeefdeadbeef}
\end{lstlisting}
\end{insecurebox}

\paragraph{Mitigazione applicata}
Nuovo \texttt{EncryptionKeyStartupCheck} in entrambi i servizi, stesso
pattern di \texttt{HmacSecretStartupCheck}: un \texttt{@EventListener} su
\texttt{ApplicationReadyEvent} che blocca il boot
(\texttt{IllegalStateException}) se una coppia chiave/salt è placeholder
noto, vuota, o sotto 16 caratteri --- ma solo quando il profilo Spring
attivo indica un deployment reale (\texttt{SPRING\_PROFILES\_ACTIVE=
<nome-servizio>}, come impostato da \texttt{docker-compose.yml} per ogni
servizio). Degrada a semplice WARN quando il profilo è vuoto o contiene
\texttt{dev}/\texttt{test} --- condizione che copre sia i test Gradle a
contesto pieno sia un avvio locale ad-hoc senza Docker, senza richiedere
una chiave reale in quei casi.

\begin{securebox}[title={EncryptionKeyStartupCheck.java (guest-service) --- fail-closed in produzione (fix)}]
\begin{lstlisting}[style=java]
// SICURO: in un deployment reale (profilo = nome servizio, mai vuoto/dev/test)
// il boot si interrompe se la chiave e' un placeholder noto o troppo corta
...
\paragraph{Verifica dal vivo}
Generate sei chiavi/salt reali (\texttt{openssl rand -base64 32} /
\texttt{openssl rand -hex 16}) e aggiunte al \texttt{.env} locale (non
committato --- \texttt{.gitignore}). Riavviati guest-service e
frontdesk-service contro il volume esistente: i log mostrano
\texttt{ENCRYPTION\_KEY\_OK} / \texttt{ENCRYPTION\_KEYS\_OK}. Verificato
anche che le due suite \texttt{@SpringBootTest} preesistenti che avevano
richiesto un \texttt{@MockitoBean} per evitare le chiavi reali
(\texttt{RoomTypeServiceIntegrationTest},
\texttt{SoftDeleteVersionCheckIntegrationTest}, si veda \threat{GAP-28})
continuano a passare: girano a profilo vuoto, trattato come segnale
dev/test dal nuovo check.

\paragraph{Riferimento} Commit: \commitref{991436f5}{fix(security): fail-closed startup check for PII encryption keys}

\paragraph{Follow-up --- generazione delle chiavi nel bootstrap segreti}
Il solo fail-closed lasciava un'installazione fresca senza un modo ovvio
per generare le 6 chiavi, oltre a leggere \texttt{.env.example} a mano.
\texttt{setup-hmac-secret.ps1}/\texttt{.sh} --- il bootstrap segreti
idempotente già eseguito da ogni installazione per
\texttt{INTERNAL\_HMAC\_SECRET}/\texttt{JWT\_SECRET}/le 7 password
DB/Redis --- sono stati estesi con lo stesso pattern per le 3 coppie
chiave/salt (chiave: segreto casuale in base64, equivalente a
\texttt{openssl rand -base64 32}; salt: byte casuali in hex, equivalente
a \texttt{openssl rand -hex 16} --- \texttt{Encryptors.delux} richiede il
salt in hex, a differenza della chiave). Verificato in una directory
scratch isolata, non sull'installazione tracciata di questa macchina:
entrambi gli script generano le 6 variabili con il formato corretto e
sono idempotenti a una seconda esecuzione.

\paragraph{Riferimento} Commit: \commitref{8b9727bf}{fix(security): generate the 6 PII encryption keys in the secrets bootstrap}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Segreti SMTP scrivibili solo in un file tracciato in git \threat{GAP-30}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante la pianificazione del punto 2 della checklist go-live
(configurazione di un relay SMTP reale per gli alert operativi), prima
di scrivere qualunque credenziale. \texttt{docker/alertmanager/alertmanager.yml}
era tracciato in git --- a differenza di \texttt{.env} --- con un commento
che invitava esplicitamente a scriverci \texttt{smarthost}/\texttt{from}/
\texttt{to}/\texttt{auth\_username}/\texttt{auth\_password} reali
\emph{"BEFORE GO-LIVE"}: Alertmanager non supporta l'espansione
\texttt{\$\{VAR\}}, quindi l'unico modo suggerito dal commento stesso era
il valore letterale nel file. Configurare un relay reale avrebbe
committato le credenziali SMTP nella storia del repository. Classificato
OWASP A02 --- Cryptographic Failures, stesso trattamento già dato a
\threat{T-CFG-02} per lo stesso tipo di problema (segreto scrivibile solo
in chiaro in un file di configurazione).

\begin{insecurebox}[title={docker/alertmanager/alertmanager.yml --- commento che invita al leak (baseline)}]
\begin{lstlisting}[style=yaml]
# INSICURO: nessuna espansione ${VAR} in questa versione di Alertmanager --
# l'unico modo per configurare un relay reale è scrivere il valore qui,
# in un file TRACCIATO IN GIT (a differenza di .env)
# ... BEFORE GO-LIVE, replace smarthost/from/to/auth_username/auth_password
# with the hotel's real SMTP relay ...
  - name: 'email-ops'
    email_configs:
...
\paragraph{Mitigazione applicata}
Il file tracciato è stato rinominato \texttt{alertmanager.yml.template}:
resta in git ma con placeholder letterali (\texttt{\$\{SMTP\_HOST\}} ecc.),
mai un valore reale. Un nuovo \texttt{docker/alertmanager/entrypoint.sh}
(tracciato, nessun segreto) sostituisce quei placeholder via \texttt{sed}
--- verificato che l'immagine \texttt{prom/alertmanager:v0.27.0} ha
\texttt{sed} ma non \texttt{envsubst} --- scrivendo il risultato in
\texttt{/alertmanager/alertmanager.generated.yml}, dentro il volume Docker
\emph{named} (\texttt{alertmanager\_data}), mai sul filesystem host, mai
raggiungibile da git. Le variabili lette sono le stesse già usate da
\texttt{notification-service} (\texttt{SMTP\_HOST/PORT/USERNAME/PASSWORD/
STARTTLS} --- stesso relay, nessuna duplicazione) più due nuove dedicate
agli alert (\texttt{ALERT\_EMAIL\_FROM}/\texttt{ALERT\_EMAIL\_TO}, identità
mittente/destinatario separata da quella guest-facing). Ogni valore passa
per una funzione di escaping (\texttt{\textbackslash}, \texttt{|}, \texttt{\&}) prima della
sostituzione, per evitare che un carattere speciale in una password reale
corrompa il comando \texttt{sed} o la YAML risultante.

\begin{securebox}[title={docker/alertmanager/entrypoint.sh --- escaping e sostituzione (fix)}]
\begin{lstlisting}[style=bash]
# SICURO: ogni valore passa per l'escaping prima di finire nel comando sed,
# e il file finale non tocca mai il filesystem host o git
escape_for_sed() {
...
exec /bin/alertmanager "--config.file=${OUTPUT_FILE}" "--storage.path=/alertmanager"
\end{lstlisting}
\end{securebox}

\paragraph{Compatibilità e non-regressione}
Ogni variabile ha un default identico al valore hardcoded precedente
(\texttt{mailpit:1025}, nessuna auth, nessun TLS, \texttt{alerts@hotelpms.local}
--- \texttt{ops@hotelpms.local}) --- un'installazione senza alcuna
configurazione aggiuntiva in \texttt{.env} continua a comportarsi
esattamente come prima. Nessuna regressione per gli ambienti dev/CI
esistenti.

\paragraph{Verifica dal vivo}
Container ricreato con la configurazione di default: log identico al
comportamento pre-fix, Alertmanager carica correttamente
\texttt{alertmanager.generated.yml}. Ripetuto con credenziali fittizie
contenenti tutti e tre i caratteri da escapare
(\texttt{test\&user|name} / \texttt{p@ss|with\&special\textbackslash chars}):
il file generato risulta YAML valida, Alertmanager lo carica senza errori
(\texttt{"Completed loading of configuration file"}), verificato leggendo
il contenuto generato direttamente dentro il container.

\paragraph{Riferimento} Commit: --- (non ancora committato al momento della scrittura di questo frammento; aggiornare con l'hash reale)
...
% ─────────────────────────────────────────────────────────────────────────────
\section{GAP-6 --- Nessun Controllo Preventivo sull'Isolamento Multi-Tenant}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
A differenza degli altri GAP di questo capitolo, non è la scoperta di un
singolo bug ma l'assenza di un \emph{meccanismo preventivo}. T-BILL-04 e
T-STAY-06 (Sezione ``Broken Access Control --- Isolamento Multi-Tenant'',
Capitolo~3) sono stati trovati manualmente durante l'esplorazione per la decisione multi-tenant
(ADR-004) --- nessun controllo automatico impediva che una futura query
repository su un'entità tenant-root (con colonna \texttt{hotel\_id} propria)
venisse scritta senza scoping \texttt{hotelId}. La scoperta di questa
classe di bug dipendeva interamente dalla revisione umana, la stessa
condizione che aveva già permesso a T-BILL-04 e T-STAY-06 di restare
inosservati in produzione.

\paragraph{Mitigazione applicata}
\texttt{TenantIsolationArchTest} (ArchUnit) aggiunto ai 5 servizi con
repository multi-tenant (auth, guest, frontdesk, billing, fb): fallisce la
build se un metodo custom su un repository tenant-root non contiene
\texttt{HotelId} nel nome e non è annotato \texttt{@TenantScopeExempt(reason=...)}.

\begin{securebox}[title={TenantIsolationArchTest.java --- struttura della regola}]
\begin{lstlisting}[style=java]
@AnalyzeClasses(packages = "com.hotelpms.billing",
...
final class TenantIsolationArchTest {
    ...
    @ArchTest
    static final ArchRule CUSTOM_QUERY_METHODS_..._MUST_SCOPE_BY_HOTEL_ID =
        classes()
            .that(/* is one of TENANT_ROOT_REPOSITORIES */)
            .should(/* every declared method: name contains "HotelId"
                       OR is @TenantScopeExempt(reason=...) */);
}
\end{lstlisting}
\end{securebox}

I metodi ereditati da \texttt{JpaRepository}/\texttt{CrudRepository}
(\texttt{findById}, \texttt{save}, \texttt{delete}) sono fuori scope --- ArchUnit
\texttt{getMethods()} restituisce solo i metodi dichiarati sull'interfaccia
stessa; quei metodi sono già guardati al call-site (pattern
\texttt{findByIdAndHotelId}, mai il \texttt{findById} ereditato, sulle entità
tenant-root).

\paragraph{Eccezioni documentate}
8 eccezioni verificate manualmente call-site per call-site (non semplicemente
silenziate): login/registrazione pre-hotel-resolve su \texttt{UserAccount}
(4, auth-service --- lo username è l'unica credenziale disponibile prima che
l'hotel del chiamante sia noto); batch GDPR notturno platform-wide su
\texttt{Guest} (1, guest-service --- pre-filtro, i vincoli di legal-hold per
hotel sono verificati a valle); query di overlap prenotazioni con
\texttt{roomIds} già scoped a monte da \texttt{verifyRoomsAvailability}/
\texttt{findCleanRooms} (3, frontdesk-service). billing-service e fb-service
non necessitano eccezioni.

\begin{insecurebox}[title={Esempio di eccezione documentata}]
\begin{lstlisting}[style=java]
@TenantScopeExempt(reason = "Login lookup happens before the caller's "
    + "hotel is known -- the username is the only credential available "
    + "at this point in the auth flow. Usernames are globally unique "
    + "across the platform by design.")
Optional<UserAccount> findByUsername(String username);
\end{lstlisting}
\end{insecurebox}

\paragraph{Pulizia correlata}
Rimosso anche codice morto trovato nella stessa esplorazione, zero
chiamanti in tutto il codebase: \texttt{Room.findByRoomNumberAndActiveTrue},
\texttt{Room.findAllByActiveTrue}, \texttt{Invoice.findByGuestId},
\texttt{Stay.findByReservationId}.

\paragraph{Verifica della regola}
Smoke test manuale: aggiunto temporaneamente un metodo non scoped a
\texttt{InvoiceRepository}, confermato che \texttt{TenantIsolationArchTest}
fallisce con un messaggio chiaro, poi rimosso. La regola cattura
effettivamente la classe di bug per cui è stata scritta, non solo compila.

\paragraph{Gap residuo, noto e accettato}
\texttt{RoomType} non ha colonna \texttt{hotel\_id} (catalogo tipologie
camera oggi globale, non per-hotel) --- richiede una migrazione di schema,
non una query mancante; fuori scope per questa regola, documentato nel
Javadoc di \texttt{TenantIsolationArchTest} e in \texttt{THREAT\_MODEL.md}
come gap da riaprire quando si affronta lo schema di \texttt{room\_types}.

\paragraph{Riferimento} Commit: \commitref{797d676}{feat(security): ADR-004 --- ArchUnit rule enforcing hotel\_id scoping}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Regola ArchUnit vacua e lookup globale non scoped \threat{GAP-24}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Un secondo audit di \texttt{TenantIsolationArchTest} (Punto 7 del piano pilota, riconferma
ADR-004) ha trovato due difetti residui nella regola descritta sopra, entrambi passati
inosservati perché la regola ``passava'' per assenza di violazioni, non perché avesse
verificato qualcosa di reale:

\begin{itemize}
    \item \texttt{HotelSettingsRepository} e \texttt{GuestPrivacySettingsRepository} erano
    entrambi nel set \texttt{TENANT\_ROOT\_REPOSITORIES} ma dichiaravano \textbf{zero} metodi
    custom (entrambe le entità usano \texttt{hotel\_id} come proprio \texttt{@Id}, quindi ogni
    lookup passava dal \texttt{findById} ereditato, fuori scope per costruzione della regola
    --- vedi sopra). La regola ArchUnit su queste due classi era quindi vacua: non c'era
    nulla da controllare.
    \item \texttt{IdentityDocumentRepository.findByGuestId}/
    \texttt{findByDocumentNumberAndDocumentType} erano lookup globali senza scoping
    \texttt{hotelId}, e non comparivano in nessun set tenant-root --- ma un'analisi dei
    chiamanti ha rivelato che erano anche \textbf{codice morto}, zero invocazioni in tutto il
    repository. Il gap e il codice inutilizzato erano gli stessi due metodi.
\end{itemize}

Nello stesso giro, l'estrazione di un helper condiviso per l'estrazione dell'hotelId dal
contesto di sicurezza (\textasciitilde 22 copie quasi identiche di
\texttt{resolveHotelId()}/\texttt{extractHotelId()} sparse sui 5 servizi) ha rivelato che
quelle copie erano realmente divergenti, non solo duplicate: circa metà non controllava mai
\texttt{Authentication == null} prima di dereferenziarla (NPE latente, irraggiungibile solo
perché \texttt{InternalAuthFilter} garantisce sempre un'autenticazione valida in pratica), e
un'implementazione propagava una \texttt{IllegalArgumentException} grezza da
\texttt{UUID.fromString} su un hotelId malformato invece della \texttt{IllegalStateException}
usata dalle altre --- un fallimento non uniforme sullo stesso invariante di sicurezza.
Classificato \textbf{OWASP A01 --- Broken Access Control}.

\begin{insecurebox}[title={HotelSettingsRepository.java --- regola ArchUnit vacua (baseline)}]
\begin{lstlisting}[style=java]
// hotel_id e' l'@Id di HotelSettings: ogni findById(hotelId) e' gia'
// scoped per costruzione, ma la regola ArchUnit non ha nulla da
// verificare finche' questa interfaccia non dichiara un proprio metodo.
public interface HotelSettingsRepository
        extends JpaRepository<HotelSettings, UUID> {
}
\end{lstlisting}
\end{insecurebox}

\begin{securebox}[title={HotelSettingsRepository.java --- metodo esplicito, regola non piu' vacua (main)}]
\begin{lstlisting}[style=java]
public interface HotelSettingsRepository
        extends JpaRepository<HotelSettings, UUID> {

    /** Sinonimo esplicito e hotelId-scoped di findById. */
...
\paragraph{Mitigazione applicata}
Rimossi i due metodi non scoped/non utilizzati di \texttt{IdentityDocumentRepository} (non
scopati, non annotati \texttt{@TenantScopeExempt}: eliminati alla radice invece di
documentare un'eccezione per codice morto). Aggiunto un \texttt{findByHotelId} esplicito a
entrambi i repository con \texttt{hotel\_id} come \texttt{@Id}, rendendo la regola ArchUnit
verificabile su di loro per la prima volta. Estratto \texttt{TenantContext.resolveHotelId()}
(\texttt{internal-auth-lib}) come unica implementazione, sostituendo tutte le \textasciitilde
22 copie sui 5 servizi, con test dedicati per ciascuno dei fallimenti divergenti trovati
(autenticazione nulla, dettagli nulli, non-\texttt{String}, stringa vuota, UUID malformato).

\paragraph{Verifica}
88 suite di test sui 5 moduli toccati, zero fallimenti. Conteggio violazioni
Checkstyle/PMD pre-esistenti confermato invariato per diff contro una baseline
\texttt{git stash} --- il refactor non introduce nuovo debito, chiude solo il gap.

\paragraph{Riferimento} Commit: \commitref{2f550eff}{refactor(security): close ArchUnit tenant-isolation blind spots, extract shared TenantContext helper}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Risposta 403 ambigua tra rotazione password forzata e diniego RBAC \threat{GAP-25}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Trovato durante un collaudo live in browser (Claude in Chrome, 2026-09-06) sul flusso
RBAC di \texttt{AuthenticationFilter} (api-gateway): un account \texttt{RECEPTIONIST}
appena creato risultava bloccato con \texttt{403} su endpoint di uso quotidiano
(\texttt{day-sheet}, \texttt{rooms}, \texttt{events/stream}), inizialmente scambiato per
un bug di autorizzazione. Un'indagine mirata sul codice sorgente (nessuna
\texttt{@PreAuthorize} presente su nessuno dei tre endpoint, confermato da test
esistenti) ha escluso un problema RBAC vero e proprio: la causa reale era che
l'account di test aveva \texttt{mustChangePassword=true} e non aveva mai completato il
cambio password richiesto prima di navigare altrove --- il gateway blocca
correttamente ogni richiesta finché la password non viene cambiata (difesa già corretta,
si veda il commento storico su \texttt{MUST\_CHANGE\_PASSWORD\_ALLOWED\_PATHS}).

Il problema reale, più piccolo ma genuino: \texttt{onError} scriveva un \texttt{403}
\textbf{senza alcun corpo} sia per il branch \texttt{mustChangePassword} (rotazione
password forzata, stato di autenticazione legittimo) sia per il branch RBAC
(\texttt{ACCESS\_DENIED}, diniego di ruolo) --- rendendo i due casi indistinguibili per
qualunque client e per chi analizza log/incidenti, e mascherando uno stato benigno da un
diniego di autorizzazione. Classificato \textbf{OWASP A09 --- Security Logging \&
Monitoring Failures}: non un bypass di enforcement (chi viene bloccato era già corretto
in entrambi i rami), ma un deficit di osservabilità che ha reso più lento diagnosticare
l'accaduto.

\begin{insecurebox}[title={AuthenticationFilter.java --- 403 senza corpo, due cause indistinguibili (baseline)}]
\begin{lstlisting}[style=java]
private Mono<Void> onError(final ServerWebExchange exchange, final String err,
        final HttpStatus httpStatus) {
    final ServerHttpResponse response = exchange.getResponse();
    response.setStatusCode(httpStatus);
    return response.setComplete(); // "err" mai scritto da nessuna parte
}
\end{lstlisting}
\end{insecurebox}

\begin{securebox}[title={AuthenticationFilter.java --- corpo JSON con il motivo (main)}]
\begin{lstlisting}[style=java]
private Mono<Void> onError(final ServerWebExchange exchange, final String err,
        final HttpStatus httpStatus) {
    final ServerHttpResponse response = exchange.getResponse();
...
\paragraph{Mitigazione applicata}
\texttt{onError} scrive ora un corpo JSON minimo con il motivo esatto del rifiuto
(\texttt{\{"error":"PASSWORD\_CHANGE\_REQUIRED"\}} contro
\texttt{\{"error":"ACCESS\_DENIED"\}}), riusando lo stesso pattern già in uso in
\texttt{InternalEventsAuthFilter.reject()} nello stesso package. Test estesi in
\texttt{AuthenticationFilterTest} per asserire il corpo esatto sui tre casi (RBAC
denied, mustChangePassword su endpoint di business, mustChangePassword anche per un
ADMIN) oltre al solo status code già coperto in precedenza.

\paragraph{Riferimento} Commit: \commitref{a33ed7ff}{fix(gateway): distinguish PASSWORD\_CHANGE\_REQUIRED from ACCESS\_DENIED in 403 body}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Allargamento deliberato RBAC: night audit e banner Dashboard aperti a RECEPTIONIST \threat{GAP-26}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Un'analisi di mercato UX commissionata dall'utente (confronto con Mews, Cloudbeds,
Oracle OPERA Cloud, Ericsoft Suite 5°) ha rilevato che tre endpoint erano
\texttt{ADMIN}/\texttt{OWNER}-only per default architetturale mai rivalutato, non per
una reale esigenza di riservatezza: \texttt{NightAuditController} (esecuzione e storico,
comprensivo del riepilogo cassa) e i due summary che pilotano i banner della Dashboard
(\texttt{StayController.getAlloggiatiFailureSummary},
\texttt{getCityTaxUnassessedSummary}). Nella prassi alberghiera reale la chiusura
giornaliera la esegue il turno di notte --- personale di reception, non la proprietà ---
e i due banner segnalano condizioni (schedine Alloggiati non inviate, tassa di soggiorno
non valutata) su cui deve intervenire chi lavora al front desk, non l'owner.

Non è un fix di una vulnerabilità: è un allargamento deliberato del perimetro di
autorizzazione, deciso dall'utente con piena consapevolezza del trade-off. Prima di
applicarlo è stato esplicitamente segnalato che l'endpoint di night audit include il
riepilogo cassa (incassi per metodo di pagamento) e che i due endpoint erano protetti da
test di sicurezza dedicati che bloccavano \texttt{RECEPTIONIST} proprio su quel dato
finanziario --- non un dettaglio da retrofit inosservato. L'utente ha confermato la
scelta due volte, anche dopo questa scoperta: chi esegue il night audit riconcilia
comunque il cassetto contanti, è prassi ordinaria del ruolo, non un'eccezione.

\begin{securebox}[title={NightAuditController.java --- costante di ruolo dedicata (main)}]
\begin{lstlisting}[style=java]
// Prima: private static final String ROLE_ADMIN_OR_OWNER =
//                "hasAnyRole('ADMIN', 'OWNER')";
private static final String ROLE_ADMIN_OWNER_OR_RECEPTIONIST =
...
In \texttt{StayController} l'allargamento è scoped a una \emph{nuova} costante distinta
dalla \texttt{ROLE\_ADMIN\_OR\_OWNER} esistente, applicata solo ai due endpoint summary:
gli altri cinque endpoint \texttt{ADMIN}/\texttt{OWNER}-only dello stesso controller
(invio/download Alloggiati, backfill tassa di soggiorno) restano invariati --- un
allargamento mirato, non una modifica della costante condivisa che li avrebbe aperti
tutti per effetto collaterale.

\paragraph{Mitigazione applicata}
Nuova costante \texttt{hasAnyRole('ADMIN','OWNER','RECEPTIONIST')} sui tre endpoint
sopra. Test di sicurezza aggiornati in entrambe le classi
(\texttt{NightAuditControllerSecurityTest}, \texttt{StayControllerSecurityTest}): le
asserzioni \texttt{403 per RECEPTIONIST} sono diventate \texttt{200}/\texttt{201} per
RECEPTIONIST, con un ruolo \texttt{GUEST} aggiunto come nuovo caso negativo che verifica
che l'enforcement resti comunque presente. Lato frontend, la rotta \texttt{/night-audit}
è stata spostata su un \texttt{ProtectedRoute} proprio con un allowlist
\texttt{NIGHT\_AUDIT\_ROLES} dedicato, separato dal resto delle rotte owner-only in
\texttt{App.tsx}; la voce di navigazione è uscita dal gruppo owner-only in
\texttt{MainLayout.tsx}/\texttt{CommandPalette.tsx}; i due fetch dei banner Dashboard non
sono più condizionati al ruolo owner/admin.

\paragraph{Riferimento} Commit: \commitref{bcf03b8e}{fix(frontdesk,frontend): open night audit and dashboard-alert summaries to RECEPTIONIST}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Hardening strutturale dell'encoder di log contro CWE-117 \threat{GAP-27}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Complementare a \threat{T-AUTH-16} e \threat{T-GW-13}: entrambi quei fix
sanitizzano il valore \emph{alla sorgente} (il call site di logging, o
l'header in ingresso al gateway), ma il pattern Logback che rende
effettivamente la riga di log resta plain-text --- \texttt{\%msg} e
\texttt{\%X\{correlationId:-\}} scritti letteralmente, senza alcuna
protezione strutturale. Un futuro call site che dimentichi di chiamare
\texttt{sanitizeForLog} (i quattro sanitizzatori del progetto sono copie
indipendenti, non un'unica utility condivisa --- nessun modulo è dipendenza
di tutti e otto i servizi, \texttt{api-gateway} è WebFlux e non dipende da
\texttt{common-web-lib}) riaprirebbe silenziosamente lo stesso CWE-117 senza
che né CodeQL né una revisione manuale se ne accorgano necessariamente.

\begin{insecurebox}[title={logback-spring.xml --- pattern plain-text (baseline)}]
...
\paragraph{Mitigazione applicata}
\texttt{\%msg} e \texttt{\%X\{correlationId:-\}} avvolti nel conversion word
\texttt{\%replace(...)\{'[\textbackslash r\textbackslash n]','\_'\}}, che
sostituisce ogni CR/LF con \texttt{\_} indipendentemente da cosa produce il
valore a monte --- CRLF diventa strutturalmente impossibile nell'output,
non solo scoraggiato per convenzione. Applicato in
\texttt{auth-service}/\texttt{billing-service}/\texttt{notification-service}
(\texttt{logback-spring.xml}, sia l'encoder console sia il blocco
\texttt{<message>} dell'appender Loki) e nell'\texttt{application.yml}
condiviso servito da \texttt{config-service} via Spring Cloud Config
(\texttt{logging.pattern.console}), che copre i servizi privi di un proprio
\texttt{logback-spring.xml} (\texttt{api-gateway}, \texttt{guest-service},
\texttt{frontdesk-service}, \texttt{fb-service}). \texttt{config-service}
non importa la configurazione che serve agli altri, quindi replica la
stessa property nel proprio \texttt{application.yml} locale. Gli stack
trace (\texttt{\%ex}) restano fuori dal wrapping --- devono restare
multi-riga.

\begin{securebox}[title={logback-spring.xml --- pattern con encoder rafforzato (fix)}]
\begin{lstlisting}[style=xml]
<pattern>%d{HH:mm:ss.SSS} [%thread] %-5level [%replace(%X{correlationId:-}){'[\r\n]','_'}] %logger{36} - %replace(%msg){'[\r\n]','_'}%n</pattern>
\end{lstlisting}
\end{securebox}
...
\paragraph{T-GW-09 --- Seconda barriera gateway mancante su una nuova risorsa admin-only}
Ogni risorsa la cui scrittura è riservata ad ADMIN/OWNER (es. \texttt{/api/v1/room-types})
è protetta da due barriere indipendenti: il controller applica
\texttt{@PreAuthorize("hasAnyRole('ADMIN','OWNER')")}, e l'API Gateway rifiuta a monte
qualunque scrittura verso quel prefisso per un ruolo diverso, tramite
\texttt{WRITE\_RESTRICTED\_PREFIXES} in \texttt{AuthenticationFilter}. La nuova risorsa
\texttt{/api/v1/rate-calendar} (bulk-apply prezzi su più tipologie camera) è nata con la
sola barriera a livello controller: non uno sfruttabile isolato (\texttt{RECEPTIONIST}
resta comunque bloccato dal \texttt{@PreAuthorize}), ma una rottura del pattern a due
livelli che rende l'enforcement dipendente da un solo punto di fallimento invece di due.
Aggiunto \texttt{"/api/v1/rate-calendar"} a \texttt{WRITE\_RESTRICTED\_PREFIXES}, allineando
la risorsa al resto delle scritture admin-only; la lettura (\texttt{GET}) resta aperta a
tutti i ruoli operativi, invariata.
Commit: \commitref{2bf58be}{fix(security): T-GW-09 --- gateway-level RBAC on /api/v1/rate-calendar bulk-apply}

\paragraph{T-AUTH-10 --- AccessDeniedException mai gestita in auth-service, 500 invece di 403}
\texttt{UserManagementController} applica \texttt{@PreAuthorize}, ma il
\texttt{GlobalExceptionHandler} di auth-service non aveva mai un \texttt{@ExceptionHandler}
esplicito per \texttt{AccessDeniedException}: il diniego cadeva nel catch-all generico
\texttt{Exception.class} e tornava HTTP 500 invece di 403. Non uno sfruttabile isolato
(\texttt{@PreAuthorize} ha sempre negato correttamente l'accesso), ma un client o un
sistema di monitoring non può distinguere un diniego di autorizzazione legittimo da un
guasto server reale, e lo status HTTP standard viene nascosto dietro un errore generico.
Trovato analizzando un finding OSINT non correlato (endpoint Actuator che tornava 500
invece di 404): la causa comune era un catch-all troppo largo, duplicato in 5 dei 6
microservizi invece di vivere nella classe condivisa \texttt{AbstractProblemDetailAdvice}
(\texttt{common-web-lib}). Corretto completando la migrazione di auth-service,
frontdesk-service e notification-service a estendere quella classe condivisa invece di
duplicare gli handler localmente — \texttt{AccessDeniedException} ora eredita l'handler
già corretto usato da billing/fb/guest-service, senza codice nuovo in auth-service.
Aggiunti alla base condivisa anche quattro handler per eccezioni Spring native mai gestite
esplicitamente in nessun servizio (\texttt{NoResourceFoundException}$\to$404,
\texttt{MethodArgumentTypeMismatchException}$\to$400,
\texttt{HttpRequestMethodNotSupportedException}$\to$405,
\texttt{HttpMediaTypeNotSupportedException}$\to$415).
Commit: \commitref{8c0fa2c}{fix(security): T-AUTH-10 --- auth-service inherits AccessDeniedException(403) handling}

\paragraph{GAP-11 --- XXE hardening mancante su FatturaPAServiceImpl}
Uno scan Semgrep mirato su injection/XXE (595 regole Java+OWASP su 905 file) ha
segnalato \texttt{DocumentBuilderFactory} e \texttt{TransformerFactory} in
\texttt{FatturaPAServiceImpl} (billing-service, generazione XML FatturaPA) privi di
hardening DOCTYPE/entità esterne. Non sfruttabile allo stato attuale: il factory
costruisce solo un \texttt{Document} vuoto in memoria, mai un parsing di input esterno;
il transformer serializza quel DOM, mai uno stylesheet esterno. Corretto comunque come
difesa in profondità, stesso standard già applicato in \texttt{AlloggiatiWebSenderServiceImpl}
(T-STAY-03): \texttt{disallow-doctype-decl} sul factory,
\texttt{ACCESS\_EXTERNAL\_DTD}/\texttt{ACCESS\_EXTERNAL\_STYLESHEET} vuoti sul
transformer. Zero cambio di comportamento osservabile.
Commit: \commitref{3fc4b07}{fix(security): GAP-11 --- XXE hardening on FatturaPAServiceImpl}

\paragraph{GAP-12 --- PropertyReferenceException mai gestita, 500 invece di 400}
Lo stesso audit Semgrep ha portato a verificare manualmente ogni endpoint paginato:
un \texttt{sort} su una property inesistente (es. \texttt{?sort=nonEsiste}) fa fallire
la validazione di Spring Data contro il metamodello JPA \emph{prima} che venga generato
SQL — non è injection — ma nessuno dei 6 \texttt{GlobalExceptionHandler} gestiva
\texttt{PropertyReferenceException}, che cadeva nel catch-all generico e tornava 500.
Stesso pattern di T-AUTH-10 (eccezione client-side mascherata da errore server). Fix:
nuovo handler condiviso in \texttt{AbstractProblemDetailAdvice}, ereditato
automaticamente da tutti e 6 i servizi senza modifiche per-servizio.
Commit: \commitref{3fc4b07}{fix(security): GAP-12 --- PropertyReferenceException(400) shared handler}

\paragraph{GAP-13 --- Actuator fb-service esposto per intero in config locale}
\texttt{fb-service/application-fb-service.yml} (config locale, non quella servita da
config-service) esponeva l'intero Actuator (\texttt{include: "*"}, incluso
\texttt{/actuator/env} e \texttt{/actuator/heapdump}) mentre config-service serve già
per lo stesso servizio il valore corretto (\texttt{prometheus,health,info}, identico
agli altri 6 servizi) — residuo locale mai allineato, unico dei 4 file
\texttt{application-<service>.yml} a toccare la config actuator. Trovato dalla regola
Semgrep \texttt{spring-actuator-fully-enabled-yaml}. Fix: allineato al valore già
corretto in config-service.
Commit: \commitref{c2992c2}{fix(security): GAP-13 --- fb-service local actuator exposure aligned}

\paragraph{GAP-14 --- nginx gira come root nell'immagine frontend}
\texttt{frontend/Dockerfile} non dichiarava alcun \texttt{USER}: il processo master
nginx girava come root (immagine base \texttt{nginx:alpine} standard). Fix: swap a
\texttt{nginxinc/nginx-unprivileged:alpine} (immagine ufficiale non-root, ascolto
interno su 8080 — le porte $<$1024 richiedono root per il bind), \texttt{USER nginx}
esplicito, \texttt{nginx.conf} aggiornato (\texttt{listen 8080}),
\texttt{docker-compose.yml} porte \texttt{"80:8080"}. La porta host pubblicata resta 80
— nessun impatto esterno o sulla CI. Verificato dal vivo: rebuild, container healthy,
\texttt{curl} dall'host su :80 risponde 200 come prima del cambio.
Commit: \commitref{a4df5c2}{fix(security): GAP-14 --- frontend nginx runs as non-root (nginx-unprivileged)}

\paragraph{GAP-15 --- GitHub Actions non pinnate a SHA, Dependabot senza cooldown}
\texttt{.github/workflows/ci.yml} referenziava 7 Action per tag mutabile
(\texttt{actions/checkout@v7}, \texttt{setup-java@v5}, \texttt{cache@v6},
\texttt{upload-artifact@v7}, \texttt{setup-node@v7}, \texttt{download-artifact@v8},
\texttt{codeql-action/upload-sarif@v4}) — un tag può essere ripuntato dal maintainer
senza preavviso (supply-chain; incidente reale di riferimento:
\texttt{tj-actions/changed-files}, 2025). Il progetto pinnava già a SHA
\texttt{aquasecurity/trivy-action} e \texttt{docker/setup-buildx-action}; le altre 7
erano rimaste sul pattern precedente. \texttt{.github/dependabot.yml} non impostava
alcun \texttt{cooldown} sugli 11 blocchi \texttt{package-ecosystem} — una dipendenza
appena pubblicata (potenzialmente compromessa) veniva proposta in PR immediatamente.
Fix: le 7 Action pinnate a SHA (stesso pattern \texttt{@<sha40> \# vX.Y.Z} già in uso,
SHA risolti dal tag corrente — zero cambio di comportamento);
\texttt{cooldown: default-days: 7} aggiunto a tutti gli 11 blocchi Dependabot.
Commit: \commitref{0cfbe60}{fix(security): GAP-15 --- pin GitHub Actions to SHA, add Dependabot cooldown}

\paragraph{GAP-16 --- postgres gira come root: rischio accettato, non una fix}
Stessa regola Semgrep di GAP-14 segnala anche \texttt{docker/postgres/Dockerfile} privo
di \texttt{USER}. A differenza di nginx, qui la fix ``ovvia'' romperebbe funzionalità
esistente: \texttt{entrypoint-wrapper.sh} scrive \texttt{/etc/pgbackrest/pgbackrest.conf}
e ne fa \texttt{chown postgres:postgres} (un processo non-root non può cambiare
proprietario a un altro utente), poi ogni comando verso Postgres/pgbackrest passa già
per \texttt{gosu postgres ...} — esattamente il pattern privilege-drop dell'immagine
ufficiale \texttt{postgres:15-alpine} upstream (il suo stesso
\texttt{docker-entrypoint.sh}, quando parte come root, fa \texttt{gosu postgres}
internamente prima di avviare il server; Postgres stesso rifiuta di girare come root).
Non esiste un'immagine ``unprivileged'' equivalente: pgBackRest richiede root
transitorio per sistemare i permessi di un volume montato al primo avvio. Nessuna
modifica di codice — rischio residuo accettato, stesso trattamento di T-OPS-02.

\paragraph{T-NOTIF-01 --- notification-service mai censito nel modello delle minacce}
notification-service (non instradato dal gateway, raggiungibile solo dalla rete
Docker interna, 3 endpoint solo-scrittura che innescano invio email transazionali
contenenti PII ospite nel corpo/subject) era già protetto dallo stesso
\texttt{InternalAuthFilter} HMAC+anti-replay di T-GW-08 fin dalla sua introduzione,
ma non compariva mai in \texttt{THREAT\_MODEL.md} se non come voce della tabella
asset --- zero threat, zero mitigazione tracciati. Senza quella protezione, un
chiamante privo di firma HMAC valida ma con accesso alla rete interna avrebbe
potuto innescare invii email arbitrari (subject/corpo/destinatario a scelta del
chiamante) tramite l'infrastruttura email legittima dell'hotel --- un vettore di
phishing verso gli ospiti. Formalizzato durante l'estrazione di
\texttt{internal-auth-lib} (GAP-7), che ha portato a riesaminare insieme la
protezione di tutti e 6 i servizi.
Commit: \commitref{5d13a7d}{test(security): characterization tests for InternalAuthFilter on auth/notification-service}

\paragraph{GAP-7 --- Sei copie divergenti di InternalAuthFilter, un solo punto di verità mancante}
\texttt{InternalAuthFilter}/\texttt{NonceStore}/\texttt{RedisNonceStore} esistevano
come 6 copie quasi-identiche (auth/billing/fb/frontdesk/guest/notification-service).
5 erano byte-identiche per disciplina nel copia-incolla, non per un meccanismo che
lo garantisse --- nessun controllo impediva a una futura modifica di introdurne una
sesta divergenza silenziosa. auth-service era la prova che il rischio non era
ipotetico: allowlist invertita anziché denylist, controllo presenza header con
\texttt{== null} anziché \texttt{StringUtils.hasText} (un header
\texttt{X-Auth-User: ""} non veniva rifiutato), contratto 401 diverso
(\texttt{sendError}+codice stringa anziché corpo JSON), zero logging sui rigetti.
Tre di queste sono difetti di sicurezza reali, non solo stilistici, scoperti da
questo lavoro di dedup e non da un pentest mirato. Fix in tre passi: test di
caratterizzazione per fissare il comportamento reale di ciascun servizio prima di
toccare nulla; riconciliazione di auth-service sul contratto condiviso; estrazione
in un nuovo modulo Gradle \texttt{internal-auth-lib} (libreria pura, nessuna
autoconfigurazione Spring) consumato da tutti e 6 i servizi tramite
\texttt{new InternalAuthFilter(hmacSecret, nonceStore, excludedPathPrefixes)} nel
rispettivo \texttt{SecurityConfig}, altrimenti invariato. Verificato dal vivo su
tutti e 6 i servizi (richiesta non firmata→401, timestamp scaduto→401, nonce
riusato→401, \texttt{/actuator/health/liveness} senza header→200) più un flusso
applicativo autenticato completo via browser attraverso il gateway reale --- nessuna
regressione. Una revisione di sicurezza indipendente (subagent dedicato) pre-merge
ha confermato l'assenza di vulnerabilità sfruttabili e suggerito un solo indurimento
difensivo aggiuntivo, poi applicato: il matching dei prefissi esenti da HMAC
(\texttt{/actuator}) avveniva per sottostringa grezza anziché per confine di
segmento --- non sfruttabile nella configurazione attuale (Spring Security
richiede comunque autenticazione su qualunque path non esplicitamente permesso),
ma corretto comunque per eliminare il rischio latente su futuri path esenti.
Commit: \commitref{5d13a7d}{test(security): characterization tests}, \commitref{8607d7c}{fix(security): reconcile auth-service InternalAuthFilter onto the shared contract}, \commitref{9cec99f}{refactor(security): extract internal-auth-lib}, \commitref{59d5bf5}{fix(security): match excludedPathPrefixes on path-segment boundaries}

\paragraph{T-FB-03 --- \texttt{@PreAuthorize} mai applicato su MenuItemController}
\texttt{SecurityConfig} di fb-service mancava l'annotazione
\texttt{@EnableMethodSecurity}: i tre \texttt{@PreAuthorize("hasAnyRole('ADMIN','OWNER')")}
su \texttt{MenuItemController} (create/update/delete) non venivano mai valutati
da Spring --- qualunque ruolo autenticato, incluso RECEPTIONIST, poteva creare,
modificare o cancellare voci di menu. Il gap non è stato trovato da un pentest
ma diffando le 5 copie di \texttt{SecurityConfig} prima di consolidarle
(GAP-8 sotto): l'assenza dell'annotazione era invisibile leggendo il singolo
file, evidente solo confrontandolo con gli altri 4 servizi --- lo stesso schema
di scoperta di GAP-7. Il test unitario esistente
(\texttt{MenuItemControllerTest}) non l'aveva mai intercettato perché usa
\texttt{MockMvcBuilders.standaloneSetup()}, che non carica la security filter
chain reale.
Fix: \texttt{@EnableMethodSecurity} aggiunta a fb-service; applicata anche a
guest-service (che non ha ancora nessun \texttt{@PreAuthorize}) come difesa in
profondità, cosicché questa classe di bug non possa ripresentarsi
silenziosamente in futuro. Nuovo \texttt{MenuItemControllerSecurityTest}
(\texttt{@WebMvcTest} con la \texttt{SecurityConfig} reale importata, non uno
standalone MockMvc) come regressione mirata.
Commit: \commitref{bdce1e5}{fix(security): shared internal API security filter chain + missing RBAC enforcement}

\paragraph{GAP-8 --- FeignHeaderConfig, SecurityConfig e GlobalExceptionHandler: stesso pattern di GAP-7}
\texttt{FeignHeaderConfig} (4 copie, 107--125 righe) e \texttt{SecurityConfig}
(5 copie, 75--79 righe) presentavano lo stesso rischio strutturale già
osservato in GAP-7: duplicazione mantenuta identica per disciplina nel
copia-incolla, non per un meccanismo che lo garantisse. Diffando le copie
prima di consolidarle sono emersi due difetti reali, non solo differenze
cosmetiche di javadoc: (1) T-FB-03 sopra; (2) né fb-service né billing-service
gestivano \texttt{AccessDeniedException} nel proprio \texttt{GlobalExceptionHandler}
--- un diniego \texttt{@PreAuthorize} (corretto: l'autorizzazione era comunque
negata, non un bypass) cadeva nell'handler generico e tornava HTTP~500 anziché
403, un contratto di risposta scorretto che billing-service aveva da quando
\texttt{OwnerReportController} ha adottato \texttt{@PreAuthorize}, mai emerso
perché il traffico reale non aveva mai esercitato il path di diniego in modo
osservabile.

Fix: \texttt{FeignHeaderConfig} estratto in
\texttt{InternalFeignAuthInterceptor} (\texttt{internal-auth-lib}) --- l'unica
divergenza comportamentale reale, il fallback di guest-service per il job
batch GDPR che gira fuori da un contesto HTTP, è stata modellata come
strategia (\texttt{FeignAuthFallbackProvider}) invece di un \texttt{if}
duplicato in ogni copia. \texttt{SecurityConfig} estratto in
\texttt{InternalApiSecurityFilterChainFactory}, stessa libreria. Handler
\texttt{AccessDeniedException} aggiunto a fb-service e billing-service
(pattern già presente in frontdesk-service).

Nello stesso lavoro, anche le 6 copie di \texttt{GlobalExceptionHandler}: solo
billing/fb/guest erano davvero identici (stesso host URI, stessa forma degli
errori di validazione) e sono stati consolidati in
\texttt{AbstractProblemDetailAdvice} (nuovo modulo \texttt{common-web-lib}).
auth-service e frontdesk-service usano una forma diversa per gli errori di
validazione (mappa campo→messaggio, non lista) e frontdesk-service genera un
\texttt{traceId} di correlazione che gli altri non hanno --- forzarli nella
stessa base avrebbe significato perdere il traceId o cambiare silenziosamente
il contratto di risposta, quindi sono rimasti separati. Trovati invece cinque
host URI diversi per il campo \texttt{type} RFC~7807
(\texttt{hotel-pms.com}, \texttt{hotelpms.com}, \texttt{api.hotelpms.com},
\texttt{hotelpms.local/problems}) --- nessuno è un dominio registrato
(progetto pre-lancio, vedi \texttt{backup/DECISIONS.md}), nessun frontend o
test e2e dipende dal valore esatto: normalizzati tutti su
\texttt{https://hotel-pms.com/errors/}.

Verificato con build completa (\texttt{./gradlew clean build}, 193 task, zero
warning PMD/checkstyle/SpotBugs) più i test di regressione esistenti su
ciascun servizio toccato.
Commit: \commitref{79cd3b1}{refactor(security): single HMAC Feign header config in internal-auth-lib}, \commitref{bdce1e5}{fix(security): shared internal API security filter chain + missing RBAC enforcement}, \commitref{54575ec}{refactor: shared RFC 7807 problem detail advice, normalize error-type host}

\paragraph{GAP-9 --- Falso positivo CodeQL: CSRF ``disabilitato'' sulla filter chain interna}
CodeQL (regola \texttt{java/spring-disabled-csrf-protection}, severità HIGH)
segnalava \texttt{csrf(AbstractHttpConfigurer::disable)} in
\texttt{InternalApiSecurityFilterChainFactory} (\texttt{internal-auth-lib},
condivisa da billing/fb/frontdesk/guest/notification-service, estratta in
GAP-8) come protezione CSRF disabilitata. Analisi confermata come falso
positivo, non un fix applicativo: questa filter chain protegge
esclusivamente chiamate interne service-to-service dietro il gateway, mai
raggiungibili da un browser. La protezione CSRF esiste per contrastare
richieste automaticamente corredate di credenziali (tipicamente cookie di
sessione) inviate da un sito malevolo per conto di un utente autenticato ---
qui non si applica su due assi indipendenti: \texttt{sessionCreationPolicy(STATELESS)}
non emette mai un cookie di sessione, e l'autenticazione avviene tramite
firma HMAC (\texttt{InternalAuthFilter}, stesso meccanismo di T-GW-08) che
una pagina web attaccante non può calcolare senza il segreto condiviso ---
esattamente il caso che la stessa documentazione Spring esclude
esplicitamente (``only for services used only by non-browser clients''). Il
traffico browser-facing reale ha una protezione CSRF distinta e corretta sul
gateway (\texttt{CsrfFilter}, T-GW-05, double-submit cookie).

Verificato con revisione indipendente (agente \texttt{security-auditor}, non
solo lettura del commento di soppressione): confermato che nessuno dei 5
servizi espone questi endpoint direttamente a un browser (tutti raggiungibili
solo attraverso l'API Gateway) e che non esiste alcuna credenziale
auto-allegata sfruttabile su questo percorso. Aggiunto un commento di
soppressione \texttt{codeql[...]} inline sopra la chiamata --- stesso pattern
già usato per il falso positivo log-injection di GAP-7 --- e alert dismissato
su GitHub con motivazione.
Commit: \commitref{f8c139a}{docs(security): GAP-9 --- suppress CodeQL false-positive CSRF alert on internal-only filter chain}

\paragraph{T-OPS-01 --- RPO fisso a 24h: da dump pianificato a WAL archiving continuo}
Il meccanismo di backup originale (\texttt{docker/db-backup/}, \texttt{pg\_dumpall}
giornaliero, gzip, cifratura \texttt{age}, upload via \texttt{mc}) aveva un RPO
fisso di 24h: un disastro poco prima del prossimo dump pianificato perdeva
fino a un'intera giornata di lavoro. Una decisione precedente
(\texttt{backup/DECISIONS.md~\S3.5}, 2026-07-30) aveva accettato
esplicitamente questo RPO, rifiutando PITR/WAL archiving per complessità
non giustificata dal profilo di rischio del prodotto. La decisione è stata
riaperta e ribaltata il 2026-08-01 su richiesta diretta dell'utente ---
non da uno dei quattro criteri di revisione originariamente documentati,
ma dal ragionamento sul caso peggiore: un dump a mezzanotte con una
perdita dati poco prima significa perdere quasi un giorno di lavoro.

Fix: sostituito interamente \texttt{docker/db-backup/} con
\textbf{pgBackRest}, integrato nel container \texttt{postgres} stesso
(\texttt{docker/postgres/}) --- l'\texttt{archive\_command} deve girare
come sottoprocesso del motore Postgres, non in un container satellite.
WAL archiving continuo (\texttt{archive\_timeout=120s}, RPO in minuti,
non più 24h) più backup full settimanali/incrementali giornalieri su due
repository pgBackRest indipendenti: \texttt{repo1} locale e \texttt{repo2}
off-site (Backblaze B2, stesso bucket già provisionato).

Bug trovato e corretto durante la verifica dal vivo, non ipotetico:
\texttt{pgbackrest backup} opera su un repository per invocazione
(default \texttt{repo1} se \texttt{--repo} non specificato), a differenza
di \texttt{archive-push} che scrive automaticamente su tutti i repository
configurati. La prima versione di \texttt{backup-scheduler.sh} non
specificava mai \texttt{--repo}: il WAL raggiungeva correttamente
entrambi i repository, ma i backup full/incrementali finivano solo su
\texttt{repo1} --- \texttt{repo2} aveva WAL continuo ma zero backup
validi (\texttt{pgbackrest info} riportava esplicitamente
\texttt{repo2: error (no valid backups)}). Fix: lo scheduler itera ora
esplicitamente su ogni repository configurato.

\textbf{Drill PITR reale eseguito e verificato}, non solo scritto: creata
una tabella marker, inserita una riga, forzato uno switch WAL
(\texttt{pg\_switch\_wal()}), eseguito
\texttt{pgbackrest restore --type=time --target=<timestamp prima
dell'insert>} in una directory scratch separata dallo stack live, avviata
come istanza Postgres temporanea su una porta alternativa. La riga
marker risulta assente nel restore, la tabella (creata prima del target)
è presente, \texttt{flyway\_schema\_history} è coerente. Il log di
pgBackRest conferma esplicitamente:
\emph{"recovery stopping before commit of transaction ..., time
2026-08-01 18:48:18"} --- esattamente la transazione dell'insert,
fermata prima del commit. Verifica visiva sul bucket B2 reale
(\texttt{secure.backblaze.com}) richiede il login dell'utente al proprio
account --- non eseguibile da un agente automatico senza le sue
credenziali, lasciata come passo manuale esplicito.

Nuovo workflow \texttt{.github/workflows/backup-restore-drill.yml}: drill
off-site settimanale (+ \texttt{workflow\_dispatch}) su un runner GitHub
effimero, non ancora eseguito dal vivo --- richiede prima la
configurazione dei secret CI.
Commit: \commitref{48828ad}{feat(backup): continuous WAL archiving + PITR via pgBackRest, replaces daily dump}, \commitref{663a0bf}{fix: include docker-compose.yml and remaining backup-hardening changes}

\paragraph{T-OPS-02 --- Cambio del modello di cifratura backup: da asimmetrico a simmetrico}
Il WAL archiving continuo di T-OPS-01 richiede che pgBackRest scriva WAL
cifrato in tempo reale, il che richiede la passphrase di cifratura
presente sullo stesso host che fa girare Postgres. pgBackRest cifra con
una passphrase \textbf{simmetrica} (\texttt{PGBACKREST\_CIPHER\_PASS},
\texttt{repo-cipher-pass}), non con una coppia di chiavi asimmetriche
come lo schema \texttt{age} precedente. La proprietà che aveva lo schema
\texttt{age} --- solo la chiave pubblica sull'host di produzione, la
privata mai --- \textbf{non è replicabile} con questo strumento: se
l'host di produzione viene compromesso, chi lo compromette ha anche la
chiave per decifrare i backup passati e futuri.

Questo non è un rischio scoperto per caso ma un trade-off dichiarato
esplicitamente all'utente \emph{prima} dell'implementazione, in cambio
di: WAL archiving continuo (T-OPS-01), verify/restore-check integrati in
pgBackRest, meno codice infrastrutturale hand-rolled da mantenere
(strumento maturo e mantenuto invece di script bash a mano, coerente con
\texttt{ADR-002}). Documentato per iscritto in
\texttt{backup/DECISIONS.md~\S3.5b} prima di procedere, non dopo.
Commit: \commitref{48828ad}{feat(backup): continuous WAL archiving + PITR via pgBackRest, replaces daily dump}

\paragraph{T-OPS-03 --- Ruolo Postgres superuser condiviso da tutti e 5 i servizi}
Ogni servizio applicativo (auth/guest/frontdesk/billing/fb) si collegava a
Postgres come \textbf{superuser \texttt{postgres} condiviso}, stessa
password per tutti (\texttt{docker-compose.yml},
\texttt{SPRING\_DATASOURCE\_USERNAME=postgres}). L'isolamento
multi-tenant applicativo (\texttt{TenantIsolationArchTest}, regola ArchUnit
che verifica lo scoping per \texttt{hotel\_id} sulle query custom dei
repository tenant-root --- si veda anche l'ADR-004 in
\texttt{backup/DECISIONS.md}, che aveva deciso esplicitamente contro la
Row Level Security Postgres proprio perché i due leak cross-tenant reali
trovati fino a quel momento erano bug applicativi, non un problema che la
RLS avrebbe prevenuto meglio del filtro stesso) non ha alcun effetto se il
servizio stesso ha già accesso illimitato in lettura/scrittura ai database
degli altri 4 a livello di connessione: un servizio compromesso (RCE,
dipendenza malevola, credenziale trafugata) leggeva/scriveva
\texttt{hotel\_auth}, \texttt{hotel\_guest}, \texttt{hotel\_billing},
\texttt{hotel\_fb} oltre al proprio, indipendentemente da qualunque
scoping applicativo. Trovato rileggendo
\texttt{docker/postgres/initdb/init.sql} durante l'hardening
pre-pilota, non da un pentest.

Fix: un ruolo Postgres \emph{least-privilege} per servizio
(\texttt{auth\_service\_app}, \texttt{guest\_service\_app},
\texttt{frontdesk\_service\_app}, \texttt{billing\_service\_app},
\texttt{fb\_service\_app}), ciascuno con \texttt{GRANT ALL} solo sul
proprio database e sul suo schema \texttt{public} (PostgreSQL~15 non
concede più \texttt{CREATE} su \texttt{public} a \texttt{PUBLIC} di
default, va quindi rigranted esplicitamente prima che Flyway possa creare
la prima tabella). Aggiunto anche un ruolo \texttt{postgres\_exporter} a
sola lettura (\texttt{pg\_monitor}, nessun accesso ai dati applicativi)
per il nuovo \texttt{postgres-exporter} (T-OPS-04).

\textbf{Secondo controllo, non ovvio}: PostgreSQL concede
\texttt{CONNECT} su ogni database a \texttt{PUBLIC} per default --- a
differenza del cambio sullo schema \texttt{public} introdotto in PG15,
questo default \emph{non} è cambiato. Verificato dal vivo prima di
aggiungere il fix: un ruolo con zero grant altrove poteva comunque aprire
una connessione al database di un altro servizio ed eseguire query
schema-indipendenti (\texttt{SELECT 1}, cataloghi di sistema) --- non
dati applicativi, ma comunque una superficie di ricognizione cross-tenant
che il solo \texttt{GRANT ALL} sul proprio database non chiudeva. Fix:
\texttt{REVOKE CONNECT ON DATABASE <db> FROM PUBLIC} per ciascuno dei 5
database, con \texttt{CONNECT} ri-concesso esplicitamente solo al ruolo
proprietario.

\textbf{Bug trovato durante il test end-to-end su un volume Postgres
realmente vuoto}, non solo con \texttt{docker compose config}: il nuovo
file dei ruoli ordinava alfabeticamente \emph{prima} del vecchio
\texttt{init.sql} che crea i 5 database (\texttt{"0"} precede
\texttt{"i"} in ASCII, e \texttt{docker-entrypoint-initdb.d} esegue gli
script in ordine lessicografico) --- i \texttt{CREATE ROLE}/\texttt{GRANT}
fallivano con \texttt{database "hotel\_auth" does not exist} perché i
database non erano ancora stati creati. Fix: rinominato \texttt{init.sql}
in \texttt{01-create-databases.sql}.

Verificato dal vivo su un container Postgres realmente fresco (build
dell'immagine, volume vuoto, nessun riuso di stato precedente): la
connessione cross-database ora fallisce con
\texttt{FATAL: permission denied for database "hotel\_billing"} /
\texttt{DETAIL: User does not have CONNECT privilege}; lo stesso ruolo
sul proprio database funziona invariato; il ruolo \texttt{postgres\_exporter}
legge correttamente \texttt{pg\_stat\_database}; il superuser
\texttt{postgres} (necessario per le operazioni pgBackRest e per
l'amministrazione) resta invariato e può ancora connettersi ovunque.
Commit: \commitref{5195fd0}{feat(infra): per-service Postgres roles + real alerting/backup supervision}

\paragraph{T-OPS-04 --- Alertmanager verso \texttt{null}, scheduler di backup non supervisionato, zero metriche host}
Tre punti ciechi operativi indipendenti che rendevano un guasto
\textbf{silenzioso} su un deployment mono-host, esattamente lo scenario
del pilota reale pianificato: (1)~\texttt{docker/alertmanager/alertmanager.yml}
aveva \texttt{receiver: 'null'} sia sulla route di default sia su quella
critica --- ogni alert, incluse le 6 regole Prometheus esistenti
(\texttt{ServiceDown}, \texttt{HighErrorRate}, \dots) e il
\texttt{BackupCycleFailed} POST ad-hoc emesso da pgBackRest, finiva solo
nella UI di Alertmanager su \texttt{:9093} --- porta che
\texttt{docker-compose.prod.yml} chiude esplicitamente all'host in
produzione, quindi in produzione l'alert non era nemmeno consultabile
manualmente; (2)~il loop \texttt{while true} di \texttt{backup-scheduler.sh}
non era supervisionato: \texttt{entrypoint-wrapper.sh} faceva
\texttt{wait} solo sul PID di Postgres, quindi un crash dello scheduler
(OOM, errore su variabile non impostata sotto \texttt{set -u}) lasciava
il container \texttt{healthy} con zero backup effettivamente in corso,
senza alcun segnale fino a un controllo manuale; (3)~zero metriche
infrastrutturali (spazio disco, stato Postgres) oltre alle 8 metriche JVM
Spring Boot già raccolte --- un disco pieno, il guasto singolo più
probabile su un deployment a un solo PC, era completamente invisibile.

Fix: receiver \texttt{email-ops} reale al posto di \texttt{null}
(puntato di default al container \texttt{mailpit} già presente per
dev/pilota, cosicché funzioni fuori dalla scatola --- da sostituire con
l'SMTP reale dell'hotel prima del go-live, stessa convenzione già in uso
nel blocco commentato preesistente del file). \texttt{backup-scheduler.sh}
ora gira sotto un supervisore in \texttt{entrypoint-wrapper.sh} che
rileva un'uscita inattesa, invia un alert (\texttt{scheduler-crashed}) e
riavvia il processo dopo 10s; pubblica inoltre due metriche
textfile-collector (\texttt{pgbackrest\_scheduler\_heartbeat\_timesta-}
\texttt{mp\_seconds}, \texttt{pgbackrest\_last\_successful\_backup\_time-}
\texttt{stamp\_seconds}, scrittura atomica via \texttt{mktemp}+\texttt{mv})
consumate da due nuove regole Prometheus (\texttt{BackupHeartbeatStale},
\texttt{BackupNotSucceededRecently}, soglia 26h). Aggiunti
\texttt{node-exporter} (disco/CPU/RAM host) e \texttt{postgres-exporter}
(connessioni, \texttt{pg\_up}) al profilo \texttt{observability}, con
nuove regole \texttt{DiskSpaceLow} e \texttt{PostgresDown}.

\textbf{Secondo bug trovato durante il test dal vivo}, distinto da quello
di T-OPS-03: il wrapper dichiarava Postgres ``pronto'' al primo
\texttt{pg\_isready} riuscito, ma l'immagine ufficiale
\texttt{postgres:15-alpine} avvia un server \emph{temporaneo} per
eseguire gli script di \texttt{docker-entrypoint-initdb.d/} su un volume
vuoto, poi lo ferma e avvia quello reale --- un \texttt{pg\_isready}
riuscito può quindi riferirsi all'istanza temporanea. Il primo ciclo di
backup (avviato subito dopo, appena Postgres risulta pronto) collideva
con quel riavvio interno e falliva con
\texttt{ERROR: [101]: NULL result required to complete request}
(la connessione di \texttt{pg\_backup\_start} veniva terminata a metà dal
riavvio). Riprodotto due volte in modo deterministico prima del fix. Fix:
richiesti 3 controlli \texttt{pg\_isready} consecutivi riusciti (1s di
distanza) prima di procedere --- il riavvio interno produce una finestra
di fallimento tra i due server che azzera il contatore, facendo attendere
l'intero ciclo di riavvio invece di competere con esso.

Verificato dal vivo: rebuild completo dell'immagine, avvio su volume
vuoto, backup full completato senza collisioni
(\texttt{backup command end: completed successfully}); processo
\texttt{backup-scheduler.sh} terminato manualmente con
\texttt{kill -9} --- log conferma rilevamento del crash, tentativo di
alert, e processo ripristinato entro 10s.
Commit: \commitref{5195fd0}{feat(infra): per-service Postgres roles + real alerting/backup supervision}
...
\paragraph{T-GST-07 --- \texttt{@PreAuthorize} mai applicato su GuestController e GuestPrivacySettingsController}
Trovato dal secondo giro di test esplorativo dal vivo (round~1 finding~\#7 +
round~2 bug~\#6), stesso pattern di T-FB-03: nessuna \texttt{@PreAuthorize}
né a livello classe né metodo su \texttt{GuestController} (delete/
anonimizzazione, export Art.~20 GDPR) né su
\texttt{GuestPrivacySettingsController} (periodo di retention GDPR
dell'hotel). L'unica protezione era la whitelist per-prefisso
nell'api-gateway, che autorizza qualunque ruolo operativo (incluso
RECEPTIONIST) su \texttt{/api/v1/guests/**}. Riprodotto dal vivo: un
RECEPTIONIST cancellava/anonimizzava un ospite, esportava il dump PII
completo e modificava la retention policy dell'hotel, tutte operazioni
di primo livello mai pensate per quel ruolo.
Fix: \texttt{@PreAuthorize("hasAnyRole('ADMIN','OWNER')")} su
\texttt{deleteGuest}/\texttt{exportGuestData} in \texttt{GuestController}
e a livello classe su \texttt{GuestPrivacySettingsController} ---
CRUD/ricerca/allegati documento restano invece aperti a RECEPTIONIST,
operatività quotidiana del front desk. Nuovi
\texttt{GuestControllerSecurityTest}/\texttt{GuestPrivacySettingsControllerSecurityTest}
(\texttt{@WebMvcTest} con \texttt{SecurityConfig} reale, stesso pattern di
\texttt{MenuItemControllerSecurityTest}). Verificato dal vivo: RECEPTIONIST
→ \texttt{403} sui 4 endpoint ristretti, \texttt{200} invariato su lettura.
Commit: \commitref{a87d329}{fix(security): T-GST-07/T-BILL-07 --- missing RBAC on guest/invoice controllers}

\paragraph{T-BILL-07 --- \texttt{@PreAuthorize} mai applicato su InvoiceController}
Stesso pattern di T-GST-07, trovato dal round~2 bug~\#5: nessuna
\texttt{@PreAuthorize} sulle operazioni fiscalmente sensibili di
\texttt{InvoiceController}. Riprodotto dal vivo con un RECEPTIONIST:
cambio \texttt{documentType} FATTURA$\leftrightarrow$RICEVUTA, generazione (e blocco
permanente) di un export FatturaPA, impostazione dello stato di
trasmissione SDI, e lancio dell'export batch per il commercialista,
tutte riuscite senza alcun controllo oltre l'autenticazione.
Fix: \texttt{@PreAuthorize("hasAnyRole('ADMIN','OWNER')")} sui quattro
endpoint (\texttt{document-type}, \texttt{fatturaPA}, \texttt{sdi-status},
\texttt{export} batch). Lasciati aperti a RECEPTIONIST: lettura/ricerca
fatture, download PDF di cortesia, e i tre endpoint di chiamata
interservizio (\texttt{createInvoiceForStay}, \texttt{addCharge},
\texttt{getLastInvoiceDateForGuest}) --- questi ultimi propagano via HMAC
il ruolo dell'utente che ha originato l'azione reale (check-in, conferma
ordine F\&B, cancellazione ospite: quest'ultimo è proprio il chiamante
del fix T-GST-06 sopra), non l'operatore che chiama \texttt{InvoiceController}
direttamente --- restringerli a ADMIN/OWNER avrebbe rotto il check-in di
un RECEPTIONIST. Nuovo \texttt{InvoiceControllerSecurityTest}. Verificato
dal vivo: RECEPTIONIST → \texttt{403} sui 4 endpoint, \texttt{200}
invariato su lettura; ADMIN invariato (un tentativo di \texttt{document-type}
su fattura \texttt{PAID} resta \texttt{409} per un guard di business
preesistente, non \texttt{403} --- nessuna regressione introdotta da questo fix).
Commit: \commitref{a87d329}{fix(security): T-GST-07/T-BILL-07 --- missing RBAC on guest/invoice controllers}

\paragraph{GAP-17 --- Trust boundary IP rotto dalla topologia reale nginx→gateway}
Trovato durante la ri-verifica post-hardening del 2026-08-16 da due
sotto-agenti indipendenti (categorie \emph{auth-jwt} e \emph{access-control}),
che hanno segnalato separatamente lo stesso problema su T-AUTH-12 e T-GW-10:
entrambi i fix avevano sostituito la fonte di trust da un header
spoofabile (\texttt{X-Forwarded-For}) al TCP \texttt{remoteAddress} grezzo
--- corretto in isolamento, ma nella topologia reale il browser non parla
mai direttamente con \texttt{api-gateway}: passa sempre per \texttt{frontend}
(nginx), \texttt{proxy\_pass http://api-gateway:8080} sulla stessa rete
Docker (\texttt{dmz-network}), senza alcuna configurazione di trust sugli
header forwarded lato gateway. Ogni richiesta dal front-door standard
arrivava quindi al gateway con lo stesso peer TCP --- l'IP interno del
container nginx, identico per \emph{ogni} utente reale --- collassando la
chiave di lockout \texttt{(username, clientIp)} di T-AUTH-12 a
\texttt{(username)} esattamente come prima del fix, e raggruppando tutti i
client reali in un unico bucket di rate limiting condiviso (T-GW-10).
Exploit concreto: 5 richieste \texttt{POST /api/v1/auth/login} con
\texttt{username=admin} e password errata, dalla normale pagina di login
pubblica, bloccavano il vero \texttt{admin} per 15 minuti, ripetibile
indefinitamente.

Fix: \texttt{docker-compose.prod.yml} non pubblica più \texttt{api-gateway:8080}
sull'host --- solo \texttt{frontend:80} resta raggiungibile dall'esterno,
verificato che \texttt{dmz-network} interno (condiviso da frontend e
api-gateway) non dipende dal port publish tramite \texttt{docker compose
config}. Nuovo flag \texttt{gateway.security.trust-forwarded-header}
(\texttt{config-service}: \texttt{false} in \texttt{api-gateway.yml},
\texttt{true} solo in \texttt{api-gateway-prod.yml}) fa sì che
\texttt{ClientIpFilter} si fidi dell'header \texttt{X-Real-IP} di nginx
(sempre impostato da \texttt{\$remote\_addr}, overwrite mai append) solo
quando quel flag è vero --- cioè solo quando la porta diretta è
effettivamente chiusa. \texttt{RateLimiterConfig} non ri-deriva più l'IP
autonomamente: legge lo stesso header \texttt{X-Client-IP} già risolto da
\texttt{ClientIpFilter}, eliminando la duplicazione di logica che aveva
causato la regressione T-GW-10 in primo luogo.

\textbf{Regressione trovata nello stesso giro di verifica}: la prima
versione del fix impostava il flag a \texttt{true} in
\texttt{api-gateway-prod.yml}, ma nessun file del repository settava mai
\texttt{SPRING\_PROFILES\_ACTIVE=prod} per \texttt{api-gateway} --- unico
servizio senza, a differenza degli altri 6 (che impostano
\texttt{SPRING\_PROFILES\_ACTIVE=<nome-servizio>}). Spring Cloud Config non
serve mai un file di profilo senza quel profilo attivo sul client: il
flag restava \texttt{false} anche a porta chiusa, e GAP-17 restava di
fatto aperto nonostante il fix. Aggiunto \texttt{SPRING\_PROFILES\_ACTIVE=prod}
all'\texttt{environment} di \texttt{api-gateway} in
\texttt{docker-compose.prod.yml}, verificato che il merge è additivo
(tutte le variabili base restano) via \texttt{docker compose config}.
Verificato anche: in dev (porta ancora aperta, flag \texttt{false}) un
\texttt{X-Real-IP} o \texttt{X-Forwarded-For} forgiato inviato
direttamente a \texttt{:8080} resta ignorato --- finding \#3
(security-report.md) non riapre in quell'ambiente.
Commit: \commitref{ff432e3}{fix(security): trust nginx X-Real-IP at gateway only once api-gateway:8080 is closed (GAP-17)}

\paragraph{GAP-18 --- Version-check assente sul soft-delete di Quotation e RateSeason}
Trovato durante la stessa ri-verifica post-hardening (sotto-agente
\emph{business-logic}): l'override \texttt{@SQLDelete} di \texttt{Quotation}
e \texttt{RateSeason} (\texttt{UPDATE ... SET active = false WHERE id = ?})
non verificava né incrementava la colonna \texttt{version} aggiunta da
T-QUOT-01/T-PRICE-01, a differenza di \texttt{Reservation} nello stesso
codebase (\texttt{UPDATE ... SET active = false, version = version + 1
WHERE id = ? AND version = ?}). Poiché l'\texttt{UPDATE} generato da
Hibernate per un \texttt{save()} normale scrive comunque tutte le colonne
mappate, incluso \texttt{active}, una race \texttt{UPDATE}-vs-\texttt{DELETE}
concorrente sulla stessa riga non generava mai un conflitto: un
\texttt{DELETE} (staff B) poteva essere silenziosamente annullato da un
\texttt{save()} concorrente (staff A) ignaro della cancellazione,
``resuscitando'' un preventivo appena cancellato come \texttt{ACCEPTED}
con una prenotazione già creata, o riportando in vita una stagione
tariffaria che un secondo admin credeva rimossa.

Fix: entrambi gli \texttt{@SQLDelete} allineati esattamente al pattern già
in uso su \texttt{Reservation.java}.

\textbf{Regressione trovata nella riverifica dello stesso fix}: il
pattern replicato da \texttt{Reservation} non enforcava mai davvero il
controllo versione a runtime --- \texttt{@SQLDelete} di Hibernate usa
\texttt{ResultCheckStyle.NONE} di default, quindi una \texttt{DELETE} con
\texttt{version} disallineata aggiorna zero righe e Hibernate non se ne
accorge, riportando successo pur non avendo cancellato nulla (race
inversa: una \texttt{DELETE} stale sopravvive silenziosamente a un
\texttt{save()} concorrente che ha vinto per primo). Lo stesso difetto
latente era già presente in \texttt{Reservation.java} fin da T-RES-01, non
introdotto da questo fix. Aggiunto \texttt{check = ResultCheckStyle.COUNT}
a tutti e tre gli \texttt{@SQLDelete} (\texttt{Quotation}, \texttt{RateSeason}
e \texttt{Reservation}) --- una \texttt{DELETE} a zero righe solleva ora la
\texttt{StaleStateException} che \texttt{GlobalExceptionHandler} già mappa
a \texttt{409 CONCURRENT\_MODIFICATION}. Nota di copertura: nessun test a
livello repository esercita la SQL reale di delete per nessuna delle tre
entità (i test di service mockano \texttt{repository.delete()}) --- il fix
è verificato per compilazione e build (\texttt{./gradlew
:frontdesk-service:test} verde) ma non da un test che dimostri
esplicitamente il \texttt{409} su una delete stale, da pianificare in un
giro successivo.

\textbf{Chiuso 2026-08-16}: nuovo \texttt{SoftDeleteVersionCheckIntegrationTest}
--- Testcontainers PostgreSQL reale, stesso setup di
\texttt{RoomTypeServiceIntegrationTest}, nessun mock su
\texttt{repository.delete()}. Per ciascuna delle tre entità: carica la
riga, incrementa \texttt{version} nel database con una \texttt{UPDATE} SQL
nativa (simula un salvataggio concorrente che l'istanza in memoria non
vede), poi tenta la delete con la \texttt{version} ormai stale ---
verifica che sollevi \texttt{ObjectOptimisticLockingFailureException}.
Verificato che il test cattura davvero la regressione, non solo che
compila: rimosso temporaneamente \texttt{check = ResultCheckStyle.COUNT}
dalle tre entità e rieseguito --- tutti e 3 i test falliscono
sull'asserzione (nessuna eccezione sollevata, la delete stale ``riesce''
silenziosamente, esattamente il sintomo descritto sopra); ripristinato il
fix e rieseguito --- tutti e 3 tornano verdi, insieme all'intera build
(\texttt{checkstyle}/\texttt{PMD}/\texttt{SpotBugs}/\texttt{jacoco}).
Commit: \commitref{5f5a698}{fix(security): version-check soft-deletes on Quotation/RateSeason, enforce it on all three (GAP-18)}, \commitref{83dfda4}{test(security): repository-level stale-delete race test for GAP-18}
...
3 ulteriori alert \texttt{java/log-injection} (2026-07-31) su
\texttt{internal-auth-lib/InternalAuthFilter.java} --- falsi positivi:
\texttt{sanitizeForLog()} rimuove già CR/LF (CWE-117) prima di ogni chiamata di
log flaggata, ma la query standard di CodeQL non riconosce un sanitizzatore
custom del progetto come interruzione del flusso di taint. Commenti di
soppressione \texttt{codeql[java/log-injection]} aggiunti ad ogni chiamata
(documentazione inline, non solo dismissal silenzioso); alert chiusi su GitHub
con motivazione.

\paragraph{Correzione (2026-09-10): i commenti \texttt{codeql[...]} non sopprimono nulla}
Durante il triage degli alert \texttt{\#588}/\texttt{\#589}/\texttt{\#590}
(stesso falso positivo di cui sopra, questa volta su
\texttt{AuthServiceImpl.java}) è emerso che il repository gira in CodeQL
\emph{default setup} (nessun workflow \texttt{codeql-analysis.yml} committato
--- confermato via \texttt{gh api .../code-scanning/analyses}, che mostra
\texttt{CodeQL} come tool ma nessuna analisi triggerata da un job del
repository, e da \texttt{docs/ROADMAP.md} P13, che documenta l'aggiornamento
della query suite a \texttt{extended} fatto via API proprio perché non esiste
un file di configurazione in-repo). In default setup i commenti di
soppressione inline \texttt{codeql[...]} \textbf{non hanno alcun effetto}
sullo stato dell'alert --- funzionano solo con \emph{advanced setup} (un
workflow CodeQL committato nel repository). I tre alert su
\texttt{AuthServiceImpl.java} sono quindi rimasti aperti contro ogni commit
successivo nonostante il commento, esattamente come già documentato per il
falso positivo CSRF di \threat{GAP-9} (che infatti richiese anche lì un
dismissal manuale, non il solo commento). Corretto: i tre alert dismissati
manualmente su GitHub (\texttt{false positive}, stessa motivazione tecnica),
e i commenti inline in \texttt{AuthServiceImpl.java} riformulati per non
affermare più un meccanismo di soppressione che non esiste sotto questa
configurazione.
...
\paragraph{Centralizzazione pin Tomcat/Netty e bump immagini container (DEP-CVE-08)}
Triage dei 34 alert code-scanning aperti su GitHub il 2026-09-10 (4 CodeQL,
30 Trivy). \texttt{tomcat.version}/\texttt{netty.version} erano duplicati
identici in 8 \texttt{build.gradle.kts} --- esattamente la duplicazione che
aveva lasciato Tomcat fermo a \texttt{10.1.55} ovunque (3 alert CRITICAL).
Centralizzati nel blocco \texttt{subprojects\{\}} del \texttt{build.gradle.kts}
di root, stesso pattern già in uso per l'override \texttt{httpcore5}. Netty
bumpato a \texttt{4.1.137.Final} (\texttt{CVE-2026-75595}, CRITICAL),
verificato risolvere correttamente via
\texttt{./gradlew :auth-service:dependencies}. Tomcat \textbf{non} bumpato a
\texttt{10.1.58}: la advisory lo cita come fixed version ma quella versione
non è pubblicata su Maven Central (\texttt{GET .../10.1.58/... -> 404},
risoluzione Gradle fallisce) --- lasciato alla versione più alta
realmente esistente, con nota di ricontrollo alla prossima release Apache.

\texttt{docker/postgres/Dockerfile} e il nuovo
\texttt{docker/redis/Dockerfile} eseguono
\texttt{apk update \&\& apk upgrade --no-cache} sopra il tag base: il solo
bump di tag non aggiorna i pacchetti Alpine già cristallizzati nell'immagine
upstream al momento della sua build (verificato: \texttt{redis:8.8.2-alpine}
da solo restava su \texttt{libcrypto3}/\texttt{libssl3} \texttt{3.5.7-r0}).
Con l'\texttt{apk upgrade}, un rescan Trivy locale di entrambe le immagini
costruite torna a 0 vulnerabilità CRITICAL/HIGH. Redis
\texttt{8.8.1-alpine}~→~\texttt{8.8.2-alpine}; Grafana
\texttt{13.1.1}~→~\texttt{13.1.5} (azzera i 2 CRITICAL
\texttt{golang.org/x/crypto}; gli HIGH residui sono nei plugin datasource Go
precompilati, fixabili solo upstream); \texttt{frontend/Dockerfile}:
\texttt{nginxinc/nginx-unprivileged:alpine} --- l'unico tag completamente
flottante del repository --- pinnato a \texttt{1.31.5-alpine}.
\texttt{.trivyignore} esteso con 16 nuove CVE Go-stdlib nel binario
\texttt{gosu} bundlato in postgres, emerse solo ora che la CI scansiona
l'immagine costruita da noi invece del tag upstream nudo (stessa
argomentazione di raggiungibilità già in uso nel file: \texttt{gosu} fa solo
\texttt{setuid}+\texttt{execve}, nessun path di rete/TLS/parsing).

Di conseguenza, \texttt{.github/workflows/ci.yml}: nuovo gate bloccante
(\texttt{exit-code: 1}, severità CRITICAL) sul job \texttt{trivy} per le 11
immagini che costruiamo noi --- postgres/redis spostate lì dal job
\texttt{trivy-thirdparty}, avendo ora un proprio Dockerfile hardenato;
\texttt{trivy-thirdparty} (solo Grafana rimasto) legge il tag da
\texttt{docker-compose.yml} a runtime invece di duplicarlo a mano --- quel
duplicato hardcoded è la causa esatta del drift di \texttt{DEP-CVE-07}; nuovo
schedule settimanale (lunedì 05:00 UTC) così gli alert di terze parti si
aggiornano anche senza push; \texttt{trivy-config: trivy.yaml} finalmente
referenziato da entrambi i job (il file esisteva dalla baseline ma nessuno
step lo usava, quindi era configurazione morta). 17 alert Trivy rimasti
orfani dalla migrazione di categoria (postgres/redis) dismissati manualmente
su GitHub con motivazione, dopo verifica via rescan locale che fossero
effettivamente risolti.
Commit: \commitref{c050521}{fix(security): centralize tomcat/netty version overrides, bump netty to 4.1.137.Final},
\commitref{2eba1cc}{fix(security): patch OS packages in postgres/redis images, bump redis/grafana, pin nginx},
\commitref{81b3f2f}{fix(ci): gate CRITICAL on our images, scan postgres/redis as built, kill matrix drift}

\paragraph{Bump vitest --- alert Dependabot residui (DEP-CVE-09)}
4 alert Dependabot sul frontend, tutti su devDependency (nessun impatto sul
bundle di produzione): \texttt{@vitest/mocker} (path traversal,
GHSA-82fw-gwwq-j7x9) risolto con bump diretto di \texttt{vitest} e
\texttt{@vitest/coverage-v8} a \texttt{\^{}4.1.11} --- già dentro il range
semver dichiarato in \texttt{package.json}, \texttt{npm audit fix} da solo
non attraversava il bump per una divergenza nella risoluzione delle
dipendenze peer. \texttt{baseline-browser-mapping}, \texttt{browserslist},
\texttt{@humanfs/node} risolti transitivamente dallo stesso
\texttt{npm audit fix}. Verificato: lint pulito, build verde, 1154/1154 test
Vitest verdi.
Commit: \commitref{209354e}{fix(deps): bump vitest to 4.1.11, closing @vitest/mocker path-traversal advisory}
