...
Questo capitolo documenta le vulnerabilità rilevate nel codice del branch \texttt{main}
...
classificabili OWASP A02 e A07, per un impatto complessivo da \textbf{alto} a
\textbf{critico}.
...
OWASP A03 e A04, che aprivano la via a injection, manipolazione dei prezzi e
...
\paragraph{T-RES-01 — Double Booking (assenza di lock concorrente)}
\texttt{ReservationServiceImpl.createReservation()} eseguiva un controllo di overlap
...
Classificate prevalentemente OWASP A05 --- Security Misconfiguration.
...
Classificato OWASP A09 --- Security Logging \& Monitoring Failures.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Propagazione X-Auth-Hotel End-to-End \threat{T-GW-06}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
Tutte le mitigazioni IDOR sui singoli servizi si basavano su un presupposto fondamentale:
che l'header \texttt{X-Auth-Hotel} venisse iniettato dall'\texttt{AuthenticationFilter}
dell'API Gateway su ogni richiesta autenticata. Questo presupposto era \emph{falso}
nella baseline: \texttt{UserAccount} in \texttt{auth-service} non possedeva il campo
\texttt{hotel\_id}, quindi \texttt{JwtService.generateToken()} non poteva includere il
claim \texttt{hotelId} nel JWT. Di conseguenza, l'\texttt{AuthenticationFilter} non aveva
il dato per iniettare \texttt{X-Auth-Hotel} sulle richieste a valle.

L'effetto pratico era che tutti i servizi che richiedevano l'header (guest, reservation,
billing, fb) restituivano sistematicamente HTTP~401 a ogni richiesta autenticata passante
per il gateway: l'isolamento multi-tenant era \emph{interamente non funzionante} in produzione.
Classificato OWASP A01 --- Broken Access Control / IDOR.

\begin{insecurebox}[title={UserAccount.java (auth-service) --- campo hotel\_id assente (baseline)}]
\begin{lstlisting}[style=java]
// INSICURO: UserAccount non ha hotel_id -> il JWT non puo' contenere hotelId
@Entity
...
public class UserAccount {
    ...
    private Role role;
    // hotel_id mai presente: nessun tenant binding per l'utente
...
public String generateToken(UserAccount user) {
    return Jwts.builder()
        .setSubject(user.getUsername())
        .claim("role", user.getRole().name())
        // hotelId mai aggiunto: X-Auth-Hotel non potra' mai essere iniettato
        .setIssuedAt(new Date())
        .setExpiration(new Date(System.currentTimeMillis() + expiration))
        .signWith(key, SignatureAlgorithm.HS256)
        .compact();
...
\paragraph{Mitigazione applicata}
La correzione è stata end-to-end su quattro livelli:

\begin{enumerate}
  \item \textbf{Flyway V3 su auth-service} --- migrazione
        \texttt{V3\_\_add\_hotel\_id\_to\_user\_account.sql}: aggiunge la colonna
        \texttt{hotel\_id UUID NOT NULL} con backfill dell'UUID amministrativo di default
        (\texttt{00000000-0000-0000-0000-000000000001}) per le righe esistenti;
        crea un indice su \texttt{(hotel\_id) WHERE active = TRUE}.
  \item \textbf{UserAccount aggiornato} --- aggiunto \texttt{private UUID hotelId} come
        campo persistente.
  \item \textbf{JwtService.generateToken/Refresh aggiornati} --- la firma ora richiede
        il parametro \texttt{UUID hotelId} e lo inserisce nel JWT come claim
        \texttt{hotelId} tramite la costante \texttt{CLAIM\_HOTEL\_ID}.
  \item \textbf{AuthenticationFilter aggiornato} --- estrae \texttt{hotelId} dal JWT
        e lo inietta come \texttt{X-Auth-Hotel} su ogni richiesta a valle; restituisce
        HTTP~401 se il claim è assente, impedendo che token legacy privi del campo
        superino il gateway.
\end{enumerate}

\begin{securebox}[title={Flyway V3 --- V3\_\_add\_hotel\_id\_to\_user\_account.sql}]
\begin{lstlisting}[style=sql]
-- Step 1: colonna nullable per evitare lock su righe esistenti
ALTER TABLE user_account
    ADD COLUMN IF NOT EXISTS hotel_id UUID;

-- Step 2: backfill righe preesistenti con UUID admin di default
UPDATE user_account
   SET hotel_id = '00000000-0000-0000-0000-000000000001'
 WHERE hotel_id IS NULL;

-- Step 3: impone NOT NULL dopo che ogni riga ha un valore
ALTER TABLE user_account
    ALTER COLUMN hotel_id SET NOT NULL;
\end{lstlisting}
\end{securebox}

\begin{securebox}[title={JwtService.java (auth-service) --- claim hotelId nel JWT (fix)}]
\begin{lstlisting}[style=java]
private static final String CLAIM_HOTEL_ID = "hotelId";

// SICURO: il JWT ora contiene il claim hotelId per l'isolamento multi-tenant
public String generateToken(final String username,
                            final Role role,
                            final UUID hotelId) {
    final Map<String, Object> claims = new HashMap<>();
    claims.put("role", role.name());
    claims.put(CLAIM_HOTEL_ID, hotelId.toString());  // tenant binding
    return buildToken(claims, username, jwtExpiration);
}
\end{lstlisting}
\end{securebox}

\begin{securebox}[title={AuthenticationFilter.java (api-gateway) --- iniezione X-Auth-Hotel (fix)}]
\begin{lstlisting}[style=java]
private static final String HEADER_HOTEL = "X-Auth-Hotel";

// SICURO: hotelId estratto dal JWT e iniettato come X-Auth-Hotel
final String hotelId = claims.get("hotelId", String.class);
if (hotelId == null || hotelId.isEmpty()) {
    return this.onError(exchange,
        "JWT missing hotelId claim", HttpStatus.UNAUTHORIZED); // 401
}
...
return chain.filter(exchange.mutate()
    .request(request.mutate()
        ...
        .header(HEADER_HOTEL, hotelId)   // X-Auth-Hotel ora sempre presente
...
\paragraph{Test}
Aggiornati \texttt{AuthServiceImplTest} (costante \texttt{TEST\_HOTEL\_ID}, mock a tre
argomenti per \texttt{generateToken/Refresh}) e \texttt{AuthenticationFilterTest}
(\texttt{hotelId} inserito nel JWT di test via \texttt{buildJwt()}, asserzione che l'header
\texttt{X-Auth-Hotel} sia presente nella richiesta a valle). Tutti i 144 test passano.

\paragraph{Riferimento} Commit: \commitref{b469e21}{fix(security): T-IDOR-GW-01 --- X-Auth-Hotel propagation end-to-end}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{HMAC Signature estesa al Tenant Context \threat{T-GW-07}}
% ─────────────────────────────────────────────────────────────────────────────

\paragraph{Descrizione}
L'analisi post-implementazione di T-GW-06 ha rivelato un gap residuo nel meccanismo di firma
interna: la funzione \texttt{computeHmac} firmava solo la coppia \texttt{"username:role"},
lasciando l'header \texttt{X-Auth-Hotel} fuori dal payload autenticato.
La garanzia crittografica copriva l'identità dell'utente (\emph{chi sei}) ma non il contesto
di tenant (\emph{a quale hotel appartieni}).
Un servizio interno compromesso avrebbe potuto ricevere una richiesta legittima dal gateway,
modificare \texttt{X-Auth-Hotel} in transito e ri-inoltrarla a un altro servizio:
la firma sarebbe rimasta valida, consentendo di accedere a dati di un hotel arbitrario pur
avendo un JWT corretto.

\paragraph{OWASP} A01:2021 --- Broken Access Control (header tampering sul tenant context).

\begin{insecurebox}[title={Baseline --- HMAC su username:role soltanto}]
\begin{lstlisting}[style=java]
// api-gateway/AuthenticationFilter.java  (baseline)
private String computeHmac(String username, String role) {
    Mac mac = Mac.getInstance("HmacSHA256");
    mac.init(new SecretKeySpec(hmacSecret.getBytes(UTF_8), "HmacSHA256"));
    byte[] digest = mac.doFinal(
        (username + ":" + role).getBytes(UTF_8));  // hotelId ASSENTE
    return HexFormat.of().formatHex(digest);
}

// InternalAuthFilter (tutti i servizi)  (baseline)
private boolean isSignatureValid(String username, String role, String sig) {
    String expected = computeHmac(username, role);  // hotelId non verificato
    return MessageDigest.isEqual(
        expected.getBytes(UTF_8), sig.getBytes(UTF_8));
}
\end{lstlisting}
\end{insecurebox}

\begin{securebox}[title={Fix --- HMAC su username:role:hotelId}]
\begin{lstlisting}[style=java]
// api-gateway/AuthenticationFilter.java  (fix)
private String computeHmac(String username, String role, String hotelId) {
    Mac mac = Mac.getInstance("HmacSHA256");
    mac.init(new SecretKeySpec(hmacSecret.getBytes(UTF_8), "HmacSHA256"));
    byte[] digest = mac.doFinal(
        (username + ":" + role + ":" + hotelId).getBytes(UTF_8));
    return HexFormat.of().formatHex(digest);
}

// InternalAuthFilter (tutti i servizi)  (fix)
private boolean isSignatureValid(
        String username, String role, String hotelId, String sig) {
    String expected = computeHmac(username, role, hotelId);
    return MessageDigest.isEqual(
        expected.getBytes(UTF_8), sig.getBytes(UTF_8));
}
\end{lstlisting}
...
\paragraph{Test}
Aggiunto \texttt{shouldProduceDifferentSignatureForDifferentHotelId} in
\texttt{AuthenticationFilterTest}: costruisce due JWT con \texttt{hotelId} distinti
(\texttt{...001} e \texttt{...002}), esegue il filtro per entrambi e verifica che i valori
di \texttt{X-Internal-Signature} siano diversi, dimostrando che la firma ora dipende dal
tenant context.

\paragraph{Riferimento} Commit: \commitref{5bdc594}{fix(security): T-GW-07 --- HMAC extended to include hotelId in signed payload}
...
\texttt{api-gateway/src/main/java/com/hotelpms/gateway/util/JwtUtil.java}: l'annotazione
...
\paragraph{Riferimento} Commit: \commitref{92c0050}{fix(ci): add
...
\begin{itemize}
  \item \textbf{A01:2021} — Broken Access Control (IDOR, RBAC, accesso non autenticato)
  \item \textbf{A07:2021} — Identification \& Authentication Failures (JWT scaduto, brute-force 429)
  \item \textbf{A01:2021} — CSRF (403 su mutation)
\end{itemize}
...
\paragraph{Test di copertura}
Sono stati aggiunti 8 test JUnit~5 in
\texttt{fb-service/.../security/InternalAuthFilterTest.java}
...
\begin{itemize}
  ...
  \item \textbf{Provisioning automatico Grafana}: datasource Loki e dashboard
        ...
        \texttt{docker/grafana/provisioning/}, eliminando ogni configurazione
        manuale post-deploy.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Configurabilità per hotel --- Flyway V5}:
        la migration \texttt{V5\_\_add\_guest\_privacy\_settings.sql} crea la
        tabella \texttt{guest\_privacy\_settings} (una riga per hotel).
        \texttt{GuestPrivacySettingsController} espone
...
\paragraph{Stato}
\textbf{\textcolor{securegreen}{✅ RISOLTO}} --- commit \texttt{56109eb},
...
\begin{enumerate}
  \item \textbf{Login} (\texttt{POST /api/v1/auth/login}):
    verifica lockout → verifica password (BCrypt cost~12) → genera
...
\begin{center}
\small
\begin{tabular}{p{3.2cm} p{4cm} l l}
\toprule
...
Password debole in DB
  & BCrypt cost~12 + lazy rehash on login
  & A02 & \texttt{4e1f1e9} \\
Token theft (access)
  & TTL 15 min; cookie httpOnly/Secure/SameSite
  & A02/A07 & \texttt{5ce3010} \\
Token theft (refresh)
  & Rotation + blacklist JTI Redis; bulk revoca TV
  & A07 & \texttt{5ce3010}, \texttt{b473265} \\
...
% ─────────────────────────────────────────────────────────────────────────────
\section{DEP-CVE-01 --- Dipendenze vulnerabili (Netty + Spring Cloud Gateway)}

\subsection*{Contesto}

Il pipeline CI esegue \textbf{Trivy} su ogni immagine Docker costruita in fase di
\texttt{build}. Nella sessione del 2026-04-15, la scansione dell'immagine
\texttt{hotel-pms/api-gateway:ci} ha riportato \textbf{4 CVE HIGH} tutte con
patch disponibile (\texttt{status: fixed}):

\begin{center}
\small
\begin{tabular}{lllll}
\hline
\textbf{CVE} & \textbf{Libreria} & \textbf{Versione installata} & \textbf{Fix} & \textbf{Tipo} \\
\hline
CVE-2026-33870 & \texttt{netty-codec-http}         & 4.1.130.Final & 4.1.132.Final & Request smuggling \\
CVE-2026-33871 & \texttt{netty-codec-http2}        & 4.1.130.Final & 4.1.132.Final & HTTP/2 DoS \\
CVE-2025-41235 & \texttt{spring-cloud-gateway-server} & 4.2.0      & 4.2.3         & Header forwarding \\
CVE-2025-41253 & \texttt{spring-cloud-gateway-server} & 4.2.0      & 4.2.6         & EL injection \\
\hline
\end{tabular}
\end{center}
...
\begin{insecurebox}[title={Netty 4.1.130 --- request smuggling e HTTP/2 DoS (gateway ingress)}]
\textbf{CVE-2026-33870} (\texttt{netty-codec-http}): parsing scorretto del
\emph{chunked transfer encoding} di HTTP/1.1 consente a un attaccante di
iniettare richieste aggiuntive nella stessa connessione TCP (\emph{HTTP request
smuggling}), potenzialmente bypassando la logica di routing e autenticazione del
gateway.

\medskip
\textbf{CVE-2026-33871} (\texttt{netty-codec-http2}): un \emph{CONTINUATION frame
flood} nel protocollo HTTP/2 consuma memoria illimitata sul lato server, producendo
un Denial of Service. Il gateway è il punto di ingresso unico di tutto il traffico
esterno: un DoS qui rende irraggiungibili tutti i microservizi del PMS.
\end{insecurebox}

\begin{insecurebox}[title={Spring Cloud Gateway 4.2.0 --- header forwarding e EL injection}]
\textbf{CVE-2025-41235}: il gateway inoltra header \texttt{X-Forwarded-*} ricevuti
da proxy non fidati senza validazione. Nel sistema PMS questo potrebbe permettere
a un attaccante di sovrascrivere \texttt{X-Auth-User} / \texttt{X-Auth-Hotel} —
gli stessi header che i microservizi interni usano per il controllo degli accessi
multi-tenant.

\medskip
\textbf{CVE-2025-41253}: un input non sanificato nelle configurazioni di routing
di Spring Cloud Gateway WebFlux può essere interpretato come \emph{Spring
Expression Language}, portando a Remote Code Execution sul gateway o all'esfiltrazione
di segreti dall'application context (chiave JWT, HMAC secret, credenziali Redis).
\end{insecurebox}

\subsection*{Intervento}

\begin{securebox}[title={Netty 4.1.132.Final via Spring Boot BOM property override}]
La proprietà \texttt{netty.version} esposta dal plugin
\texttt{io.spring.dependency-management} è stata sovrascritta nel blocco
\texttt{ext} di \texttt{api-gateway/build.gradle.kts}:

\begin{verbatim}
set("netty.version", "4.1.132.Final")
\end{verbatim}

Questa tecnica forza l'intero grafo delle dipendenze Netty (codec-http,
codec-http2, buffer, common, transport, \ldots) alla versione patchata,
senza alterare il BOM Spring Cloud né le dipendenze degli altri moduli.
\end{securebox}

\begin{securebox}[title={Spring Cloud Gateway 4.2.6 via dependencyManagement override}]
Le coordinate \texttt{spring-cloud-gateway-server} e
\texttt{spring-cloud-starter-gateway} sono state fissate a \texttt{4.2.6}
tramite il blocco \texttt{dependencies} \{\ \} all'interno di
\texttt{dependencyManagement}:

\begin{verbatim}
dependencyManagement {
    imports { mavenBom("...spring-cloud-dependencies:2024.0.0") }
    dependencies {
        dependency("...spring-cloud-gateway-server:4.2.6")
        dependency("...spring-cloud-starter-gateway:4.2.6")
    }
}
\end{verbatim}

Questo override ha priorità sul BOM importato, aggiornando il gateway al
rilascio che corregge entrambe le CVE (\texttt{4.2.3} per CVE-2025-41235,
\texttt{4.2.6} per CVE-2025-41253) senza toccare il resto del train
\texttt{2024.0.0}.
\end{securebox}

\begin{securebox}[title={Impatto sulla postura di sicurezza}]
Dopo il commit \texttt{14d29b9}:
\begin{itemize}
  \item La scansione Trivy non rileva più CVE HIGH/CRITICAL nell'immagine
        \texttt{api-gateway:ci} (tutte e 4 le segnalazioni risolte per fix
        di versione, non per soppressione).
  \item L'unica CVE precedentemente soppressa (\texttt{CVE-2026-22732},
        Spring Security) rimane in \texttt{.trivyignore} con giustificazione
        documentata in attesa di Spring Boot 3.5.x.
  \item Il meccanismo di override BOM è verificabile e reversibile:
        rimuovere le due righe nel \texttt{build.gradle.kts} ripristina il
        comportamento del BOM originale.
\end{itemize}
\end{securebox}

\subsection*{Nota — CI SCA: cache NVD e chiave API}

In parallelo alla risoluzione delle CVE, è stato corretto un problema di
performance nel job CI \texttt{sca} (OWASP Dependency Check): la chiave
di cache per il database NVD utilizzava \texttt{run\_id}, unico per ogni
esecuzione, rendendo di fatto inutile la cache e causando il ri-scaricamento
dell'intero database (~200 MB) a ogni run, con tempi superiori a un'ora.
La chiave è stata corretta in formato \texttt{nvd-<os>-<YYYY-MM-DD>},
condivisa tra tutte le esecuzioni dello stesso giorno (commit \texttt{d1a7cdc}).

Era stato inoltre previsto di registrare una \textbf{NVD API Key} gratuita
(portale NIST) per aumentare il rate limit delle chiamate NVD da
1\,req/6\,s a 50\,req/30\,s, riducendo il tempo di prima esecuzione da
circa 15 minuti a circa 3 minuti. L'implementazione è stata progettata
e il codice predisposto (lettura da env var \texttt{NVD\_API\_KEY} →
property \texttt{nvdApiKey} del plugin Gradle), ma \textbf{non è stato
possibile completare la registrazione}: al momento dell'accesso, il
portale NVD di NIST risultava in manutenzione e non accettava nuove
richieste di chiave. Il codice di integrazione è stato quindi rimosso
per mantenere la configurazione pulita; l'ottimizzazione rimane
applicabile non appena il servizio torna disponibile.

% ─────────────────────────────────────────────────────────────────────────────
\section{DEP-CVE-02 --- Commons FileUpload + Commons IO (tutti i servizi)}

\textbf{Commit}: \texttt{e9b6eb6} — branch \texttt{feature/secure-coding-hardening}

\subsection*{CVE coinvolte}

\begin{center}
\small
\begin{tabular}{lllll}
\hline
\textbf{CVE} & \textbf{Libreria} & \textbf{Vers. vuln.} & \textbf{Fix} & \textbf{Severity} \\
\hline
CVE-2025-48976 & commons-fileupload & 1.5 & 1.6.0 & HIGH \\
CVE-2024-47554  & commons-io         & 2.11.0 & 2.14.0 & HIGH \\
\hline
\end{tabular}
\end{center}
...
\begin{insecurebox}[title={CVE-2025-48976 --- Apache Commons FileUpload DoS}]
Un attaccante può inviare una richiesta multipart con un numero elevato di
header per singola parte (\emph{part headers}), causando un consumo di CPU e
memoria sproporzionato nel parsing (Denial of Service). Tutte le 8 immagini
Docker dei microservizi includevano la versione 1.5 come dipendenza transitiva
di Spring Boot 3.4.x.
\end{insecurebox}

\begin{insecurebox}[title={CVE-2024-47554 --- Apache Commons IO XmlStreamReader DoS}]
\texttt{XmlStreamReader} non impone un limite alla dimensione del BOM/encoding
declaration durante il parsing di input non fidato. Un input malevolo può
bloccare il thread reader in un loop che consuma tutta la memoria disponibile.
La versione 2.11.0 era inclusa transitivamente in tutti gli 8 servizi.
\end{insecurebox}

\subsection*{Risoluzione}

\begin{securebox}[title={dependencyManagement.dependencies su tutti gli 8 microservizi}]
\texttt{commons-fileupload} e \texttt{commons-io} \emph{non} sono gestiti
dal BOM di Spring Boot 3.4.x: \texttt{CommonsMultipartResolver} è stato
rimosso in Spring Framework 6.1, portando all'eliminazione di queste librerie
dalla BOM di gestione. Di conseguenza, la tecnica
\texttt{ext\{set("commons-fileupload.version", "1.6.0")\}} non ha effetto sul
grafo delle dipendenze transitorie.

La risoluzione corretta utilizza il blocco
\texttt{dependencyManagement \{ dependencies \{ ... \} \}} del plugin
\texttt{io.spring.dependency-management}, che forza la versione
indipendentemente dalla provenienza del transitive pull:
...
Questo è lo stesso meccanismo usato in \texttt{api-gateway} per forzare
\texttt{spring-cloud-gateway-server:4.2.6} (DEP-CVE-01). Applicato a tutti
gli 8 microservizi (auth, billing, config, fb, guest, inventory, reservation,
stay).
\end{securebox}

\subsection*{Rimozione del job SCA (OWASP Dependency Check)}

Il job CI \texttt{sca} (OWASP Dependency Check) è stato rimosso dal workflow
\texttt{ci.yml} per le seguenti ragioni:

\begin{itemize}
  \item Il database NVD di NIST risultava in manutenzione per periodi prolungati,
        rendendo impossibile il download iniziale del DB e causando timeout
        superiori a un'ora.
  \item La scansione Trivy (\texttt{aquasecurity/trivy-action}) copre già
        l'obiettivo A06 con un database di vulnerabilità aggiornato
        autonomamente e tempi di esecuzione nell'ordine dei secondi.
  \item I due strumenti si sovrapponevano sulla stessa categoria di rischio
        (componenti vulnerabili) senza complementarità pratica nel contesto
        di questo progetto.
\end{itemize}

La copertura OWASP A06 rimane garantita dalla pipeline Trivy (matrice ×8
servizi + 3 immagini third-party), che ha rilevato e verificato la risoluzione
di entrambe le CVE descritte in questa sezione.

% ─────────────────────────────────────────────────────────────────────────────
\section{DEP-CVE-03 --- Spring Cloud Config Server (CVE-2026-22739)}
...
\textbf{CVE} & \textbf{Libreria} & \textbf{Vers. vuln.} & \textbf{Fix} & \textbf{Severity} \\
\hline
CVE-2026-22739 & spring-cloud-config-server & 4.2.0 & 4.3.2 / 5.0.2 & HIGH \\
\hline
\end{tabular}
\end{center}
...
\begin{insecurebox}[title={CVE-2026-22739 --- Path Traversal via Profile Parameter}]
Un attaccante con accesso all'endpoint HTTP del Config Server può fornire un
valore artigianale per il parametro \texttt{profile} che contiene sequenze di
path traversal (\texttt{../}). Il server non sanifica sufficientemente il
parametro prima di costruire il percorso del file di configurazione, permettendo
la lettura di file arbitrari dal filesystem del container
(\emph{Arbitrary File Read}).
\end{insecurebox}

\subsection*{Rischio residuale e mitigazione compensativa}

La versione corretta (4.3.2) appartiene alla release train Spring Cloud
2025.0.x, incompatibile con Spring Boot 3.4.x (utilizzato per garantire la
compatibilità con Spring Cloud 2024.0.x). Non è possibile aggiornare solo
\texttt{spring-cloud-config-server} senza rischiare incompatibilità nel grafo
BOM.

\begin{securebox}[title={Mitigazioni compensative}]
\begin{itemize}
  \item \textbf{Isolamento di rete}: il config-service (porta 8888) non è
        instradato attraverso l'API Gateway e non è accessibile dall'esterno.
        Solo i microservizi interni con la corretta configurazione Spring Cloud
        Config Client possono effettuare richieste.
  \item \textbf{Nessun file sensibile nel container}: il filesystem del
        container non contiene credenziali o file di sistema rilevanti oltre
        ai config YAML già serviti.
  \item \textbf{Soppressione documentata}: la CVE è stata aggiunta a
        \texttt{.trivyignore} con giustificazione completa, in modo che la
        pipeline CI rimanga verde e il rischio resti tracciato nel
        \texttt{THREAT\_MODEL.md}.
  \item \textbf{Piano di migrazione}: la CVE sarà risolta con il prossimo
        aggiornamento a Spring Boot 3.5.x + Spring Cloud 2025.0.x.
\end{itemize}
\end{securebox}

% ─────────────────────────────────────────────────────────────────────────────
\section{DEP-CVE-04 --- Upgrade Spring Boot 3.5.x, CVE residua Netty epoll e triage alert Trivy}

\textbf{Commit}: \texttt{bbb7a7e} — branch \texttt{main}

\subsection*{Aggiornamento Spring Boot 3.5.x + Spring Cloud 2025.0.x}

A valle della chiusura del branch \texttt{feature/secure-coding-hardening},
tutti i 9 microservizi sono stati aggiornati da Spring Boot \texttt{3.4.x}
a Spring Boot \texttt{3.5.14} e da Spring Cloud \texttt{2024.0.0} a
\texttt{2025.0.0}. L'aggiornamento risolve:

\begin{itemize}
  \item \textbf{CVE-2026-22739} (config-server path traversal, DEP-CVE-03):
        la versione patchata \texttt{spring-cloud-config-server~4.3.x} è
        inclusa nel BOM \texttt{2025.0.0}; le mitigazioni compensative
        documentate in DEP-CVE-03 rimangono valide come defence-in-depth.
  \item \textbf{CVE-2025-41253} (Spring Cloud Gateway EL injection):
        \texttt{spring-cloud-gateway-server} aggiornato a \texttt{4.3.2}
        tramite override esplicito in \texttt{dependencyManagement}
        (la versione base del BOM 2025.0.0 è 4.3.0, non ancora patchata).
  \item \textbf{CVE-2026-42583/42584/42579/42587} (Netty codec):
        Netty fissato a \texttt{4.1.133.Final} via proprietà BOM.
  \item \textbf{CVE-2026-42198} (PostgreSQL JDBC): driver aggiornato a
        \texttt{42.7.11} tramite \texttt{dependencyManagement}.
  \item \textbf{CVE-2026-5598} (BouncyCastle): aggiornato a
        \texttt{bcprov-jdk18on:1.84}.
\end{itemize}

\medskip
\textbf{Breaking change gestita}: Spring Boot 3.5.x proibisce il proxying
CGLIB di classi \texttt{final} per AOP transazionale. I controller
\texttt{GuestController} e \texttt{GuestPrivacySettingsController} (che
usavano il modificatore \texttt{final}) sono stati resi non-final per
consentire l'advice \texttt{@Transactional}.

% ─────────────────────────────────────────────────────────────────────────────
\subsection*{CVE residua aperta: CVE-2026-42577 (Netty native epoll)}

\begin{center}
\small
\begin{tabular}{lllll}
\hline
\textbf{CVE} & \textbf{Libreria} & \textbf{Versione installata} & \textbf{Fix disponibile} & \textbf{Severity} \\
\hline
CVE-2026-42577 & \texttt{netty-transport-native-epoll} & 4.1.133.Final & Netty 4.2.x & HIGH \\
\hline
\end{tabular}
\end{center}

\begin{insecurebox}[title={CVE-2026-42577 --- DoS via RST su connessione TCP half-closed}]
Il transport \texttt{netty-transport-native-epoll} non gestisce correttamente
un segmento TCP RST ricevuto su una connessione in stato \emph{half-closed}
(shutdown di scrittura già effettuato). Un attaccante può indurre il server
a entrare in un loop di CPU-spin o a sprecare risorse di memoria fino al
riavvio del processo (\emph{Denial of Service}).
\end{insecurebox}

\begin{securebox}[title={Rischio pratico: nullo nell'attuale configurazione}]
Il JAR \texttt{netty-transport-native-epoll} è presente nel classpath dell'API
Gateway come dipendenza transitiva di Project Reactor/Netty, ma il trasporto
attivo in Spring Boot è \textbf{JDK NIO} (selezionato automaticamente in
assenza di native library pre-loading). Il codice che attiverebbe il native
epoll (\texttt{NioEventLoopGroup} vs \texttt{EpollEventLoopGroup}) non viene
mai eseguito nella configurazione standard di Spring WebFlux.

\medskip
\textbf{Perché il fix non è applicato}: la correzione richiede Netty
\texttt{4.2.x} (minor version bump), incompatibile con il BOM di Spring Boot
3.5.x che gestisce la serie \texttt{4.1.x}. Un override a \texttt{4.2.x}
introdurrebbe incompatibilità non garantite nel layer I/O del gateway.

\medskip
\textbf{Stato nel Security tab}: l'alert \textbf{\#296} rimane aperto come
unico alert nella categoria \emph{nostra codebase}. La decisione di non
sopprimerlo è intenzionale: mantenerlo visibile documenta il debito tecnico
e ricorda il piano di aggiornamento previsto al passaggio a Kubernetes
(dove Netty 4.2.x potrà essere adottato in un contesto BOM più recente).
\end{securebox}

% ─────────────────────────────────────────────────────────────────────────────
\subsection*{Triage alert Trivy --- immagini terze parti (Grafana)}

Il pipeline CI esegue scansioni Trivy su tutte le immagini Docker, incluse
le immagini di infrastruttura (\texttt{grafana/grafana},
\texttt{postgres:15-alpine}, \texttt{redis:alpine}).
L'aggiornamento di Grafana da \texttt{10.2.0} a \texttt{11.5.0}
(commit \texttt{bbb7a7e}) ha ridotto il numero di CVE nell'immagine, ma
non le ha eliminate del tutto: il binary di Grafana incorpora un runtime
Go e librerie Alpine (OpenSSL, musl, zlib) con CVE accumulate che non
dipendono dalla versione di Grafana stessa bensì dall'immagine base Alpine.

\begin{center}
\small
\begin{tabular}{llr}
\hline
\textbf{Immagine} & \textbf{Componente vulnerabile} & \textbf{Alert} \\
\hline
\texttt{grafana/grafana:11.5.0} (layer Alpine) & OpenSSL, musl libc, curl, zlib & 22 \\
\texttt{grafana/grafana:11.5.0} (binary Go)    & Go runtime, librerie grafana & 77 \\
\texttt{postgres:15-alpine} (\texttt{gosu})    & Go runtime in gosu binary & 9 \\
\hline
\textbf{Totale terze parti} & & \textbf{108} \\
\hline
\end{tabular}
\end{center}

\begin{securebox}[title={Decisione di triage: dismiss documentato con motivazione tecnica}]
Tutti i 108 alert di terze parti sono stati \textbf{dismissi} nel Security
tab di GitHub con stato \texttt{won't fix} e la seguente motivazione,
registrata permanentemente nell'audit trail di GitHub:

\begin{quote}
\emph{``Grafana 11.5.0 monitoring tool — internal port 3000, not
internet-exposed, no guest PII. CVEs are in bundled Alpine/Go runtime,
not in hotel PMS code. Accepted risk for pilot; reviewed at Kubernetes
migration.''}
\end{quote}

\textbf{Perché il dismiss è la scelta corretta} (e non l'upgrade a una
versione più recente di Grafana):
\begin{itemize}
  \item Grafana è un tool di monitoring \emph{interno}: porta 3000 non
        instradabile dall'esterno, zero dati PII degli ospiti.
  \item Le CVE più critiche nel nome
        (es.\ CVE-2026-28387 ``RCE via use-after-free in DANE TLSA'',
        CVE-2025-69419 ``RCE via out-of-bounds write in PKCS\#12'')
        richiedono che un attaccante invii payload costruiti ad hoc
        \emph{direttamente a Grafana}, impossibile dall'esterno.
  \item Le vulnerabilità appartengono all'immagine base Alpine del
        binary Grafana: anche aggiornando la versione di Grafana il problema
        persiste finché Alpine non aggiorna le proprie librerie di sistema.
  \item In fase Kubernetes, Grafana sarà isolato da \texttt{NetworkPolicy}
        che blocca tutto il traffico non autorizzato verso il namespace
        monitoring.
\end{itemize}
\end{securebox}

\begin{securebox}[title={Stato finale del Security tab (GitHub) al 2026-05-10}]
\begin{itemize}
  \item \textbf{Alert aperti}: 1 (\#296 — CVE-2026-42577, Netty native
        epoll, rischio pratico nullo, debito tecnico tracciato).
  \item \textbf{Alert dismissi ``won't fix''}: 108 (Grafana + postgres/gosu,
        tutti terze parti, motivazione documentata).
  \item \textbf{Alert dismissi ``false positive''}: 1
        (CSRF Spring Security — implementazione Double Submit Cookie custom).
  \item \textbf{Dependabot alerts}: 0 (dipendenze Maven e npm senza CVE
        attive dopo l'aggiornamento Spring Boot 3.5.x).
\end{itemize}

Il Security tab contiene un solo alert aperto, appartenente alla categoria
\emph{nostra codebase}, con spiegazione tecnica precisa e piano di
risoluzione documentato. Questo è il risultato atteso di un processo di
triage maturo: non l'assenza di alert, ma la comprensione e la
contestualizzazione di ciascuno.
\end{securebox}

% ─────────────────────────────────────────────────────────────────────────────
\section{Copertura OWASP Top 10 (2021) — Riepilogo}

La tabella seguente mostra la copertura complessiva raggiunta al termine
di tutte le sessioni di hardening, aggiornata alla revisione del
2026-04-15.
...
\hline
\textbf{Categoria} & \textbf{Nome} & \textbf{Interventi} \\
\hline
\owasp{A01} & Broken Access Control
  & T-GST-01/03, T-RES-02, T-BILL-01, T-FB-01, T-FE-03,
    T-GW-05/06/07, GAP-3 \\
\owasp{A02} & Cryptographic Failures
  & T-AUTH-03 (BCrypt cost 12), T-CFG-02 (segreti env), T-STAY-03 (TLS) \\
\owasp{A03} & Injection
  & T-GST-02 (Bean Validation pattern), T-RES-03 (date validation),
    T-FE-01 (XSS ESLint rule) \\
\owasp{A04} & Insecure Design
  & T-RES-01 (optimistic lock), T-STAY-01 (state allowlist),
    T-FB-02 (server-side pricing), T-BILL-02 (amount normalisation),
    T-GST-04 (mass-assignment), T-GST-05 (GDPR retention) \\
\owasp{A05} & Security Misconfiguration
  & T-GW-02 (rate limiting), T-GW-03 (security headers),
    T-GW-04 (CORS whitelist), T-CFG-01 (actuator port),
    T-CFG-03 (config-service Basic Auth), T-FE-04 (CSP nginx) \\
\owasp{A06} & Vulnerable and Outdated Components
  & DEP-CVE-01 (Netty 4.1.132, Gateway 4.2.6),
    DEP-CVE-02 (commons-fileupload 1.6.0, commons-io 2.14.0),
    DEP-CVE-03 (config-server CVE-2026-22739, risolto con Spring Boot 3.5.x),
    DEP-CVE-04 (Spring Boot 3.5.14 + Spring Cloud 2025.0.0; CVE-2026-42577
    Netty epoll rischio residuale documentato; 108 alert Grafana/terze parti
    triaggiati e dismissi) \\
\owasp{A07} & Identification \& Authentication Failures
  & T-AUTH-01 (no enumeration), T-AUTH-02 (lockout),
    T-AUTH-03 (BCrypt), T-AUTH-04 (token rotation + revocation),
    T-FE-02 (httpOnly cookie) \\
\owasp{A08} & Software and Data Integrity Failures
  & \textit{n/a — Gradle dependency verification e signed artifacts
    fuori scope esame} \\
\owasp{A09} & Security Logging \& Monitoring Failures
  & T-AUTH-05 (audit log auth), T-STAY-02 (audit log stay),
    T-BILL-03 (audit log billing), GAP-4 (Loki SIEM aggregation) \\
\owasp{A10} & Server-Side Request Forgery
  & \textit{n/a — nessun endpoint accetta URL controllati dal client} \\
\hline
\end{tabular}
\end{center}

\medskip
Su 10 categorie OWASP Top 10 (2021), \textbf{8 sono coperte} da almeno
un intervento verificato e committato. A08 e A10 non si applicano
all'architettura corrente del sistema.
