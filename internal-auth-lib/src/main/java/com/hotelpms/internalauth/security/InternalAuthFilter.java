 ...
 * <ol>
 * <li><b>Presence check</b> – rejects requests missing (or blank) any of the
 * required headers: {@code X-Auth-User}, {@code X-Auth-Role},
 * {@code X-Auth-Hotel}, {@code X-Internal-Signature},
 * {@code X-Auth-Timestamp}, or {@code X-Auth-Nonce}.</li>
 ...
 * signature, making it impossible for any internal caller to spoof
 * {@code X-Auth-User} / {@code X-Auth-Role} headers without intercepting the
 * secret.
 *
 * <p>
 * Shared across every microservice (AUDIT_ANALISI_2026-07.md item 2 —
 * previously 6 near-identical copies, one of them, auth-service's, genuinely
 * divergent). The only remaining per-service axis is
 * {@link #excludedPathPrefixes}: most services exempt only {@code /actuator};
 * auth-service additionally exempts its public unauthenticated endpoints
 * ({@code /login}, {@code /register}, ...), which are called directly by the
 * frontend rather than routed through the gateway.
 */
public final class InternalAuthFilter extends OncePerRequestFilter {
     ...
     *                              exactly the value configured in the API
     *                              Gateway
     * @param nonceStore            the store used to detect re-used nonces
     *                              (T-GW-08)
     * @param excludedPathPrefixes  request URI prefixes that bypass HMAC
     *                              validation entirely (e.g. {@code /actuator}
     *                              so Docker health checks and Prometheus
     *                              scraping can reach it without gateway
     *                              headers, plus any endpoints the frontend
     *                              calls directly rather than via the gateway)
     */
     ...
     * evaluation, preventing timing-based side-channel attacks.
     *
     ...
     *                  header
     * @return {@code true} if the recomputed digest matches the received
     *         signature
     */
