...
/**
 * Canonical unit tests for the shared {@link InternalAuthFilter}
 * (AUDIT_ANALISI_2026-07.md item 2 — extracted from 6 near-identical
 * per-service copies into {@code internal-auth-lib}).
 *
 * <p>Every consuming service also keeps its own (now much shorter)
 * characterization test asserting the class behaves correctly when
 * constructed with its own {@code excludedPathPrefixes}; this suite is the
 * one source of truth for the shared logic itself, including the
 * multi-prefix exclusion behavior no single service exercises alone.
 */
class InternalAuthFilterTest {
    ...
    private static final String PROTECTED_PATH = "/api/v1/protected";

    private static final List<String> EXCLUDED_PATH_PREFIXES = List.of("/actuator", "/api/v1/public");
    ...
    private static String filterHmacMaterial() {
        return "unit-test-internal-auth-filter-shared-lib";
    }
    ...
    private static MockHttpServletRequest buildRequest(
        ...
        return buildRequest(PROTECTED_PATH, user, role, hotelId, timestamp, nonce, signature);
    }

    private static MockHttpServletRequest buildRequest(final String path,
            final String user, final String role, final String hotelId,
            final String timestamp, final String nonce, final String signature) {
    ...
    @Nested
    @DisplayName("shouldNotFilter — multi-prefix exclusion")
    class ExcludedPathPrefixes {

        @Test
        @DisplayName("Request matching the first excluded prefix bypasses the filter")
        void shouldBypassFilterForFirstExcludedPrefix() throws IOException, ServletException {
            final MockHttpServletRequest request = new MockHttpServletRequest();
        ...
        @Test
        @DisplayName("Request matching the second excluded prefix bypasses the filter")
        void shouldBypassFilterForSecondExcludedPrefix() throws IOException, ServletException {
            final MockHttpServletRequest request = new MockHttpServletRequest();
            request.setRequestURI("/api/v1/public/status");
            final MockHttpServletResponse response = new MockHttpServletResponse();
        ...
        @Test
        @DisplayName("Request matching no excluded prefix is enforced")
        void shouldEnforceOnNonExcludedPath() throws IOException, ServletException {
            final MockHttpServletRequest request = new MockHttpServletRequest();
    ...
    @Nested
    @DisplayName("Presence check — missing or blank header → 401 JSON body")
    class PresenceCheck {

        @Test
        @DisplayName("Missing X-Auth-User → 401 with JSON error body")
        void shouldRejectWhenUsernameMissing()
            ...
            assertThat(response.getContentAsString())
                    .isEqualTo("{\"error\":\"Missing required gateway authentication headers\"}");
        }

        @Test
        @DisplayName("Blank X-Auth-User (present but empty) → 401")
        ...
        void shouldRejectWhenNonceMissing() throws IOException, ServletException {
            final String timestamp = String.valueOf(System.currentTimeMillis());
            final MockHttpServletRequest request =
    ...
    class HmacCheck {
        ...
        @Test
        @DisplayName("Valid headers on a non-excluded path → passes chain and populates SecurityContext")
