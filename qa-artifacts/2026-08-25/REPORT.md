...
| 6 — Interruzioni | 🔄 in corso | notification-service reale spento/riavviato via `docker stop` — circuit breaker Resilience4j confermato funzionante (checkout mai bloccato); scoperto gap ambientale `mailpit` mancante (vedi sopra). Fault injection UI (`06-interruptions.spec.ts`, 4/4 verdi): abort+retry ospite, 500 su prenotazione, doppio-click, CSRF rimosso. Fault injection API (`07-service-resilience.spec.ts`, 5/5 verdi): pagamento midflight, ordine F&B doppio, submit Alloggiati midflight, concorrenza su modifica prenotazione — **2 difetti 🔴 trovati e risolti** (vedi sopra: prezzo mai salvato su modifica prenotazione, falso conflitto camera). Mancano ancora: sessione scaduta a metà form, back-navigation durante POST in volo, `docker stop` di frontdesk/billing/guest/fb/auth uno alla volta |
