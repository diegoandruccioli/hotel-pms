...
| ✅ N/A | 12 | ~~Seed admin (`admin`/`password`) inserito incondizionatamente da Flyway `V1__init_schema.sql` con `must_change_password=FALSE`, nessun warning di avvio~~ — **verificato non riproducibile**: `V5__add_owner_role_and_must_change_password.sql:29-32` forza già `TRUE` sulla stessa riga, subito dopo la `ALTER TABLE` citata dall'audit; falso positivo da lettura parziale del file, vedi `misconfig.md` §5 | `V1__init_schema.sql:64-89`, `V5__...sql:23-32` | misconfig |
